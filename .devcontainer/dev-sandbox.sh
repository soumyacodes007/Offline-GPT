...
# Start the OfflineGPT desktop dev stack inside a Daytona/devcontainer sandbox.
...
if [ "${1:-}" = "--help" ] || [ "${1:-}" = "-h" ]; then
  printf '%s\n' \
    ...
    "" \
    "Starts the OfflineGPT desktop dev stack inside a Daytona/devcontainer sandbox." \
...
cd "$REPO_DIR"
export OFFLINEGPT_WORKSPACE_DIR="$REPO_DIR"
...
echo "==> Starting OfflineGPT dev stack in background..."
bash "$ELECTRON_SCRIPT" --detach

echo "==> Waiting for Electron CDP on :$OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT..."
for _ in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:$OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT/json/list" >/dev/null 2>&1; then
    echo "OfflineGPT sandbox dev stack is ready."
