...
import {
  callOfflineGPTCloudUploadAction,
  OFFLINEGPT_CLOUD_UPLOAD_ACTIONS,
  OFFLINEGPT_CLOUD_UPLOADS_EXTENSION_ID,
} from "./cloud-uploads.js";

const OFFLINEGPT_EXPERIMENTAL_EXTENSION_ACTIONS = [
  ...GOOGLE_WORKSPACE_EXTENSION_ACTIONS,
  ...OPENAI_IMAGE_GENERATION_EXTENSION_ACTIONS,
  ...OFFLINEGPT_CLOUD_UPLOAD_ACTIONS,
];
...
export function listExperimentalExtensionActions(extensionId: string, connectSnapshot?: ConnectSnapshot) {
  ...
  const actions = filter
    ? OFFLINEGPT_EXPERIMENTAL_EXTENSION_ACTIONS.filter((action) => action.extensionId === filter)
    : OFFLINEGPT_EXPERIMENTAL_EXTENSION_ACTIONS;
...
export async function callExperimentalExtensionAction(config: ServerConfig, env: EnvService, input: unknown, connectSnapshot?: ConnectSnapshot) {
  ...
  const registered = OFFLINEGPT_EXPERIMENTAL_EXTENSION_ACTIONS.find((item) => item.extensionId === extensionId && item.action === action);
  if (!registered) {
    throw new ApiError(404, "extension_action_not_found", "OfflineGPT extension action not found");
  }
  ...
  if (extensionId === OFFLINEGPT_CLOUD_UPLOADS_EXTENSION_ID) {
    const result = await callOfflineGPTCloudUploadAction(config, action, args, context);
    if (result) return result;
  }

  throw new ApiError(501, "extension_action_not_implemented", `${registered.title} is registered but not implemented on offlinegpt-server yet.`, { extensionId, action, args });
}
