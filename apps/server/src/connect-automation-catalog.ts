...
export type OfflineGPTAutomationIndex = z.infer<typeof automationIndexSchema>;

const catalogCache = new Map<string, { expiresAt: number; value: Promise<OfflineGPTAutomationIndex | null> }>();

async function readIndex(cloud: Record<string, unknown>, fetcher: McpFetch): Promise<OfflineGPTAutomationIndex | null> {
  const text = await readMcpResourceText({
    config: cloud,
    uri: AUTOMATION_INDEX_URI,
    fetcher,
    clientName: "offlinegpt-server-automation-catalog",
  });
...
/**
 * Resolve the member's Automation index from the first working offlinegpt-cloud
 ...
 */
export async function readOfflineGPTAutomationCatalog(
  config: ServerConfig,
  fetcher: McpFetch = externalFetch,
): Promise<OfflineGPTAutomationIndex | null> {
  try {
...
export function resetOfflineGPTAutomationCatalogCacheForTests(): void {
  catalogCache.clear();
}

function scheduleText(schedule: OfflineGPTAutomationIndex["automations"][number]["schedule"]): string {
 ...
 */
export function renderOfflineGPTAutomationInstruction(index: OfflineGPTAutomationIndex | null): string {
  if (!index) return "";
  if (index.automations.length === 0) {
    return [
      "This member owns no Automations. If they ask what Automations they have, say there are none.",
      "If they describe recurring work, propose one with offlinegpt_execute id automation.propose.",
    ].join("\n");
  }
