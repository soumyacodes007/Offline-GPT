...
function resolveOfflineGptDataDir(): string {
  return offlinegptServerDataDir();
}

export function auditLogPath(workspaceId: string): string {
  return join(resolveOfflineGptDataDir(), "audit", `${workspaceId}.jsonl`);
}

export function legacyAuditLogPath(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "offlinegpt", "audit.jsonl");
}
