...
export async function resolveServerConfig(cli: CliArgs): Promise<ServerConfig> {
  const configPath = cli.configPath ?? offlinegptServerConfigPath();
  const fileConfig = await loadFileConfig(configPath);
  const configDir = dirname(configPath);

  const envWorkspaces = parseList(process.env.OFFLINEGPT_WORKSPACES);
  ...
  const envOpencodeBaseUrl = process.env.OFFLINEGPT_OPENCODE_BASE_URL;
  const envOpencodeDirectory = process.env.OFFLINEGPT_OPENCODE_DIRECTORY;
  const envOpencodeUsername = process.env.OFFLINEGPT_OPENCODE_USERNAME;
  const envOpencodePassword = process.env.OFFLINEGPT_OPENCODE_PASSWORD;
  ...
  const tokenFromEnv = process.env.OFFLINEGPT_TOKEN;
  const hostTokenFromEnv = process.env.OFFLINEGPT_HOST_TOKEN;
  ...
  const approvalMode =
    cli.approvalMode ??
    (process.env.OFFLINEGPT_APPROVAL_MODE as ApprovalMode | undefined) ??
    fileConfig.approval?.mode ??
    "manual";

  const approvalTimeoutMs =
    cli.approvalTimeoutMs ??
    (process.env.OFFLINEGPT_APPROVAL_TIMEOUT_MS ? Number(process.env.OFFLINEGPT_APPROVAL_TIMEOUT_MS) : undefined) ??
  ...
  const envCorsOrigins = process.env.OFFLINEGPT_CORS_ORIGINS;
  ...
  const envReadOnly = process.env.OFFLINEGPT_READONLY;
  ...
  const envLogFormat = process.env.OFFLINEGPT_LOG_FORMAT;
  const logFormat =
    cli.logFormat ??
  ...
  const envLogRequests = parseBoolean(process.env.OFFLINEGPT_LOG_REQUESTS);
  ...
  const host = cli.host ?? process.env.OFFLINEGPT_HOST ?? fileConfig.host ?? DEFAULT_HOST;
  const port = cli.port ?? (process.env.OFFLINEGPT_PORT ? Number(process.env.OFFLINEGPT_PORT) : undefined) ?? fileConfig.port ?? DEFAULT_PORT;
