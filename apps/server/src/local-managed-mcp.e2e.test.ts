...
import { EnterpriseMcpClientError } from "@offlinegpt/enterprise-mcp-client";
...
async function connectGateway(runtimeConfig: Record<string, unknown>): Promise<Client> {
  ...
  const client = new Client({ name: "offlinegpt-managed-mcp-test", version: "1.0.0" }, { capabilities: {} });
  await client.connect(new StreamableHTTPClientTransport(url, { requestInit: { headers } }));
  return client;
}

describe("OfflineGPT-managed local MCP OAuth gateway", () => {
  test("leaves ordinary MCP fallbacks usable when no managed vault exists", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "offlinegpt-local-managed-mcp-fallback-"));
  ...
  test("rolls back a new managed connection when the initial OAuth handshake fails", async () => {
    ...
    const workspaceRoot = await mkdtemp(join(tmpdir(), "offlinegpt-local-managed-mcp-rollback-"));
    roots.push(workspaceRoot);
    ...
    try {
      ...
      stops.push(() => server.stop());
      const offlinegptBaseUrl = `http://127.0.0.1:${server.port}`;

      const created = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/managed`, {
        method: "POST",
      ...
      expect(await created.json()).toMatchObject({
        ...
        message: "OfflineGPT could not connect to this MCP server. Check its OAuth settings and availability, then try again.",
      });

      const status = await fetch(
        `${offlinegptBaseUrl}/workspace/ws_managed/mcp/unreachable-oauth/managed`,
        { headers: clientHeaders(config.token) },
      );
  ...
  test("returns safe connection errors for DCR and protocol negotiation failures", async () => {
    const previousRuntimeDb = process.env.OFFLINEGPT_RUNTIME_DB;
    const previousDevMode = process.env.OFFLINEGPT_DEV_MODE;
    const previousTelemetry = globalThis.__offlinegptDesktopTelemetry;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "offlinegpt-local-managed-mcp-handshake-errors-"));
    roots.push(workspaceRoot);
    process.env.OFFLINEGPT_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.OFFLINEGPT_DEV_MODE = "1";
    ...
    try {
      ...
      stops.push(() => server.stop());
      const offlinegptBaseUrl = `http://127.0.0.1:${server.port}`;
      const expectedMessage = "OfflineGPT could not connect to this MCP server. Check its OAuth settings and availability, then try again.";
      ...
      const reconnect = await fetch(
        `${offlinegptBaseUrl}/workspace/ws_managed/mcp/registration-failure/managed/connect`,
      ...
      const reconnectStatus = await fetch(
        `${offlinegptBaseUrl}/workspace/ws_managed/mcp/registration-failure/managed`,
        { headers: clientHeaders(config.token) },
      );
      ...
      const started = await fetch(
        `${offlinegptBaseUrl}/workspace/ws_managed/mcp/initialize-failure/managed/connect`,
        { method: "POST", headers: clientHeaders(config.token) },
      );
      ...
      const callbackStatus = await fetch(
        `${offlinegptBaseUrl}/workspace/ws_managed/mcp/initialize-failure/managed`,
        { headers: clientHeaders(config.token) },
      );
      ...
      const discoveryStarted = await fetch(
        `${offlinegptBaseUrl}/workspace/ws_managed/mcp/discovery-failure/managed/connect`,
        { method: "POST", headers: clientHeaders(config.token) },
      );
      ...
      const discoveryStatus = await fetch(
        `${offlinegptBaseUrl}/workspace/ws_managed/mcp/discovery-failure/managed`,
        { headers: clientHeaders(config.token) },
      );
      ...
      const internalFailure = await fetch(
        `${offlinegptBaseUrl}/workspace/ws_managed/mcp/unexpected-sdk-failure/managed/connect`,
        { method: "POST", headers: clientHeaders(config.token) },
      );
      ...
      globalThis.__offlinegptDesktopTelemetry = previousTelemetry;
      if (previousRuntimeDb === undefined) delete process.env.OFFLINEGPT_RUNTIME_DB;
      else process.env.OFFLINEGPT_RUNTIME_DB = previousRuntimeDb;
      if (previousDevMode === undefined) delete process.env.OFFLINEGPT_DEV_MODE;
      else process.env.OFFLINEGPT_DEV_MODE = previousDevMode;
  ...
  test("returns actionable input errors without persisting managed connections", async () => {
    const previousRuntimeDb = process.env.OFFLINEGPT_RUNTIME_DB;
    const previousDevMode = process.env.OFFLINEGPT_DEV_MODE;
    const previousAllowPrivateUrls = process.env.OFFLINEGPT_ALLOW_PRIVATE_MCP_URLS;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "offlinegpt-local-managed-mcp-input-errors-"));
    roots.push(workspaceRoot);
    process.env.OFFLINEGPT_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    ...
    try {
      ...
      for (const input of cases) {
        ...
        const list = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp`, {
          headers: clientHeaders(config.token),
        });
        expect(list.status).toBe(200);
        expect(JSON.stringify(await list.json())).not.toContain(`"name":"${input.name}"`);
        const status = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/${input.name}/managed`, {
          headers: clientHeaders(config.token),
        });
        expect(status.status).toBe(404);
      }
      ...
      if (previousAllowPrivateUrls === undefined) delete process.env.OFFLINEGPT_ALLOW_PRIVATE_MCP_URLS;
      else process.env.OFFLINEGPT_ALLOW_PRIVATE_MCP_URLS = previousAllowPrivateUrls;
    }
  });
  ...
  test("owns OAuth, exposes provider tools to OpenCode, refreshes, survives restart, and disconnects", async () => {
    const previousRuntimeDb = process.env.OFFLINEGPT_RUNTIME_DB;
    const previousDevMode = process.env.OFFLINEGPT_DEV_MODE;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "offlinegpt-local-managed-mcp-"));
    roots.push(workspaceRoot);
    process.env.OFFLINEGPT_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.OFFLINEGPT_DEV_MODE = "1";
    ...
    try {
      ...
      const offlinegptPort = await freePort();
      ...
      expect(callbackUrl).toStartWith(`${offlinegptBaseUrl}/mcp/oauth/callback`);
      const callback = await fetch(callbackUrl!);
      expect(callback.status).toBe(200);

      const status = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
        headers: clientHeaders(config.token),
      });
      ...
      expect(String(firstRuntimeConfig?.url)).toStartWith(`${offlinegptBaseUrl}/mcp/managed/ws_managed/mock-oauth`);
      ...
      expect(await firstClient.callTool({ name: "mock_echo", arguments: { text: "through OfflineGPT" } }))
        .toMatchObject({ content: [{ type: "text", text: "through OfflineGPT" }] });
      await expect(firstClient.callTool({ name: "mock_provider_error", arguments: {} })).rejects.toThrow();
      const providerErrorStatus = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
        headers: clientHeaders(config.token),
      });
      ...
      const revokedStatus = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
      ...
      const disconnected = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/mock-oauth/auth`, {
        method: "DELETE",
        headers: clientHeaders(restartedConfig.token),
      });
      ...
      const reconnect = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed/connect`, {
        method: "POST",
        headers: clientHeaders(restartedConfig.token),
      });
      expect(reconnect.status).toBe(200);
      expect(await reconnect.json()).toMatchObject({ status: "needs_auth" });
      const reconnectStatus = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
        headers: clientHeaders(restartedConfig.token),
      });
      ...
      expect((await readdir(runtimeStorageDir(restartedConfig)))
        .some((entry) => entry.includes(".offlinegpt-backup-"))).toBe(false);
    } finally {
      if (previousRuntimeDb === undefined) delete process.env.OFFLINEGPT_RUNTIME_DB;
      else process.env.OFFLINEGPT_RUNTIME_DB = previousRuntimeDb;
      if (previousDevMode === undefined) delete process.env.OFFLINEGPT_DEV_MODE;
  ...
  test("quarantines and rebuilds the vault after a secure-storage key change, then reconnects", async () => {
    const previousRuntimeDb = process.env.OFFLINEGPT_RUNTIME_DB;
    const previousDevMode = process.env.OFFLINEGPT_DEV_MODE;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "offlinegpt-local-managed-mcp-rotation-"));
    roots.push(workspaceRoot);
    process.env.OFFLINEGPT_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.OFFLINEGPT_DEV_MODE = "1";
    ...
    try {
      ...
      const backups = (await readdir(storageDir))
        .filter((entry) => entry.startsWith("local-managed-mcp-vault.json.offlinegpt-backup-"));
      ...
      const afterDiscoveryFailure = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
      ...
      const reconnected = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/mock-oauth/managed`, {
        headers: clientHeaders(rotatedConfig.token),
      });
      expect(await reconnected.json()).toMatchObject({ status: "connected", hasCredential: true, enabled: true });
      const second = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp/mock-oauth-b/managed`, {
        headers: clientHeaders(rotatedConfig.token),
      });
      ...
      const workspaceMcp = await fetch(`${offlinegptBaseUrl}/workspace/ws_managed/mcp`, {
        headers: clientHeaders(rotatedConfig.token),
      });
      ...
      expect((await readdir(storageDir))
        .filter((entry) => entry.startsWith("local-managed-mcp-vault.json.offlinegpt-backup-"))).toHaveLength(1);
    } finally {
      if (previousRuntimeDb === undefined) delete process.env.OFFLINEGPT_RUNTIME_DB;
      else process.env.OFFLINEGPT_RUNTIME_DB = previousRuntimeDb;
      if (previousDevMode === undefined) delete process.env.OFFLINEGPT_DEV_MODE;
  ...
  test("serves the plaintext index read-only while secure storage is unavailable", async () => {
    const previousRuntimeDb = process.env.OFFLINEGPT_RUNTIME_DB;
    const previousDevMode = process.env.OFFLINEGPT_DEV_MODE;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "offlinegpt-local-managed-mcp-unavailable-"));
    roots.push(workspaceRoot);
    process.env.OFFLINEGPT_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.OFFLINEGPT_DEV_MODE = "1";
  ...
  test("quarantines a legacy v1 vault it cannot decrypt and prunes orphaned gateway runtime entries", async () => {
    const previousRuntimeDb = process.env.OFFLINEGPT_RUNTIME_DB;
    const previousDevMode = process.env.OFFLINEGPT_DEV_MODE;
    const workspaceRoot = await mkdtemp(join(tmpdir(), "offlinegpt-local-managed-mcp-v1-"));
    roots.push(workspaceRoot);
    process.env.OFFLINEGPT_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    process.env.OFFLINEGPT_DEV_MODE = "1";
    ...
    try {
      ...
      cipher.setAAD(Buffer.from("offlinegpt-local-managed-mcp-v1", "utf8"));
