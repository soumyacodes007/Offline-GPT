...
import { OfflineGPTAnthropicToolSchema } from "./offlinegpt-anthropic-tool-schema.js";
...
beforeAll(async () => {
  ...
  await OfflineGPTAnthropicToolSchema();
...
describe("OfflineGPTAnthropicToolSchema fetch patch", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./offlinegpt-anthropic-tool-schema.js");
    expect(Object.keys(mod)).toEqual(["OfflineGPTAnthropicToolSchema"]);
  });
});
