...
import { offlinegptOfficeAttachmentsPluginPath } from "../offlinegpt-extensions-plugin-path.js";
import { OfflineGPTOfficeAttachments } from "./offlinegpt-office-attachments.js";
...
async function withWorkspace(fn: (root: string) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "offlinegpt-office-plugin-"));
  try {
    await fn(root);
  } finally {
...
async function transform(root: string, messages: unknown[]) {
  const plugin = await OfflineGPTOfficeAttachments({ directory: root });
...
describe("OfflineGPTOfficeAttachments", () => {
  ...
  test("rejects file URLs that resolve outside the workspace through a symlink", async () => {
    await withWorkspace(async (root) => {
      const outside = await mkdtemp(join(tmpdir(), "offlinegpt-office-outside-"));
      try {
        const docx = docxFixture();
  ...
  test("never materializes attachment bytes through a symlinked inbox folder", async () => {
    await withWorkspace(async (root) => {
      const outside = await mkdtemp(join(tmpdir(), "offlinegpt-office-inbox-outside-"));
      try {
  ...
  test("materializes into the real inbox folder and reuses identical bytes", async () => {
    await withWorkspace(async (root) => {
      ...
      expect(first).toBe(`.opencode/offlinegpt/inbox/chat-attachments/${sha256(docx).slice(0, 16)}-QuarterlyBrief.docx`);
      await expect(readdir(join(root, ".opencode", "offlinegpt", "inbox", "chat-attachments"))).resolves.toEqual([`${sha256(docx).slice(0, 16)}-QuarterlyBrief.docx`]);
  ...
  test("uses factory directory and ignores Daytona non-git worktree root", async () => {
    await withWorkspace(async (root) => {
      const docx = docxFixture();
      const plugin = await OfflineGPTOfficeAttachments({ directory: root, worktree: "/" });
      ...
      expect(materialized).toContain(".opencode/offlinegpt/inbox/chat-attachments/");
  ...
  test("is registered in runtime config and bundled by the build script", async () => {
    ...
    expect(plugin).toContain(offlinegptOfficeAttachmentsPluginPath());
    ...
    expect(packageJson.scripts.build).toContain("offlinegpt-office-attachments.ts");
  });

  test("module exposes only the plugin factory", async () => {
    const mod = await import("./offlinegpt-office-attachments.js");
    expect(Object.keys(mod)).toEqual(["OfflineGPTOfficeAttachments"]);
  });
});
