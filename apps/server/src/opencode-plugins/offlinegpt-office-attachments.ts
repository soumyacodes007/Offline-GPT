...
import {
  ...
  type XlsxSheetData,
  type ZipEntry,
} from "@offlinegpt/workbook";
import { offlineGptspaceFileForReading, offlineGptspaceFileForWriting } from "./workspace-file-identity.js";
...
const MATERIALIZED_DIR = join(".opencode", "offlinegpt", "inbox", "chat-attachments");
...
async function readWorkspaceFile(realRoot: string, path: string, label: string): Promise<Buffer> {
  ...
  const { handle, info } = await offlineGptspaceFileForReading(realRoot, path, label);
  try {
...
async function writeMaterializedFile(realRoot: string, target: string, bytes: Buffer): Promise<void> {
  const { handle } = await offlineGptspaceFileForWriting(realRoot, target, null, "Materialized Office attachment");
  try {
...
function normalizedText(part: OfficeFilePart, materialized: MaterializedAttachment | null, extractedText: string, error?: string): string {
  return [
    "OfflineGPT normalized an Office attachment before sending this request to the model.",
...
export const OfflineGPTOfficeAttachments = async (factoryInput?: unknown) => {
