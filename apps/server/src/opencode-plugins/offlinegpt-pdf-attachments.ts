 ...
 *   text; further pages come from the `offlinegpt_pdf_pages` tool on demand.
...
const PAGE_TOOL_NAME = "offlinegpt_pdf_pages";
...
function modelNote(support: ModelInputSupport, inlinePages: number, derived: DerivedPdf, reason: Reason): string {
  const why = reason.kind === "limits"
    ? `${reason.detail}, so OfflineGPT`
    : support.known
      ? "This model does not accept PDF input directly, so OfflineGPT"
      : "This model's input capabilities are not listed, so OfflineGPT treated it as text-only and";
...
function derivedNote(source: PdfSource, derived: DerivedPdf, support: ModelInputSupport, inlinePages: number, reason: Reason, placement: "message" | "tool_result"): string {
  ...
  const lines = [
    `OfflineGPT prepared the PDF "${safePdfFilename(source.filename)}" before sending this request to the model.`,
...
function failureNote(source: PdfSource, message: string, derived: DerivedPdf | null): string {
  return [
    `OfflineGPT could not prepare the PDF "${safePdfFilename(source.filename)}" for this model.`,
    `pdf_path: ${derived?.pdfPath ?? "unavailable"}`,
    `error: ${message}`,
...
function claimNative(inspection: Inspection, budget: NativeBudget): { ok: true } | { ok: false; detail: string } {
  ...
  if (inspection.bytes > policy.maxRawBytes) {
    return { ok: false, detail: `This PDF is ${megabytes(inspection.bytes)}; above ${megabytes(policy.maxRawBytes)} OfflineGPT stops sending the PDF itself, which would be re-uploaded on every step` };
  }
...
export const OfflineGPTPdfAttachments = async (factoryInput?: unknown) => {
  ...
  return {
    ...
    tool: {
      [PAGE_TOOL_NAME]: {
        ...
        args: {
          pdf_path: z.string().min(1).describe("Workspace-relative path of the PDF, for example the pdf_path line of an OfflineGPT PDF note."),
