// Both engine generations call this same OfflineGPT boundary before side effects.
...
export async function checkManagedTool(tool: string, raw: unknown): Promise<void> {
  ...
  else if (tool === "offlinegpt_execute") {
...
export async function check(action: ManagedPolicyAction, input: Record<string, unknown>): Promise<void> {
  const base = process.env.OFFLINEGPT_SERVER_URL;
  const token = process.env.OFFLINEGPT_POLICY_TOKEN;
  if (!base || !token) throw new Error("OfflineGPT policy service is unavailable.");
