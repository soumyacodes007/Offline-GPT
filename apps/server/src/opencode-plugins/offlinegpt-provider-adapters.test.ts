...
import { offlinegptFeatureContributionSchema } from "@offlinegpt/types/offlinegpt-provider";

import { buildOfflineGptProviderContributions } from "./offlinegpt-provider-adapters.js";

describe("OfflineGPT provider adapters", () => {
  test("normalizes sessions and extensions into semantic contributions", () => {
    const contributions = buildOfflineGptProviderContributions([]);
    ...
    for (const contribution of contributions) {
      expect(offlinegptFeatureContributionSchema.safeParse(contribution).success).toBe(true);
    }
  });
  ...
  test("keeps known Connect skills direct and search available for unknown capabilities", () => {
    ...
    expect(connect?.guidance).toEqual([{
      ...
      title: "Customer briefing",
      description: "Prepare a customer briefing from connected sources.",
      provider: { id: "offlinegpt-cloud", kind: "connect" },
      loading: "catalog",
    }]);
    expect(connect?.affordances.map((affordance) => ({
    ...
    }))).toEqual([
      {
        ...
        executor: { kind: "tool", tool: "offlinegpt-cloud_search_capabilities" },
      },
      {
        id: "connect.capability.execute",
        executor: { kind: "tool", tool: "offlinegpt-cloud_execute_capability" },
      },
    ]);
    expect(
  ...
  test("exposes an Automations proposal affordance that writes nothing", () => {
    const proposal = buildOfflineGptProviderContributions([])
