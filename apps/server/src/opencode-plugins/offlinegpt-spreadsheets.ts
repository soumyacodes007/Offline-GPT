...
import { WorkspaceFileError, offlineGptspaceFileForReading, offlineGptspaceFileForWriting } from "./workspace-file-identity.js";
import {
...
/**
 * OfflineGPT Spreadsheets Plugin
 *
...
async function writeWorkbookFile(root: string, file: WorkspaceFile, bytes: Uint8Array, overwrite: boolean): Promise<{ replaced: boolean }> {
  ...
  let opened;
  try {
    opened = await offlineGptspaceFileForWriting(realRoot, path, existing, label);
  } catch (error) {
...
export const OfflineGPTSpreadsheets = async (factoryInput?: unknown) => {
