...
import { buildOfflineGptRuntimeConfigObject } from "../offlinegpt-runtime-config.js";
import { offlinegptPdfAttachmentsPluginPath } from "../offlinegpt-extensions-plugin-path.js";
...
import { OfflineGPTPdfAttachments } from "./offlinegpt-pdf-attachments.js";
...
function noteOf(message: unknown): string {
  const note = partsOf(message).find((part) => part.type === "text" && typeof part.text === "string" && part.text.startsWith("OfflineGPT "));
  if (!note) throw new Error("Expected an OfflineGPT note part");
  return textOf(note);
}
...
async function withWorkspace(fn: (root: string) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "offlinegpt-pdf-plugin-"));
  try {
    await fn(root);
  } finally {
...
async function transform(root: string | null, messages: unknown[], factoryExtras: Record<string, unknown> = { client: { provider: { list: async () => catalog } } }) {
  const plugin = await OfflineGPTPdfAttachments({ ...(root ? { directory: root } : {}), ...factoryExtras });
...
describe("OfflineGPT PDF attachments plugin", () => {
  ...
  test("gives an image-capable model rendered pages in order plus page-marked text, and keeps the user's own text", async () => {
    await withWorkspace(async (root) => {
      ...
      expect(note).toContain('OfflineGPT prepared the PDF "report.pdf"');
      ...
      expect(note).toContain("page_images_on_disk: pages 1-3 under .opencode/offlinegpt/inbox/pdf-pages/");
      expect(note).toContain("more_pages: call offlinegpt_pdf_pages with pdf_path");
      expect(note).toContain("content_note: extracted_text is the document's content");
      expect(note).toContain("model_note: This model does not accept PDF input directly, so OfflineGPT attached the first 3 pages as images");
  ...
  test("treats an unlisted model as text-only and says so", async () => {
    await withWorkspace(async (root) => {
      ...
      expect(noteOf(message)).toContain("This model's input capabilities are not listed, so OfflineGPT treated it as text-only");
    });
  });
  ...
  test("routes a PDF that exceeds native provider limits through the derived path", async () => {
    await withWorkspace(async (root) => {
      ...
      expect(note).toContain("model_note: This PDF's 101 pages exceed the 100 native PDF pages left for this request, so OfflineGPT");
  ...
  test("inlines at most 20 page images and points to the page tool for the rest", async () => {
    await withWorkspace(async (root) => {
      ...
      expect(note).toContain("more_pages: call offlinegpt_pdf_pages with pdf_path and up to 8 page numbers to see other pages as images");
  ...
  test("a second workspace attaching the same bytes derives its own copy instead of borrowing paths from the first", async () => {
    await withWorkspace(async (first) => {
      await withWorkspace(async (second) => {
        ...
        const note = partsOf(result[0]).map((part) => String(part.text)).find((text) => text.startsWith("OfflineGPT prepared the PDF")) ?? "";
  ...
  test("reads workspace file: URLs and refuses paths that escape the workspace", async () => {
    await withWorkspace(async (root) => {
      const outside = await mkdtemp(join(tmpdir(), "offlinegpt-pdf-outside-"));
      try {
        const inboxDir = join(root, ".opencode", "offlinegpt", "inbox", "chat-attachments");
  ...
  test("shares one native page budget across every PDF in the step, in transcript order", async () => {
    await withWorkspace(async (root) => {
      ...
      const note = noteOf(message);
      expect(note).toContain('OfflineGPT prepared the PDF "second.pdf"');
  ...
  test("keeps a PDF-capable model on the derived path when native input would dominate its context window", async () => {
    await withWorkspace(async (root) => {
      ...
      const note = noteOf(message);
      expect(note).toContain("Sent natively this PDF would take roughly 14k tokens of this model's 32k context window on every step, so OfflineGPT attached the first 6 pages as images");
  ...
  test("stops sending the PDF itself above the per-step upload ceiling even when the model accepts PDFs", async () => {
    await withWorkspace(async (root) => {
      ...
      const note = noteOf(message);
      expect(note).toContain("above 10 MB OfflineGPT stops sending the PDF itself, which would be re-uploaded on every step");
      expect(note).toContain("--- page 1 ---\nLarge report");
    });
  }, 30_000);
  ...
  test("normalizes PDF attachments inside tool results the same way", async () => {
    await withWorkspace(async (root) => {
      ...
      expect(String(textState.output)).toContain('OfflineGPT prepared the PDF "handbook.pdf"');
  ...
  test("the page tool renders requested pages for image-capable sessions and returns text for text-only ones", async () => {
    await withWorkspace(async (root) => {
      const pages = Array.from({ length: 25 }, (_page, index) => `Page ${index + 1}`);
      const plugin = await OfflineGPTPdfAttachments({ directory: root, client: { provider: { list: async () => catalog } } });
      ...
      expect(pdfPath.startsWith(".opencode/offlinegpt/inbox/chat-attachments/")).toBe(true);

      const tool = plugin.tool.offlinegpt_pdf_pages;
  ...
  test("is registered in runtime config, bundled with its wasm runtime, and packaged by the desktop app", async () => {
    const runtime = await buildOfflineGptRuntimeConfigObject();
    const plugin = runtime.plugin;
    if (!Array.isArray(plugin)) throw new Error("Expected plugin list");
    expect(plugin).toContain(offlinegptPdfAttachmentsPluginPath());
    ...
    expect(packageJson.scripts.build).toContain("offlinegpt-pdf-attachments.ts");
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./offlinegpt-pdf-attachments.js");
    expect(Object.keys(mod)).toEqual(["OfflineGPTPdfAttachments"]);
  });
});
