...
import { offlinegptSpreadsheetsPluginPath } from "../offlinegpt-extensions-plugin-path.js";
import { buildZip, excelSerialToIso, isDateNumberFormat, listZipEntries, openXlsxWorkbook, readZipEntryData, renderSheetTable, utf8Text, writeXlsxWorkbook } from "@offlinegpt/workbook";
import { OfflineGPTSpreadsheets } from "./offlinegpt-spreadsheets.js";

const PACKAGE_ROOT = join(dirname(fileURLToPath(import.meta.url)), "../..");

type Plugin = Awaited<ReturnType<typeof OfflineGPTSpreadsheets>>;
...
async function withWorkspace(fn: (root: string, plugin: Plugin) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "offlinegpt-spreadsheets-"));
  try {
    await fn(root, await OfflineGPTSpreadsheets({ directory: root }));
  } finally {
...
describe("OfflineGPTSpreadsheets", () => {
  ...
  test("keeps every path inside the workspace and explains unsupported files", async () => {
    await withWorkspace(async (root, plugin) => {
      const outside = await mkdtemp(join(tmpdir(), "offlinegpt-spreadsheets-outside-"));
      try {
  ...
  test("appends one spreadsheets instruction section to the existing system entry", async () => {
    const plugin = await OfflineGPTSpreadsheets({ directory: tmpdir() });
  ...
  test("is registered in runtime config and bundled by the build script", async () => {
    ...
    expect(plugin).toContain(offlinegptSpreadsheetsPluginPath());
    ...
    expect(packageJson.scripts.build).toContain("offlinegpt-spreadsheets.ts");
  });

  test("module exposes only the plugin factory", async () => {
    const mod = await import("./offlinegpt-spreadsheets.js");
    expect(Object.keys(mod)).toEqual(["OfflineGPTSpreadsheets"]);
  });
});
