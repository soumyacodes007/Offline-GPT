...
/**
 * OfflineGPT Capabilities Knowledge Plugin
 *
 * Injects knowledge about OfflineGPT's capabilities into the agent's system
 ...
 * - Using OfflineGPT Cloud
 * - Finding OfflineGPT docs before falling back to code
...
export function automationRuntimeKnowledge(runtimeProvider = process.env.DEN_RUNTIME_PROVIDER) {
  ...
  const shared = [
    "OfflineGPT has first-class Automations. Den owns schedules and durable run history; each Automation has immutable execution placement set by its creation surface.",
  ...
  if (runtimeProvider === "daytona") {
    return [
      ...shared,
      "This chat is running in OfflineGPT Cloud. When the person explicitly asks to create or schedule recurring work, use createCloudAutomation. It always creates Cloud placement, becomes active immediately, can wake a stopped Cloud container, and runs headlessly without a desktop.",
      ...
      "Cloud agent Automations use the person's current OfflineGPT Connect integrations. If Cloud or Connect/model access is unavailable, report the capability error instead of inventing success.",
    ].map((line) => `- ${line}`).join("\n");
  }
  return [
    ...shared,
    "This chat is running in OfflineGPT Desktop. For new recurring work, use offlinegpt_execute id automation.propose so the person can review and create it in the app. Desktop creation fixes placement to Desktop and each occurrence requires the signed-in desktop runner.",
...
- **OfflineGPT Cloud models**: Users can sign up for OfflineGPT Cloud at the Den sign-in page for managed AI models without needing their own API keys.
...
## Fixing Authorized Folders
- Go to Settings > Permissions to manage which folders OfflineGPT can access.
...
- If the runtime steering says OfflineGPT Cloud is not ready, do not substitute documentation, browser, or UI tools for the connected-service action; direct the user to \`Settings > Library\` for inventory and \`Settings > Debug\` (developer mode) to repair and test agent access.
- Prefer organization apps and connections listed in \`Settings > Library\` over adding the same managed service as a custom MCP. \`Settings > Library\` and custom MCP commands/URLs are also the path for a custom or local MCP server that OfflineGPT Cloud does not provide.
- OfflineGPT Connect's public hosted endpoint for external MCP clients is \`https://api.offlinegptlabs.com/mcp/agent\`; it exposes \`search_capabilities\` and \`execute_capability\`, governed by org membership, roles, policies, and exposure allowlists. \`app.offlinegptlabs.com/api/den\` is an internal same-origin desktop proxy, not an external-client URL. Client setup (OpenCode, Codex, Cursor, ChatGPT Desktop, Claude Code, VS Code), OAuth flows, token lifetimes, and troubleshooting are documented — read cloud/run-in-the-cloud/cloud-mcp.mdx with offlinegpt_docs_read before answering from memory.

## Other sessions
- For questions about another chat (what was said, decided, or done), use the session affordances described under OfflineGPT app context; match by ID, title, workspace, or topic words, ask a short clarifying question if several sessions match, and answer only from the returned transcript — say so when it is limited or missing older context.

## OfflineGPT Cloud
- Users sign up at the Den portal (accessible from the status bar "Sign in" button).
- Cloud features: managed AI models, team workspaces, shared skills, Collections, org provisioning, and the hosted OfflineGPT Cloud MCP server.
...
- Never write a cron entry, launchd/systemd unit, Task Scheduler job, or workspace script as a substitute for an OfflineGPT Automation.

## Creating Plugins
- Plugins extend OfflineGPT/OpenCode with custom tools.
...
When users ask "what can I do?" or "what can OfflineGPT do?", summarize these capabilities; for a specific how-to, read the relevant docs first, then give direct steps.`;

const docsSearchArgsSchema = z.object({
  query: z.string().min(1).describe("OfflineGPT docs search query, for example 'connect slack mcp'."),
...
const docsReadArgsSchema = z.object({
  path: z.string().min(1).describe("Docs-relative path returned by offlinegpt_docs_search, for example start-here/connect-your-stack/connect-services.mdx."),
});
...
function docsCandidates(): string[] {
  ...
  return [
    process.env.OFFLINEGPT_DOCS_DIR?.trim() ?? "",
    join(here, "..", "offlinegpt-docs"),
    join(here, "..", "..", "offlinegpt-docs"),
...
export const OfflineGPTCapabilitiesKnowledge = async () => ({
  "experimental.chat.system.transform": async (_input: unknown, output: { system: string[] }) => {
    appendAgentInstructions(output.system, createInstructionSection("capabilities-knowledge", OFFLINEGPT_CAPABILITIES_KNOWLEDGE));
  },
  tool: {
    offlinegpt_docs_search: {
      description: "Search the bundled OfflineGPT documentation. Use this first for OfflineGPT product questions before inspecting implementation code.",
    ...
    offlinegpt_docs_read: {
      description: "Read a bundled OfflineGPT documentation page by docs-relative path returned from offlinegpt_docs_search.",
      ...
      async execute(rawArgs: unknown) {
        ...
        if (!entry) throw new Error(`OfflineGPT docs page not found: ${normalized}`);
