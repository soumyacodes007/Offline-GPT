...
import * as OfflineGPTExtensionsPreviewEntry from "./offlinegpt-extensions-preview.js";
import {
...
const originalUiControlDiscovery = process.env.OFFLINEGPT_UI_CONTROL_DISCOVERY;
...
afterEach(() => {
  ...
  if (originalUiControlDiscovery === undefined) delete process.env.OFFLINEGPT_UI_CONTROL_DISCOVERY;
  else process.env.OFFLINEGPT_UI_CONTROL_DISCOVERY = originalUiControlDiscovery;
});

async function transformedSystem(plugin: Awaited<ReturnType<typeof OfflineGPTExtensionsPreview>>): Promise<string> {
  const output: { system: string[] } = { system: [] };
  await plugin["experimental.chat.system.transform"]({}, output);
  return output.system.join("\n");
}

function startFakeOfflineGPTServer(options: { failPromptText?: string; failSessionListWorkspaceId?: string } = {}) {
  ...
  process.env.OFFLINEGPT_SERVER_URL = `http://127.0.0.1:${server.port}`;
  process.env.OFFLINEGPT_SERVER_TOKEN = "test-token";
  return { requests, uiControlRequests };
}

describe("OfflineGPTExtensionsPreview MCP Apps result preservation", () => {
  test("keeps standard MCP UI result fields in completed tool metadata", async () => {
    const plugin = await OfflineGPTExtensionsPreview();
...
describe("OfflineGPTExtensionsPreview session tools", () => {
  test("plugin entry exposes only the factory export for the OpenCode loader", () => {
    expect(Object.keys(OfflineGPTExtensionsPreviewEntry)).toEqual(["OfflineGPTExtensionsPreview"]);
  });

  test("projects built-in, extension, and Connect providers into one agent context", async () => {
    startFakeOfflineGPTServer();
    const plugin = await OfflineGPTExtensionsPreview({
      client: {
        mcp: {
          status: async () => ({
    ...
    const output = await plugin.tool.offlinegpt_context.execute();
  ...
  test("routes semantic session queries without navigating the UI", async () => {
    ...
    const output = await plugin.tool.offlinegpt_query.execute({
      id: "session.read",
  ...
  test("searches past chat transcript text and prefers the user's matching message", async () => {
    const fake = startFakeOfflineGPTServer();
    const plugin = await OfflineGPTExtensionsPreview();
  ...
  test("keeps search results when one workspace native mount is unavailable", async () => {
    const fake = startFakeOfflineGPTServer({ failSessionListWorkspaceId: "ws_2" });
    const plugin = await OfflineGPTExtensionsPreview();
  ...
  test("merges factory directory into transform steering when hook input omits it", async () => {
    const fake = startFakeOfflineGPTServer();
    const plugin = await OfflineGPTExtensionsPreview({ directory: "/tmp/archive" });
    const output: { system: string[] } = { system: [] };
    ...
    expect(output.system.join("\n")).toContain(OFFLINEGPT_CLOUD_SKILL_AUTHORING_INSTRUCTION);
    expect(output.system.join("\n")).not.toContain(OFFLINEGPT_LOCAL_SKILL_AUTHORING_INSTRUCTION);
  ...
  test("uses the factory engine client as transform steering source of truth", async () => {
    ...
    const mcp = {
      result: { data: { "offlinegpt-cloud": { status: "connected" } } },
      async status(request: unknown) {
        requests.push(request);
        return this.result;
      },
    };
    const plugin = await OfflineGPTExtensionsPreview({ client: { mcp }, directory: "/tmp/archive" });
    const output: { system: string[] } = { system: [] };
  ...
  test("uses neutral transform steering when the engine reports failed Cloud status", async () => {
    const requests: unknown[] = [];
    const mcp = {
      result: { data: { "offlinegpt-cloud": { status: "failed" } } },
      async status(request: unknown) {
        requests.push(request);
        return this.result;
      },
    ...
    expect(output.system[0]).toContain(OFFLINEGPT_EXTENSION_DISCOVERY_INSTRUCTION);
    expect(output.system[0].indexOf("## Built-in Browser")).toBeLessThan(output.system[0].indexOf(OFFLINEGPT_EXTENSION_DISCOVERY_INSTRUCTION));
    expect(output.system[0]).toContain(OFFLINEGPT_LOCAL_SKILL_AUTHORING_INSTRUCTION);
    expect(output.system[0]).not.toContain(OFFLINEGPT_CLOUD_SKILL_AUTHORING_INSTRUCTION);
    expect(output.system[0]).not.toContain("not ready");
    expect(output.system[0]).not.toContain("Repair and test");
    expect(output.system[0]).not.toContain("Do not use OfflineGPT documentation tools");
  });
  ...
  test("creates and starts multiple sessions through the OfflineGPT backend", async () => {
    const fake = startFakeOfflineGPTServer();
    const plugin = await OfflineGPTExtensionsPreview({ directory: "/tmp/archive" });

    const output = await plugin.tool.offlinegpt_execute.execute({
      id: "session.create",
      args: {
        sessions: [
  ...
  test("still asks the desktop to refetch when a created session's prompt fails to start", async () => {
    const fake = startFakeOfflineGPTServer({ failPromptText: "Fail this prompt." });
    const plugin = await OfflineGPTExtensionsPreview({ directory: "/tmp/archive" });

    await plugin.tool.offlinegpt_execute.execute({
      id: "session.create",
  ...
  test("does not ask the desktop to refetch when no session reached the engine", async () => {
    ...
    await expect(plugin.tool.offlinegpt_execute.execute({
      id: "session.create",
...
describe("OfflineGPTExtensionsPreview semantic tool surface", () => {
  test("exposes semantic tools, native visualization and the WebMCP browser broker", async () => {
    ...
    expect(system).not.toContain("<offlinegpt_default_skill");
    expect(system).not.toContain("offlinegpt_ui_");
    expect(system).not.toContain("offlinegpt_session_");
    expect(system).not.toContain("offlinegpt_extension_");
    expect(system).not.toContain("offlinegpt_browser_");
    expect(system).toContain("Use offlinegpt_context");
    expect(system).toContain("use offlinegpt_visualization");
  ...
  test("renders a bounded visualization without calling a backend", async () => {
    ...
    expect(JSON.parse(await plugin.tool.offlinegpt_visualization.execute(design))).toEqual(design);
    expect(fake.requests).toHaveLength(0);
    await expect(plugin.tool.offlinegpt_visualization.execute({ ...design, sections: [] })).rejects.toThrow();
    await expect(plugin.tool.offlinegpt_visualization.execute({ ...design, revision: 0 })).rejects.toThrow();
    await expect(plugin.tool.offlinegpt_visualization.execute({ ...design, sections: Array(9).fill(design.sections[0]) })).rejects.toThrow();
    await expect(plugin.tool.offlinegpt_visualization.execute({ ...design, sections: [{ title: "Unsafe", blocks: [{ kind: "html", label: "Code" }] }] })).rejects.toThrow();
    expect(fake.requests).toHaveLength(0);
  });
  ...
  test("discards a model-supplied workspaceId and pins the conversation's workspace", async () => {
    const plugin = await OfflineGPTExtensionsPreview({ directory: "/tmp/archive", workspaceId: "ws_conversation" });
...
async function startFakeWebMcpUiBridge() {
  ...
  const directory = await mkdtemp(join(tmpdir(), "offlinegpt-webmcp-ui-"));
  const discoveryPath = join(directory, "offlinegpt-ui-control.json");
  await writeFile(discoveryPath, JSON.stringify({
    baseUrl: `http://127.0.0.1:${server.port}`,
    token: "ui-test-token",
  }));
  process.env.OFFLINEGPT_UI_CONTROL_DISCOVERY = discoveryPath;
  stops.push(() => {
    server.stop(true);
