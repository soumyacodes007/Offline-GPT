...
import { OfflineGPTAnthropicAdaptiveThinking } from "./offlinegpt-anthropic-adaptive-thinking.js";

async function runHook(apiId: string, options: Record<string, unknown>) {
  const hooks = await OfflineGPTAnthropicAdaptiveThinking();
  const output: { options: Record<string, unknown> } = { options };
  await hooks["chat.params"]({ model: { id: apiId, api: { id: apiId } } }, output);
  return output.options;
}

describe("OfflineGPTAnthropicAdaptiveThinking chat.params", () => {
  ...
  test("module exposes only the plugin factory", async () => {
    const mod = await import("./offlinegpt-anthropic-adaptive-thinking.js");
    expect(Object.keys(mod)).toEqual(["OfflineGPTAnthropicAdaptiveThinking"]);
  });
});
