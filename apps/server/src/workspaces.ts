...
export function workspaceIdForOfflineGpt(hostUrl: string, workspaceId?: string | null): string {
  const normalizedHostUrl = hostUrl.trim();
  const normalizedWorkspaceId = workspaceId?.trim() ?? "";
  const key = normalizedWorkspaceId
    ? `offlinegpt::${normalizedHostUrl}::${normalizedWorkspaceId}`
    : `offlinegpt::${normalizedHostUrl}`;
  return workspaceIdForKey(key);
}
...
export function buildWorkspaceInfos(
  ...
  return workspaces.map((workspace) => {
    ...
    const id = workspace.id?.trim()
      || (workspaceType === "remote"
        ? remoteType === "offlinegpt"
          ? workspaceIdForOfflineGpt(workspace.offlinegptHostUrl ?? workspace.baseUrl ?? "", workspace.offlinegptWorkspaceId)
    ...
    return {
      ...
      offlinegptHostUrl: workspace.offlinegptHostUrl,
      offlinegptToken: workspace.offlinegptToken,
      offlinegptWorkspaceId: workspace.offlinegptWorkspaceId,
      offlinegptWorkspaceName: workspace.offlinegptWorkspaceName,
