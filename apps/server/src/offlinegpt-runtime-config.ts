 ...
 *
 * This is the single source of truth for the offlinegpt agent definition,
...
import {
  offlinegptExtensionsPreviewPluginPath,
  offlinegptCapabilitiesKnowledgePluginPath,
  offlinegptAnthropicAdaptiveThinkingPluginPath,
  offlinegptAnthropicToolSchemaPluginPath,
  offlinegptTitleRecoveryPluginPath,
  offlinegptOfficeAttachmentsPluginPath,
  offlinegptSpreadsheetsPluginPath,
  offlinegptChromeDevtoolsPluginPath,
  offlinegptPdfAttachmentsPluginPath,
} from "./offlinegpt-extensions-plugin-path.js";
...
export async function buildOfflineGptRuntimeConfigObject(
  config?: ServerConfig,
  ...
  return buildOfflineGptRuntimeConfigObjectFromSnapshot(runtimeConfig);
}

export function buildOfflineGptRuntimeConfigObjectFromSnapshot(
  ...
  return {
    ...
    ...(runtimeConfig.managedPolicy?.allowCustomProviders === false ? { enabled_providers: [
      ...Object.keys(provider).filter((id) => /^(?:lpr_|offlinegpt$)/i.test(id)),
      ...(runtimeConfig.managedPolicy.allowZenModel !== false ? ["opencode"] : []),
    ] } : {}),
    permission: { ...engineConfig.permission, ...permissions },
    default_agent: runtimeConfig.default_agent ?? "offlinegpt",
    agent: {
      offlinegpt: {
        description: "OfflineGPT default agent",
        mode: "primary",
        temperature: 0.2,
        prompt: OFFLINEGPT_AGENT_PROMPT,
        permission: {
          ...permissions,
          skill: {
            // OfflineGPT supplies its own current skill routing and no longer
...
export async function buildOfflineGptRuntimeConfig(config?: ServerConfig): Promise<string> {
  return stableStringify(await buildOfflineGptRuntimeConfigObject(config));
}

export function offlinegptRuntimeConfigFilePath(config: ServerConfig): string {
...
export interface OfflineGptRuntimeConfigWriteResult {
  path: string;
  changed: boolean;
}

const fileWriteQueue = new Map<string, Promise<OfflineGptRuntimeConfigWriteResult>>();
 ...
 * mid-dispose.
 */
export async function writeOfflineGptRuntimeConfigFile(
  config: ServerConfig,
): Promise<OfflineGptRuntimeConfigWriteResult> {
  const path = offlinegptRuntimeConfigFilePath(config);
  const job = async () => {
    const content = await buildOfflineGptRuntimeConfig(config);
 ...
 */
export function keepOfflineGptRuntimeConfigFileFresh(config: ServerConfig): () => void {
  return onRuntimeOpencodeConfigWrite((writeConfig, writtenWorkspaceId) => {
    ...
    void writeOfflineGptRuntimeConfigFile(writeConfig).catch(() => undefined);
  });
}
