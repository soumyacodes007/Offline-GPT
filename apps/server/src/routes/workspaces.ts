...
import { defaultWorkspaceOfflineGptConfig, ensureWorkspaceFiles } from "../workspace-init.js";
import { seedOfflineGptWorkspaceConfigIfEmpty } from "../offlinegpt-workspace-config-store.js";
...
function parseOfflineGptWorkspaceIdFromUrl(input: string | null | undefined): string | null {
...
function stripOfflineGptWorkspaceMount(input: string | null | undefined): string | null {
  const raw = input?.trim() ?? "";
  if (!raw) return null;
  try {
    const url = new URL(raw);
...
function offlinegptRemoteWorkspaceId(hostUrl: string, workspaceId: string | null | undefined): string {
  const remoteWorkspaceId = workspaceId?.trim() || parseOfflineGptWorkspaceIdFromUrl(hostUrl);
...
function selectOfflineGptWorkspaceForConnection(list: unknown, directory: string | null): Record<string, unknown> | null {
...
function offlinegptWorkspaceDisplayName(workspace: Record<string, unknown>): string | null {
  return readStringField(workspace, "displayName")
    || readStringField(workspace, "offlinegptWorkspaceName")
    || readStringField(workspace, "name")
    || readStringField(workspace, "id")
    || null;
}

async function fetchOfflineGptWorkspaceList(hostUrl: string, token: string, hostToken: string): Promise<unknown> {
  const url = `${hostUrl.replace(/\/+$/, "")}/workspaces`;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 8_000);
  const headers = new Headers();
  if (token) headers.set("Authorization", `Bearer ${token}`);
  if (hostToken) headers.set("X-OfflineGPT-Host-Token", hostToken);
  ...
  try {
    ...
    if (!response.ok) {
      throw new ApiError(
        502,
        "offlinegpt_workspace_discovery_failed",
        `OfflineGPT workspace discovery failed (${response.status} ${response.statusText || "HTTP error"})`,
      );
    }
    return await response.json();
  } catch (error) {
    if (error instanceof ApiError) throw error;
    throw new ApiError(502, "offlinegpt_workspace_discovery_failed", "OfflineGPT workspace discovery failed", {
      error: String(error),
    });
  } finally {
...
async function discoverOfflineGptWorkspace(input: {
  hostUrl: string;
  token: string;
  hostToken: string;
  directory: string | null;
}): Promise<Record<string, unknown> | null> {
  const list = await fetchOfflineGptWorkspaceList(input.hostUrl, input.token, input.hostToken);
  return selectOfflineGptWorkspaceForConnection(list, input.directory);
}
...
function serializeWorkspaceConfigEntry(workspace: WorkspaceInfo): Record<string, unknown> {
  ...
  return {
    ...
    ...(workspace.offlinegptHostUrl ? { offlinegptHostUrl: workspace.offlinegptHostUrl } : {}),
    ...(workspace.offlinegptToken ? { offlinegptToken: workspace.offlinegptToken } : {}),
    ...(workspace.offlinegptWorkspaceId ? { offlinegptWorkspaceId: workspace.offlinegptWorkspaceId } : {}),
    ...(workspace.offlinegptWorkspaceName ? { offlinegptWorkspaceName: workspace.offlinegptWorkspaceName } : {}),
...
export function registerWorkspaceRoutes(options: RegisterWorkspaceRoutesOptions): void {
  ...
  addRoute(routes, "POST", "/workspaces/local", "host", async (ctx) => {
    ...
    // Seed the per-workspace offlinegpt config in the runtime DB (replaces the
    // legacy `.opencode/offlinegpt.json` file). No-op if a row already exists.
    await seedOfflineGptWorkspaceConfigIfEmpty(
      config,
      workspaceId,
      defaultWorkspaceOfflineGptConfig(workspacePath, preset),
    );
  ...
  addRoute(routes, "POST", "/workspaces/remote", "host", async (ctx) => {
    ...
    const remoteType = readStringField(body, "remoteType") === "opencode" ? "opencode" : "offlinegpt";
    const directory = readStringField(body, "directory") || null;
    const displayName = readStringField(body, "displayName") || null;
    const rawOfflineGptHostUrl = readStringField(body, "offlinegptHostUrl") || null;
    ...
    const offlinegptToken = readStringField(body, "offlinegptToken");
    const offlinegptHostToken = readStringField(body, "offlinegptHostToken");
    ...
    let offlinegptWorkspaceId = remoteType === "offlinegpt"
      ? readStringField(body, "offlinegptWorkspaceId")
        || parseOfflineGptWorkspaceIdFromUrl(rawOfflineGptHostUrl)
        || parseOfflineGptWorkspaceIdFromUrl(baseUrl)
      : "";
    let offlinegptWorkspaceName = readStringField(body, "offlinegptWorkspaceName") || null;

    if (remoteType === "offlinegpt" && !offlinegptWorkspaceId) {
      const discovered = await discoverOfflineGptWorkspace({
        hostUrl: offlinegptHostUrl ?? baseUrl,
        token: offlinegptToken,
        hostToken: offlinegptHostToken,
        directory,
      });
      offlinegptWorkspaceId = discovered ? readStringField(discovered, "id") : "";
      offlinegptWorkspaceName = discovered ? offlinegptWorkspaceDisplayName(discovered) : offlinegptWorkspaceName;
      if (!offlinegptWorkspaceId) {
        throw new ApiError(
          400,
          "offlinegpt_workspace_not_found",
          directory
    ...
    const workspace: WorkspaceInfo = {
      id: remoteType === "offlinegpt"
        ? offlinegptRemoteWorkspaceId(offlinegptHostUrl ?? baseUrl, offlinegptWorkspaceId)
        : workspaceIdForRemote(baseUrl, directory),
      name: displayName ?? offlinegptWorkspaceName ?? "Remote workspace",
      path: directory ?? "",
      preset: "remote",
      ...
      ...(remoteType === "offlinegpt" && offlinegptHostUrl ? { offlinegptHostUrl } : {}),
      ...(offlinegptToken ? { offlinegptToken } : {}),
      ...(remoteType === "offlinegpt" && offlinegptWorkspaceId ? { offlinegptWorkspaceId } : {}),
      ...(remoteType === "offlinegpt" && offlinegptWorkspaceName ? { offlinegptWorkspaceName } : {}),
  ...
  addRoute(routes, "DELETE", "/workspaces/:id", "host", async (ctx) => {
    ...
    await recordAudit(workspace.path, {
      ...
      target: "workspace",
      summary: "Deleted workspace from OfflineGPT server",
      timestamp: Date.now(),
    });
