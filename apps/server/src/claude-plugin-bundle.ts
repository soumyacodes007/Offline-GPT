...
function githubApiBase(): string {
  return (process.env.OFFLINEGPT_GITHUB_API_BASE?.trim() || "https://api.github.com").replace(/\/+$/, "");
}

function githubRawBase(): string {
  return (process.env.OFFLINEGPT_GITHUB_RAW_BASE?.trim() || "https://raw.githubusercontent.com").replace(/\/+$/, "");
}
...
async function fetchGithubJson(url: string): Promise<unknown> {
  const response = await externalFetch(url, {
    headers: { Accept: "application/vnd.github+json", "User-Agent": "offlinegpt-server" },
...
async function fetchGithubText(url: string): Promise<string> {
  const response = await externalFetch(url, {
    headers: { Accept: "text/plain", "User-Agent": "offlinegpt-server" },
    signal: AbortSignal.timeout(20_000),
  });
  if (!response.ok) {
    const text = await response.text().catch(() => "");
...
export async function resolveClaudePluginBundle(input: { url: string; ref?: string }): Promise<ClaudePluginBundle> {
  ...
  if (manifest.hooks !== undefined) {
    warnings.push("This plugin declares hooks, which OfflineGPT does not support yet. Hooks were skipped.");
  }
  ...
  const addMcpServers = (value: unknown) => {
    ...
    for (const [name, config] of Object.entries(record)) {
      ...
      if (mcpConfigReferencesPluginRoot(config)) {
        warnings.push(`MCP server "${name}" uses \${CLAUDE_PLUGIN_ROOT} (a plugin-local command), which OfflineGPT does not support yet. It was skipped.`);
        continue;
      }
  ...
  if (memberships.length === 0) {
    throw new ApiError(400, "plugin_empty", "This plugin has no MCP servers, skills, commands, or agents OfflineGPT can install.");
  }
