...
const REQUIRED_CLOUD_AGENT_TOOL_IDS = REQUIRED_CLOUD_TOOL_IDS.map(
  (toolId) => `${OFFLINEGPT_CLOUD_MCP_NAME}_${toolId}`,
);
...
function assessEffectiveToolPolicy(
  ...
  const offlinegptAgent = snapshot.agents.find((agent) => agent.name === "offlinegpt");
  if (!offlinegptAgent) {
    return {
      status: "unavailable",
      decisions: {},
      deniedToolIds: [],
      unavailableReasons: ["effective_offlinegpt_agent_missing"],
    };
  }
  if (
    snapshot.defaultAgent !== "offlinegpt"
    || offlinegptAgent.hidden
    || (offlinegptAgent.mode !== "primary" && offlinegptAgent.mode !== "all")
  ) {
    return {
      status: "unavailable",
      decisions: {},
      deniedToolIds: [],
      unavailableReasons: ["effective_offlinegpt_agent_unusable_as_default"],
    };
  }
  const decisions: Record<string, EffectiveToolPolicyDecision> = {};
  for (const toolId of REQUIRED_CLOUD_AGENT_TOOL_IDS) {
    decisions[toolId] = effectiveToolDecision(offlinegptAgent.permission, toolId);
  }
...
function promptEvidence(configuredAgent: Record<string, unknown> | null) {
  ...
  return {
    ...
    markers: {
      ...
      artifacts: prompt.includes("## OfflineGPT Artifacts"),
    },
  };
}
...
async function inspectProjectAgent(workspace: WorkspaceInfo, signal?: AbortSignal): Promise<ProjectAgentInspection> {
  ...
  try {
    ...
    return {
      ...
      agentConfigOverride: Object.hasOwn(agents, "offlinegpt"),
      agentFileOverride: await Promise.all([
        exists(join(workspace.path, ".opencode", "agent", "offlinegpt.md")),
        exists(join(workspace.path, ".opencode", "agents", "offlinegpt.md")),
  ...
  try {
    return {
      inventory: await inspectMcpLayersFromRuntimeSnapshot(workspace.path, runtime, {
        signal,
        toolPolicy: {
          agentName: "offlinegpt",
          mcpName: OFFLINEGPT_CLOUD_MCP_NAME,
...
export function cloudCatalogCheck(probe: CloudCatalogProbe): AgentContextDiagnosticCheck {
  ...
  if (probe.status === "observed") {
    ...
    return diagnosticCheck({
      ...
      message: transientUnauthorizedRecovered
        ? "The OfflineGPT runtime saw a transient HTTP 401 during the independent Cloud MCP initialize request and recovered on retry; this points to a proxy or auth-gateway blip, not a revoked OfflineGPT Cloud credential."
        : exact
        ? "The canonical OfflineGPT Cloud catalog exposes exactly the two required capability tools."
        : "The OfflineGPT Cloud catalog does not match the required two-tool contract.",
      owner: transientUnauthorizedRecovered ? "network-admin" : exact ? "offlinegpt-server" : "offlinegpt-support",
      action: transientUnauthorizedRecovered
        ...
        : "Review the OfflineGPT Cloud deployment and restore the canonical capability catalog.",
    });
  }
  ...
  let message = "The selected workspace does not have a usable managed OfflineGPT Cloud MCP configuration.";
  let owner: AgentContextDiagnosticCheck["owner"] = "offlinegpt-server";
  let action = "Reconnect OfflineGPT Cloud from Settings > Connect and rerun diagnostics.";
  ...
  switch (probe.code) {
    ...
    case "remote_workspace_unavailable":
      ...
      action = "Run diagnostics on the OfflineGPT server that owns the workspace.";
      break;
    case "cloud_mcp_missing":
      owner = "offlinegpt-client";
      message = "The selected client workspace has no synced OfflineGPT Cloud MCP entry.";
      break;
    case "cloud_mcp_disabled":
      owner = "offlinegpt-client";
      message = "The selected workspace OfflineGPT Cloud MCP entry is disabled.";
      action = "Enable or reconnect OfflineGPT Cloud from Settings > Connect, then rerun diagnostics.";
      break;
    case "cloud_mcp_not_remote":
      message = "The managed OfflineGPT Cloud entry is not configured as a remote MCP.";
      action = "Reconnect OfflineGPT Cloud to restore its managed remote configuration.";
      break;
    case "invalid_endpoint":
      message = "The managed OfflineGPT Cloud endpoint is not credential-safe or does not end at the required /mcp/agent route.";
      action = "Reconnect OfflineGPT Cloud to restore its managed endpoint, then rerun diagnostics.";
      break;
    case "untrusted_endpoint":
      status = "warning";
      evidenceKind = "unavailable";
      message = probe.enterpriseActivationPresent
        ? "The runtime endpoint probe was not performed: this installation is enterprise activated, but against a different control-plane origin than the configured OfflineGPT Cloud MCP. No request was sent, so this is a configuration mismatch, not a network, TLS, or MCP failure."
      ...
      action = probe.enterpriseActivationPresent
        ? "Reconcile the enterprise activation origin with the configured OfflineGPT Cloud MCP origin, or have an administrator add the exact endpoint origin to OFFLINEGPT_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS, then rerun diagnostics."
        : "Activate this installation against your on-prem Den, or have an administrator set OFFLINEGPT_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS on the OfflineGPT desktop/server process to the exact endpoint origin, then rerun diagnostics.";
      break;
    ...
    case "duplicate_authorization":
      ...
      action = "Reconnect OfflineGPT Cloud so the client can replace the managed credential, then rerun diagnostics.";
      break;
    case "timeout":
    case "network_error":
    case "http_error":
      ...
      if (probe.httpStatus === 451) {
        message = "The independent runtime endpoint probe received HTTP 451, which indicates an egress proxy, firewall, or allowlist deny rather than an OfflineGPT Cloud catalog shape problem.";
        action = "Add the configured OfflineGPT Cloud MCP host and its documented redirect targets to the corporate allowlist, then rerun diagnostics.";
      } else {
        message = "The independent runtime endpoint probe could not be completed through the configured network path.";
        action = "Verify server egress, DNS, TLS, proxy policy, and the configured OfflineGPT Cloud service, then rerun diagnostics.";
      }
      break;
    ...
    case "dns_error":
      ...
      action = "Verify DNS resolution from the OfflineGPT server, then rerun diagnostics.";
      break;
    case "connection_refused":
      owner = "network-admin";
      message = "The configured OfflineGPT Cloud service refused the independent runtime probe connection.";
    ...
    case "tls_error":
      owner = "network-admin";
      message = "The independent runtime endpoint probe failed TLS certificate validation or negotiation on the OfflineGPT runtime trust store.";
    ...
    case "unauthorized":
      owner = "offlinegpt-client";
      message = "OfflineGPT Cloud rejected the configured credential during the independent runtime probe.";
      action = "Reconnect OfflineGPT Cloud so the client can replace the managed credential, then rerun diagnostics.";
      break;
    case "forbidden":
      owner = "organization-admin";
      message = "OfflineGPT Cloud rejected the probe for membership, scope, or policy reasons rather than credential validity.";
    ...
    case "mcp_route_not_found":
      ...
      action = "Verify the OfflineGPT Cloud or on-prem Den deployment version exposes the /mcp/agent route, then rerun diagnostics.";
      break;
    case "rate_limited":
      status = "warning";
      owner = "offlinegpt-support";
      message = "OfflineGPT Cloud rate-limited the independent runtime probe.";
      action = "Wait before rerunning diagnostics; contact OfflineGPT support if rate limiting persists.";
      break;
    case "gateway_unavailable":
      owner = "network-admin";
      message = "A gateway or upstream in front of the OfflineGPT Cloud service reported it unavailable during the independent runtime probe.";
    ...
    case "invalid_catalog":
      owner = "offlinegpt-support";
      message = "OfflineGPT Cloud returned a response that does not satisfy the bounded MCP handshake protocol contract.";
      action = "Review the OfflineGPT Cloud deployment and restore a conformant MCP handshake response.";
      break;
    case "required_tools_missing":
      owner = "offlinegpt-support";
      message = "The OfflineGPT Cloud catalog handshake succeeded, but the catalog does not contain both required capability tools.";
      action = "Review the OfflineGPT Cloud deployment and restore the canonical capability catalog.";
      break;
  }
  return diagnosticCheck({
    ...common,
...
export function cloudDifferentialCheck(probe: CloudCatalogProbe, engineReachableNow: boolean): AgentContextDiagnosticCheck {
  ...
  if (verdict === "runtime_and_engine_connected") {
    return diagnosticCheck({
      ...
      message: "The independent OfflineGPT runtime probe and the engine registration evidence both report the OfflineGPT Cloud endpoint as reachable.",
      owner: "offlinegpt-server",
  ...
  if (verdict === "runtime_connected_engine_failed") {
    return diagnosticCheck({
      ...
      status: "failed",
      evidenceKind: "derived",
      message: "The OfflineGPT runtime reached the Cloud endpoint directly, but the engine registration evidence reports a failure; the engine-side connection path or registration lifecycle is implicated, not the endpoint.",
      owner: "opencode-engine",
      action: "Reconnect OfflineGPT Cloud or restart the selected workspace engine, then rerun diagnostics; the endpoint itself is reachable from this machine.",
    });
  }
  if (verdict === "runtime_failed_engine_connected") {
    return diagnosticCheck({
      ...
      status: "warning",
      evidenceKind: "derived",
      message: "The engine registration evidence reports a live connection, but the independent runtime probe failed; the OfflineGPT runtime network path is implicated rather than the endpoint or the engine.",
      owner: "network-admin",
      action: "Compare proxy, DNS, and trust-store configuration between the OfflineGPT runtime and the engine process, then rerun diagnostics.",
    });
  }
  ...
  if (verdict === "engine_evidence_stale_or_unavailable") {
    return diagnosticCheck({
      ...
      status: "warning",
      evidenceKind: "derived",
      message: "The engine registration evidence for OfflineGPT Cloud is stale or was never recorded, so only the independent runtime observation is current.",
      owner: "opencode-engine",
...
export function cloudEndpointTransportCheck(
  ...
  if (notRequired) {
    return diagnosticCheck({
      ...
      message: "The managed OfflineGPT Cloud MCP registration is connected; the differential check compares runtime and engine reachability, so a separate TLS-layer probe was not needed.",
      owner: "offlinegpt-server",
      action: "No action is required.",
      details,
    });
  ...
  if (probe.verifiedHandshake === "ok") {
    return diagnosticCheck({
      ...
      message: "The OfflineGPT Cloud endpoint completed a credential-free TLS handshake; use the differential check for authentication, MCP protocol, and engine-registration attribution.",
      owner: "offlinegpt-server",
  ...
  if (
    ...
    return diagnosticCheck({
      ...
      owner: "network-admin",
      action: "Fix or bypass the egress device that stalls TLS 1.3 ClientHello traffic for OfflineGPT Cloud hosts, or temporarily force TLS 1.2 where your policy permits it, then rerun diagnostics.",
      details,
    });
  }
  if (probe.verifiedHandshake === "failed" && probe.verifyErrorCode === "ETIMEDOUT") {
    if (
      ...
      return diagnosticCheck({
        ...
        owner: "network-admin",
        action: "Verify the egress proxy and firewall pass TLS ClientHello traffic to OfflineGPT Cloud hosts, then compare with a known Node or openssl TLS 1.2-only probe and confirm every OfflineGPT runtime honors any temporary TLS-version pinning policy.",
        details,
      });
    }
    return diagnosticCheck({
      ...
      owner: "network-admin",
      action: "Verify that the egress proxy and firewall pass TLS 1.3 ClientHello traffic to OfflineGPT Cloud hosts; compare with a TLS 1.2-only probe or temporarily force TLS 1.2 where policy permits, then rerun diagnostics.",
      details,
    });
  }
  if (isCloudEndpointCertificateVerificationFailure(probe)) {
    ...
    if (/corporate|interception|proxy|mitm/iu.test(servedIssuer)) {
      return diagnosticCheck({
        ...
        message: "The OfflineGPT Cloud endpoint appears to be TLS-inspected or re-signed by a corporate proxy; the runtime does not trust that inspecting issuer.",
        owner: "network-admin",
        action: "Install the corporate inspection root for the OfflineGPT runtime with NODE_EXTRA_CA_CERTS, or bypass TLS inspection for OfflineGPT Cloud hosts, then rerun diagnostics.",
        details,
      });
    }
    if (probe.verifyErrorCode === "UNABLE_TO_VERIFY_LEAF_SIGNATURE" && probe.servedChainLength === 1) {
      return diagnosticCheck({
        ...
        message: "The OfflineGPT Cloud endpoint served a leaf-only TLS chain and verification failed with UNABLE_TO_VERIFY_LEAF_SIGNATURE; the missing intermediate/fullchain must be repaired before blaming credentials or application logic.",
        owner: "network-admin",
        action: "Serve the complete certificate chain from the endpoint or let the OfflineGPT runtime add the AIA intermediate to NODE_EXTRA_CA_CERTS, then rerun diagnostics.",
        details,
      });
    }
    return diagnosticCheck({
      ...
      message: `The OfflineGPT Cloud endpoint TLS handshake failed with ${probe.verifyErrorCode ?? "a certificate verification error"}; the OS or corporate CA chain is not visible to this runtime.`,
      owner: "network-admin",
      action: "Provide the corporate CA chain to OfflineGPT with NODE_EXTRA_CA_CERTS or fix the server to present its full certificate chain; the served-chain evidence below shows what the endpoint sent.",
      details,
    });
  }
  if (probe.verifiedHandshake === "failed") {
    return diagnosticCheck({
      ...
      message: "The OfflineGPT Cloud endpoint could not be reached with a credential-free TCP/TLS handshake.",
      owner: "network-admin",
  ...
  return diagnosticCheck({
    ...
    owner: "offlinegpt-server",
    action: "Review the managed OfflineGPT Cloud MCP configuration if transport evidence is needed.",
    details,
  });
}
  ...
  if (request.organizationConnectionsProbe.status === "skipped") {
    ...
    return diagnosticCheck({
      ...
      message: remotePrivacy
        ? "Local Den organization topology was intentionally omitted from the remote OfflineGPT diagnostics request."
        : "Organization connection readiness was not observed for this run.",
      owner: remotePrivacy ? "offlinegpt-client" : "member",
...
function engineAgentCheck(
  ...
  const agent = snapshot.agents.find((candidate) => candidate.name === "offlinegpt");
  return diagnosticCheck({
    id: "engine-agent",
    status: agent ? "passed" : "failed",
    evidenceKind: "observed",
    code: agent ? "effective_offlinegpt_agent_observed" : "effective_offlinegpt_agent_missing",
    message: agent
      ? "The selected engine resolved the OfflineGPT agent."
      : "The selected engine did not resolve an OfflineGPT agent.",
    owner: agent ? "opencode-engine" : "offlinegpt-server",
    action: agent
      ? "No action is required."
      : "Restore the OfflineGPT runtime agent injection and restart the selected workspace engine.",
    details: {
      engineApiReadPerformed: true,
      effectiveAgentCount: snapshot.agents.length,
      offlinegptAgentPresent: Boolean(agent),
      offlinegptAgentHidden: agent?.hidden ?? null,
...
function runtimeHealthCheck(
  ...
  return diagnosticCheck({
    ...
    code,
    message,
    owner: corrupt ? "offlinegpt-server" : engineConfigured ? "offlinegpt-server" : "member",
    action: status === "passed"
      ? "No action is required."
      : corrupt
        ? "Repair the OfflineGPT runtime state before relying on injected configuration."
        : remote
          ? "Run diagnostics on the OfflineGPT server that owns the workspace."
...
export async function runAgentContextDiagnostics(input: {
  ...
  const expectedRuntimeConfig = buildOfflineGptRuntimeConfigObjectFromSnapshot(globalRuntimeInspection.config);
  const expectedAgents = isRecord(expectedRuntimeConfig.agent) ? expectedRuntimeConfig.agent : {};
  const expectedAgent = isRecord(expectedAgents.offlinegpt) ? expectedAgents.offlinegpt : null;
  const effectiveOfflineGptAgent = effectiveEngine?.agents.find((agent) => agent.name === "offlinegpt") ?? null;
  const effectiveAgentModeUsable = effectiveOfflineGptAgent?.mode === "primary"
    || effectiveOfflineGptAgent?.mode === "all";
  const effectiveAgentUsable = Boolean(
    effectiveOfflineGptAgent
    && effectiveEngine?.defaultAgent === "offlinegpt"
    && !effectiveOfflineGptAgent.hidden
  ...
  const reportedAgent = effectiveEngine
    ? effectiveOfflineGptAgent
      ? { prompt: effectiveOfflineGptAgent.prompt }
      : null
    : expectedAgent;
  ...
  const canonicalConnectPluginSpec = expectedPlugins.find(
    (spec) => pluginLabel(spec) === "offlinegpt-extensions-preview",
  ) ?? null;
  ...
  const selectedCloudMcpPresent = Object.hasOwn(runtimeMcpMap(runtime), OFFLINEGPT_CLOUD_MCP_NAME);
  ...
  const engineConfigItems = effectiveEngine?.mcps.map((item) => ({
    ...
    disabledByTools: item.name === OFFLINEGPT_CLOUD_MCP_NAME
  ...
  const runtimeCloudItem = combinedInventoryItems.find((item) =>
    item.source === "config.remote" && item.name === OFFLINEGPT_CLOUD_MCP_NAME,
  );
  if (
    runtimeCloudItem
  ...
  const runtimeCloudConfig = runtimeMcpMap(runtime)[OFFLINEGPT_CLOUD_MCP_NAME] ?? null;
  ...
  const cloudProbe = await probeOfflineGptCloudCatalog({
  ...
  const checks: AgentContextDiagnosticCheck[] = [
    ...
    diagnosticCheck({
      ...
      message: !connectSnapshotAvailable
        ...
        : crossWorkspaceSteeringDrift
          ? "Global Connect steering sees OfflineGPT Cloud, but the selected workspace does not contain that managed MCP."
          : "The expected Connect steering branch is internally consistent for the selected workspace.",
      owner: !connectSnapshotAvailable || crossWorkspaceSteeringDrift ? "offlinegpt-server" : "offlinegpt-client",
      action: !connectSnapshotAvailable
        ? "Verify the OfflineGPT server runtime state and rerun diagnostics."
        : crossWorkspaceSteeringDrift
          ? "Reconnect or sync OfflineGPT Cloud for the selected workspace."
          : "No action is required.",
      details: {
    ...
    diagnosticCheck({
      ...
      code: effectiveEngine
        ? !effectiveOfflineGptAgent
          ? "effective_offlinegpt_agent_missing"
          : effectiveEngine.defaultAgent !== "offlinegpt"
            ? "effective_default_agent_mismatch"
            : effectiveOfflineGptAgent.hidden
              ? "effective_offlinegpt_agent_hidden"
              : !effectiveAgentModeUsable
                ? "effective_offlinegpt_agent_not_primary"
            : "effective_offlinegpt_agent_selected"
        : projectOverrideDetected
          ? "configured_agent_has_override_layers"
          : "runtime_agent_intent_only",
      message: effectiveEngine
        ? !effectiveOfflineGptAgent
          ? "The effective engine configuration does not contain the OfflineGPT agent."
          : effectiveEngine.defaultAgent !== "offlinegpt"
            ? "The effective engine default does not select the OfflineGPT agent."
            : effectiveOfflineGptAgent.hidden
              ? "The effective OfflineGPT agent is hidden and cannot be used as the default agent."
              : !effectiveAgentModeUsable
                ? "The effective OfflineGPT agent is subagent-only and cannot be used as the default agent."
            : "The effective engine default selects the resolved OfflineGPT agent."
        : projectOverrideDetected
          ? "The configured OfflineGPT agent intent has project override layers and could not be confirmed live."
          : "Only the configured OfflineGPT agent intent was available; effective resolution was not observed.",
      ...
      action: effectiveEngine && effectiveAgentUsable
        ? "No action is required."
        : effectiveEngine
          ? "Restore the OfflineGPT agent and default-agent injection, then restart the selected workspace engine."
          : "Check the selected workspace engine health and rerun diagnostics.",
      details: {
        configuredAgentPresent: Boolean(expectedAgent),
        effectiveAgentPresent: effectiveEngine ? Boolean(effectiveOfflineGptAgent) : null,
        effectiveDefaultAgentIsOfflineGpt: effectiveEngine ? effectiveEngine.defaultAgent === "offlinegpt" : null,
        effectiveAgentHidden: effectiveOfflineGptAgent?.hidden ?? null,
        effectiveAgentMode: effectiveOfflineGptAgent?.mode ?? null,
    ...
    diagnosticCheck({
      ...
      message: promptMatchesCanonicalIntent
        ? effectiveEngine
          ? "The effective OfflineGPT base prompt exactly matches the canonical configured injection and contains every required marker."
          : "The configured OfflineGPT base prompt intent matches its canonical generated injection and contains every required marker."
        : !promptMarkersPresent
          ? effectiveEngine
            ? "The effective OfflineGPT base prompt is missing one or more required markers."
            : "The configured OfflineGPT base prompt intent is missing one or more required markers."
          : effectiveEngine
            ? "The effective OfflineGPT base prompt contains the markers but does not match the canonical configured injection."
            : "The configured OfflineGPT base prompt markers are present, but its digest does not match the canonical generated injection.",
      owner: effectiveEngine ? "opencode-engine" : "offlinegpt-server",
      action: promptMatchesCanonicalIntent
        ? "No action is required."
        : "Restore the canonical OfflineGPT runtime agent definition.",
      details: {
        ...prompt.markers,
    ...
    diagnosticCheck({
      ...
      message: cloudToolPolicyStatus === "denied"
        ? !effectiveEngine && staticallyDeniedCloudAgentToolIds.size > 0
          ? "A passively inspected static OpenCode policy denies one or more required OfflineGPT Cloud capability tools."
          : "The effective OpenCode agent policy hides one or more required OfflineGPT Cloud capability tools."
        : cloudToolPolicyStatus === "unavailable"
          ? "Required OfflineGPT Cloud tool visibility could not be verified from the effective selected-engine agent."
          : "The effective OpenCode agent policy does not deny either required OfflineGPT Cloud candidate tool ID; the live engine tool registry was not read.",
      ...
      action: cloudToolPolicyStatus === "denied"
        ? "Allow the denied offlinegpt-cloud capability tool IDs in top-level or OfflineGPT agent permission policy, then rerun diagnostics."
        : cloudToolPolicyStatus === "unavailable"
          ? "Check the selected workspace engine health and rerun diagnostics."
      ...
      details: {
        searchCapabilities: reportedToolPermission(`${OFFLINEGPT_CLOUD_MCP_NAME}_search_capabilities`),
        executeCapability: reportedToolPermission(`${OFFLINEGPT_CLOUD_MCP_NAME}_execute_capability`),
    ...
    diagnosticCheck({
      ...
      action: canonicalPluginSpecMatched
        ? "No action is required."
        : "Restore the canonical OfflineGPT runtime plugin bundle.",
      details: {
    ...
    diagnosticCheck({
      ...
      message: effectiveEngine
        ? inventoryTotal > 200
          ...
          : "The selected engine's merged MCP configuration and OfflineGPT-managed dynamic injection intent were inventoried as separate evidence sources."
        : layerHealthProblem
      ...
      owner: effectiveEngine
        ? "opencode-engine"
        : layerHealthProblem ? "member" : inventory.collisions.length > 0 ? "member" : "offlinegpt-server",
      action: effectiveEngine
        ? inventoryTotal > 200
          ? "Reduce the configured MCP count or inspect the engine and OfflineGPT runtime sources directly."
    ...
    diagnosticCheck({
      ...
      message: failedRegistrationCount > 0
        ...
        : missingRegistrationCount > 0
          ...
          : remoteMcps.length > 0
            ? "Every enabled OfflineGPT-managed MCP has a current connected registration result; configured-disabled entries are not treated as injected tools."
            : "No server-managed MCP registration was available to inspect.",
      owner: failedRegistrationCount > 0 || missingRegistrationCount > 0 ? "opencode-engine" : "offlinegpt-server",
    ...
    diagnosticCheck({
      ...
      owner: "opencode-engine",
      action: "Review the bounded OfflineGPT Cloud catalog probe and exact managed registration response evidence instead.",
      details: {
  ...
  const report: AgentContextDiagnosticsReport = {
    ...
    agent: {
      ...
      configuredOfflineGptAgent: {
        state: effectiveEngine
          ? effectiveOfflineGptAgent ? "present" : "missing"
          : expectedAgent?.disable === true
            ? "configured-disabled"
            : expectedAgent
              ? "present"
              : "missing",
        mode: effectiveOfflineGptAgent
          ? effectiveOfflineGptAgent.mode
