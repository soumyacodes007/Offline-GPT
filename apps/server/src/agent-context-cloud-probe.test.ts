...
import {
  ...
  type ProbeOfflineGptCloudCatalogInput,
} from "./agent-context-cloud-probe.js";

const TOKEN = "Bearer ow_diagnostics_token_abcdefghijklmnopqrstuvwxyz";
const ENDPOINT = "https://app.offlinegptlabs.com/api/den/mcp/agent";
const SESSION_ID = "diagnostics-session-id";
const PROTOCOL_VERSION = "2025-06-18";

function input(overrides: Partial<ProbeOfflineGptCloudCatalogInput> = {}): ProbeOfflineGptCloudCatalogInput {
...
function initializeResponse(extraHeaders: Record<string, string> = {}): Response {
  return new Response(JSON.stringify({
    jsonrpc: "2.0",
    id: "offlinegpt-agent-diagnostics-initialize",
    result: {
      capabilities: {},
...
afterEach(() => {
  delete process.env.OFFLINEGPT_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS;
});

describe("OfflineGPT Cloud catalog probe", () => {
  test("performs initialize, initialized notification, bounded tools/list, and session cleanup with allowlisted headers", async () => {
    ...
    const observed = await probeOfflineGptCloudCatalog(probeInput);
    expect(calls).toBe(4);
  ...
  test("accepts a finite JSON tools/list response", async () => {
    const observed = await probeOfflineGptCloudCatalog(input({
  ...
  test("requires both expected tools and never reflects a provider-controlled tool ID", async () => {
    ...
    const missing = await probeOfflineGptCloudCatalog(input({
    ...
    const reversed = await probeOfflineGptCloudCatalog(input({
  ...
  test("blocks remote workspaces, unavailable state, and unsafe endpoints before fetch", async () => {
    ...
    const cases: Array<[Partial<ProbeOfflineGptCloudCatalogInput>, CloudCatalogProbeCode]> = [
      ...
      [{ config: { type: "remote", enabled: true, url: "https://app.offlinegptlabs.com/api/den/mcp/agent?token=secret", headers: { Authorization: TOKEN } } }, "invalid_endpoint"],
      [{ config: { type: "remote", enabled: true, url: "https://app.offlinegptlabs.com/api/den/mcp/agent/", headers: { Authorization: TOKEN } } }, "invalid_endpoint"],
      [{ config: { type: "remote", enabled: true, url: "https://app.offlinegptlabs.com/api/den/mcp/agent/status", headers: { Authorization: TOKEN } } }, "invalid_endpoint"],
      [{ config: { type: "remote", enabled: true, url: "https://app.offlinegptlabs.com/api/den/mcp/agentish", headers: { Authorization: TOKEN } } }, "invalid_endpoint"],
      [{ config: { type: "remote", enabled: true, url: "http://app.offlinegptlabs.com/mcp/agent", headers: { Authorization: TOKEN } } }, "invalid_endpoint"],
      [{ config: { type: "remote", enabled: true, url: "https://localhost.evil/mcp/agent", headers: { Authorization: TOKEN } } }, "untrusted_endpoint"],
    ];
    for (const [overrides, code] of cases) {
      const blocked = await probeOfflineGptCloudCatalog(input({ ...overrides, fetchImpl }));
  ...
  test("allows exact loopback and explicitly configured HTTPS origins only", async () => {
    let blockedCalls = 0;
    const blocked = await probeOfflineGptCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
    ...
    const loopback = await probeOfflineGptCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
    ...
    process.env.OFFLINEGPT_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS = "https://den.customer.example";
    const trusted = await probeOfflineGptCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
        url: "https://den.customer.example/custom/mcp/agent",
  ...
  test("rejects duplicate or malformed authorization without forwarding configured headers", async () => {
    ...
    const duplicate = await probeOfflineGptCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
        url: ENDPOINT,
    ...
    const injected = await probeOfflineGptCloudCatalog(input({
      config: {
        type: "remote",
        enabled: true,
        url: ENDPOINT,
  ...
  test("rejects redirects and cancels a response that exceeds 64 KiB", async () => {
    const redirected = await probeOfflineGptCloudCatalog(input({
    ...
    const tooLarge = await probeOfflineGptCloudCatalog(input({
  ...
  test("keeps the absolute deadline active while a response body stalls", async () => {
    ...
    const timedOut = await probeOfflineGptCloudCatalog(input({
  ...
  test("classifies abort-aware fetch and body failures as deadline timeouts", async () => {
    const fetchTimedOut = await probeOfflineGptCloudCatalog(input({
    ...
    const bodyTimedOut = await probeOfflineGptCloudCatalog(input({
  ...
  test("does not start authenticated egress when the parent signal is already aborted", async () => {
    ...
    controller.abort();
    let calls = 0;
    const cancelled = await probeOfflineGptCloudCatalog(input({
  ...
  test("parent abort settles a hostile fetch that ignores its signal", async () => {
    ...
    const pending = probeOfflineGptCloudCatalog(input({
  ...
  test("keeps each caller cancellation-scoped and does not cache settled results", async () => {
    ...
    const first = probeOfflineGptCloudCatalog(input({ requestId: "single-flight-one", fetchImpl }));
    const joined = probeOfflineGptCloudCatalog(input({ requestId: "single-flight-two", fetchImpl }));
    ...
    const afterSettlement = await probeOfflineGptCloudCatalog(input({
  ...
  test("caps global active probes without sharing a caller-owned request", async () => {
    ...
    const active = Array.from({ length: 16 }, (_, index) => probeOfflineGptCloudCatalog(input({
    ...
    const sameFingerprintBusy = await probeOfflineGptCloudCatalog(input({
      workspaceId: "ws_busy_0",
      requestId: "busy-joined",
      fetchImpl,
    }));
    const busy = await probeOfflineGptCloudCatalog(input({
  ...
  test("returns only a safe network code when fetch throws a secret-bearing error", async () => {
    const failed = await probeOfflineGptCloudCatalog(input({
  ...
  test("attributes initialize-phase failures to initialize stages", async () => {
    ...
    const failed = await probeOfflineGptCloudCatalog(probeInput);
    expect(failed).toMatchObject({
      performed: true,
      status: "failed",
  ...
  test("splits invalid UTF-8, invalid JSON, and malformed envelopes into distinct codes", async () => {
    const utf8 = await probeOfflineGptCloudCatalog(input({
      requestId: "bad-utf8",
    ...
    const json = await probeOfflineGptCloudCatalog(input({
      requestId: "bad-json",
    ...
    const envelope = await probeOfflineGptCloudCatalog(input({
  ...
  test("exports a reference ID only from a strictly validated safe header", async () => {
    const safe = await probeOfflineGptCloudCatalog(input({
    ...
    const unsafe = await probeOfflineGptCloudCatalog(input({
  ...
  test("reports boolean-only proxy and extra-CA posture from the environment seam", async () => {
    const configured = await probeOfflineGptCloudCatalog(input({
  ...
  test("trusts the activated enterprise origin and reports the trust source", async () => {
    ...
    // so an on-prem probe must never contact OfflineGPT-hosted Cloud.
    ...
    const activated = await probeOfflineGptCloudCatalog(activatedInput);
    ...
    expect(requestedUrls.some((url) => offlineGptHostedOrigins.has(new URL(url).origin))).toBe(false);
    ...
    let mismatchCalls = 0;
    const mismatch = await probeOfflineGptCloudCatalog(input({
    ...
    const unactivated = await probeOfflineGptCloudCatalog(input({
