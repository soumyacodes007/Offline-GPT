...
async function writeFakeOpencodeBin(root: string): Promise<string> {
  ...
  await writeFile(binPath, [
    ...
    "const capturePath = process.env.OFFLINEGPT_CAPTURE_MODELS_URL_FILE;",
...
describe("resolveOpencodeModelsUrl", () => {
  ...
  test("uses the production catalog outside development", async () => {
    expect(await resolveOpencodeModelsUrl({ env: {} })).toBe("https://models.offlinegptlabs.com/");
  });
...
describe("startEmbeddedServer managed OpenCode models URL", () => {
  test("injects an explicit OPENCODE_MODELS_URL override", async () => {
    ...
    const previousCapturePath = process.env.OFFLINEGPT_CAPTURE_MODELS_URL_FILE;
    const previousHome = process.env.HOME;
    const previousOpencodeBaseUrl = process.env.OFFLINEGPT_OPENCODE_BASE_URL;

    try {
      process.env.OFFLINEGPT_DEV_MODE = "1";
      process.env.OPENCODE_MODELS_URL = "https://catalog.example.test/models";
      process.env.OFFLINEGPT_CAPTURE_MODELS_URL_FILE = capturePath;
      ...
      restoreProcessEnv("OFFLINEGPT_DEV_MODE", previousDevMode);
      restoreProcessEnv("OPENCODE_MODELS_URL", previousModelsUrl);
      restoreProcessEnv("OFFLINEGPT_CAPTURE_MODELS_URL_FILE", previousCapturePath);
      restoreProcessEnv("HOME", previousHome);
      restoreProcessEnv("OFFLINEGPT_OPENCODE_BASE_URL", previousOpencodeBaseUrl);
