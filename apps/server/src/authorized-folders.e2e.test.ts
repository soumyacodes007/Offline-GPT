...
const roots: string[] = [];
const priorDataDir = process.env.OFFLINEGPT_DATA_DIR;
...
async function createWorkspaceRoot() {
  return createTempRoot("offlinegpt-authorized-folders-");
}

async function startOfflineGptServer(workspaceRoot: string, options?: { readOnly?: boolean }) {
...
beforeEach(async () => {
  const envRoot = await createTempRoot("offlinegpt-authorized-folders-env-");
  process.env.OFFLINEGPT_DATA_DIR = join(envRoot, "data");
  process.env.OFFLINEGPT_TOKEN_STORE = join(envRoot, "tokens.json");
});
...
afterEach(async () => {
  ...
  if (priorDataDir === undefined) {
    delete process.env.OFFLINEGPT_DATA_DIR;
  } else {
    process.env.OFFLINEGPT_DATA_DIR = priorDataDir;
  }
...
describe("authorized folders routes", () => {
  ...
  test("lists visible folders and counts preserved hidden entries", async () => {
    ...
    const { base, config } = await startOfflineGptServer(root);
  ...
  test("requires client auth, collaborator scope, and writable server", async () => {
    ...
    const { base } = await startOfflineGptServer(root);
    ...
    const readOnly = await startOfflineGptServer(readOnlyRoot, { readOnly: true });
