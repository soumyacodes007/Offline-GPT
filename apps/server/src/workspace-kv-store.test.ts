...
async function tempWorkspace(): Promise<{ root: string; dbPath: string; config: ServerConfig }> {
  const root = await mkdtemp(join(tmpdir(), "offlinegpt-workspace-kv-store-"));
  roots.push(root);
...
function corruptStoreJson(dbPath: string): void {
  ...
  try {
    ...
    sqlite.query("UPDATE offlinegpt_workspace_configs SET config_json = ? WHERE workspace_id = ?").run("{", WORKSPACE_ID);
...
describe("workspace kv store", () => {
  ...
  test("returns each migrated store's documented defaults for missing and malformed rows", async () => {
    ...
    expect(await readRuntimeOpencodeConfig(config, WORKSPACE_ID)).toEqual({});
    expect(await readOfflineGptWorkspaceConfig(config, WORKSPACE_ID)).toEqual({});
    ...
    await writeOfflineGptWorkspaceConfig(config, WORKSPACE_ID, () => ({ workspace: { name: "Runtime" } }));
