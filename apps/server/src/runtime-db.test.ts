...
async function tempRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "offlinegpt-runtime-db-primitive-"));
  roots.push(root);
  return root;
}
...
describe("runtime DB primitive", () => {
  test("resolves default and env-overridden runtime database paths", async () => {
    ...
    delete process.env.OFFLINEGPT_RUNTIME_DB;
    ...
    expect(runtimeDbPath(configWithoutPath)).toBe(join(homedir(), ".config", "offlinegpt", "runtime.sqlite"));

    const overridePath = join(root, "state", "override.sqlite");
    process.env.OFFLINEGPT_RUNTIME_DB = ` ${overridePath} `;
    ...
    process.env.OFFLINEGPT_RUNTIME_DB = "   ";
    expect(runtimeDbPath(config)).toBe(join(root, "runtime.sqlite"));
  });
  ...
  test("keeps each migrated store in its own runtime DB table", async () => {
    ...
    await writeOfflineGptWorkspaceConfig(config, WORKSPACE_ID, (current) => ({
      ...current,
    ...
    expect((await readOfflineGptWorkspaceConfig(config, WORKSPACE_ID)).workspace).toEqual({ label: "Workspace config" });
