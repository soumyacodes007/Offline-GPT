...
async function startOfflineGpt(options?: { branch?: string }) {
  const workspaceRoot = await mkdtemp(join(tmpdir(), "offlinegpt-claude-plugin-"));
  roots.push(workspaceRoot);
  setEnv("OFFLINEGPT_RUNTIME_DB", join(workspaceRoot, "runtime.sqlite"));

  const github = startMockGithub(options);
  setEnv("OFFLINEGPT_GITHUB_API_BASE", `http://127.0.0.1:${github.port}`);
  setEnv("OFFLINEGPT_GITHUB_RAW_BASE", `http://127.0.0.1:${github.port}`);
...
describe("claude plugin bundles", () => {
  test("dryRun returns the Will-install preview with warnings", async () => {
    ...
    const response = await fetch(`${offlinegpt.base}/workspace/ws_1/claude-plugins`, {
      method: "POST",
      headers: offlinegpt.headers,
    ...
    expect(existsSync(join(offlinegpt.workspaceRoot, ".opencode/skills/slack-plugin"))).toBe(false);
  });

  test("resolves branch names containing slashes in /tree/ URLs", async () => {
    const offlinegpt = await startOfflineGpt({ branch: "release/v1" });
  ...
  test("installs skills, commands, and MCP servers; uninstall cleans up", async () => {
    ...
    const installResponse = await fetch(`${offlinegpt.base}/workspace/ws_1/claude-plugins`, {
      method: "POST",
      headers: offlinegpt.headers,
    ...
    const skillPath = join(offlinegpt.workspaceRoot, ".opencode/skills/slack-plugin/slack-search/SKILL.md");
    const commandPath = join(offlinegpt.workspaceRoot, ".opencode/commands/slack-plugin/standup.md");
    ...
    const listResponse = await fetch(`${offlinegpt.base}/workspace/ws_1/mcp`, { headers: offlinegpt.headers });
    ...
    expect(offlinegpt.engine.requests.some((entry) => entry.method === "POST" && entry.pathname === "/mcp")).toBe(true);

    // Uninstall through the shared cloud-plugins route.
    const removeResponse = await fetch(
      `${offlinegpt.base}/workspace/ws_1/cloud-plugins/${encodeURIComponent(installBody.item.pluginId)}`,
      { method: "DELETE", headers: offlinegpt.headers },
    );
    expect(removeResponse.status).toBe(200);
    expect(existsSync(skillPath)).toBe(false);
    expect(existsSync(commandPath)).toBe(false);
    const afterRemove = await fetch(`${offlinegpt.base}/workspace/ws_1/mcp`, { headers: offlinegpt.headers });
