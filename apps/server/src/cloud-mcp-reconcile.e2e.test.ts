...
import { OFFLINEGPT_CLOUD_EXPECTED_TOOLS, OFFLINEGPT_CLOUD_PLUGIN_CANARIES, clearOfflineGptCloudMcpProbeFlights, cloudMcpDeliveryState } from "./cloud-mcp-health.js";
import {
  CONNECT_MCP_SERVER_INDEX_SCHEMA_VERSION,
  CONNECT_MCP_SERVER_INDEX_URI,
  readOfflineGPTConnectMcpAppHostCatalog,
  type OfflineGPTConnectMcpServerIndexInput,
...
type MockOpencodeOptions = {
  ...
  connectServers?: OfflineGPTConnectMcpServerIndexInput["servers"];
...
const cloudConfigsByOfflineGptBase = new Map<string, CloudConfig>();
...
afterEach(async () => {
  ...
  cloudConfigsByOfflineGptBase.clear();
...
async function createRoot(prefix = "offlinegpt-cloud-mcp-"): Promise<string> {
...
async function createRuntimeDbRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "offlinegpt-cloud-mcp-runtime-"));
  runtimeDbRoots.push(root);
  return root;
}

function allReadyToolIds(): string[] {
  return [...OFFLINEGPT_CLOUD_EXPECTED_TOOLS, ...OFFLINEGPT_CLOUD_PLUGIN_CANARIES];
}
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (url.pathname.startsWith("/mcp/") && url.pathname.endsWith("/disconnect") && request.method === "POST") {
        ...
        if (url.pathname === "/mcp/offlinegpt-cloud/disconnect") registerCount = 0;
      ...
      if (url.pathname === "/mcp" && request.method === "GET") {
        ...
        if (options.cloudFailedError) {
          return Response.json({ "offlinegpt-cloud": { status: "failed", error: options.cloudFailedError } });
        }
        if (options.connectAfterStatusReads && statusReads < options.connectAfterStatusReads) {
          return Response.json({ "offlinegpt-cloud": { status: "failed", error: "slow connect" } });
        }
        return Response.json(registerCount > 0 || options.initialConnected ? { "offlinegpt-cloud": { status: "connected" } } : {});
      }
      ...
      if (url.pathname === "/provider") {
        ...
        const models = options.providerModelExists === false
          ...
          : {
              ...
              "gpt-5": { id: "gpt-5", providerID: "offlinegpt", name: "GPT-5", capabilities: { toolcall } },
            };
        return Response.json({
          all: [
            { id: "anthropic", name: "Anthropic", source: "config", env: [], options: {}, models },
            { id: "offlinegpt", name: "OfflineGPT", source: "config", env: [], options: {}, models },
          ],
          default: {},
          connected: ["anthropic", "offlinegpt"],
        });
      }
...
async function startOfflineGpt(workspaces: WorkspaceInfo[]): Promise<{ base: string; config: ServerConfig }> {
  ...
  cloudConfigsByOfflineGptBase.set(base, cloudConfig(cloudUrlFromBase(workspaces[0]?.baseUrl)));
  return { base, config };
}
...
function cloudConfigForOfflineGpt(base: string): CloudConfig {
  return cloudConfigsByOfflineGptBase.get(base) ?? CLOUD_CONFIG;
}

async function reconcile(base: string, workspaceId = "ws_1", body: Record<string, unknown> = {}): Promise<Response> {
  const config = body.config ?? cloudConfigsByOfflineGptBase.get(base) ?? CLOUD_CONFIG;
  return fetch(`${base}/workspace/${workspaceId}/mcp/offlinegpt-cloud/reconcile`, {
    method: "POST",
    headers: headers(),
...
async function getHealth(base: string, workspaceId = "ws_1", query = ""): Promise<Response> {
  return fetch(`${base}/workspace/${workspaceId}/mcp/offlinegpt-cloud/health${query}`, { headers: headers() });
}

describe("offlinegpt-cloud MCP strict reconcile", () => {
  test("clean ready persists desired config, verifies tools, and redacts the token", async () => {
    const root = await createRoot();
    const mock = startMockOpencode();
    const offlinegpt = await startOfflineGpt([workspace("ws_1", root, `http://127.0.0.1:${mock.server.port}`)]);

    const response = await reconcile(offlinegpt.base, "ws_1", {
    ...
    expect(requireArray(tools.present, "tools.present").sort()).toEqual([...OFFLINEGPT_CLOUD_EXPECTED_TOOLS].sort());
    ...
    expect((await readGlobalRuntimeOpencodeConfig(offlinegpt.config)).mcp?.["offlinegpt-cloud"]?.url).toBe(cloudConfigForOfflineGpt(offlinegpt.base).url);
    expect((await readRuntimeOpencodeConfig(offlinegpt.config, "ws_1")).mcp?.["offlinegpt-cloud"]).toBeUndefined();
  ...
  test("keeps provider descriptors private while purging stale runtime endpoints", async () => {
    ...
    const offlinegpt = await startOfflineGpt([workspace("ws_1", root, `http://127.0.0.1:${mock.server.port}`)]);
    await writeRuntimeOpencodeConfig(offlinegpt.config, "ws_1", () => ({
      mcp: {
    ...
    const runtime = await readRuntimeOpencodeConfig(offlinegpt.config, "ws_1");
    expect(runtime.mcp?.["user-server"]).toEqual({ type: "remote", url: "https://user.example/mcp" });
    expect(runtime.mcp?.["offlinegpt-cloud"]).toBeUndefined();
    expect((await readGlobalRuntimeOpencodeConfig(offlinegpt.config)).mcp?.["offlinegpt-cloud"]).toBeTruthy();
    expect(Object.keys(runtime.mcp ?? {}).filter((name) => name.startsWith("offlinegpt-connect-"))).toEqual([]);
    expect((await readOfflineGPTConnectMcpAppHostCatalog(offlinegpt.config, "ws_1")).servers).toEqual([
    ...
    expect(registrations.map((request) => requireRecord(request.body, "registration").name)).toEqual(["offlinegpt-cloud"]);
    expect(mock.requests.some((request) => request.pathname === "/mcp/offlinegpt-connect-stale/disconnect")).toBe(true);
  });
  ...
  test("live status read heals a failed registration record after reconcile returns early", async () => {
    ...
    const offlinegpt = await startOfflineGpt([workspace("ws_1", root, baseUrl)]);
    ...
    const response = await reconcile(offlinegpt.base);
    ...
    expect(inspectEngineMcpRegistration(
      ...
      "offlinegpt-cloud",
      cloudConfigForOfflineGpt(offlinegpt.base),
    )).toBe("connected");

    const health = await responseRecord(await getHealth(offlinegpt.base));
  ...
  test("rejects malformed desired config without persisting or registering it", async () => {
    ...
    const cases: Array<{ config: Record<string, unknown>; code: string }> = [
      { config: { ...CLOUD_CONFIG, url: "https://api.offlinegptlabs.com/mcp" }, code: "cloud_endpoint_invalid" },
    ...
    for (const item of cases) {
      const body = await responseRecord(await reconcile(offlinegpt.base, "ws_1", { config: item.config }));
      expect(firstFailure(body).code).toBe(item.code);
      expect(firstFailure(body).stage).toBe("desired_config");
    }

    const mismatch = await responseRecord(await reconcile(offlinegpt.base, "ws_1", {
  ...
  test("a collaborator token cannot globally persist an untrusted Cloud MCP endpoint", async () => {
    ...
    expect((await readGlobalRuntimeOpencodeConfig(offlinegpt.config)).mcp?.["offlinegpt-cloud"]).toBeUndefined();
    expect((await readRuntimeOpencodeConfig(offlinegpt.config, "ws_1")).mcp?.["offlinegpt-cloud"]).toBeUndefined();
    expect(mock.requests.some((request) => request.method === "POST" && request.pathname === "/mcp")).toBe(false);
  });
  ...
  test("normalizes a harmless trailing slash on the Cloud MCP endpoint", async () => {
    ...
    const body = await responseRecord(await reconcile(offlinegpt.base, "ws_1", {
      config: { ...CLOUD_CONFIG, url },
    }));
    expect(body.phase).toBe("ready");
    expect((await readGlobalRuntimeOpencodeConfig(offlinegpt.config)).mcp?.["offlinegpt-cloud"]?.url).toBe(url.slice(0, -1));
  });
  ...
  test("GET health reports persisted malformed desired config even when the engine looks live", async () => {
    ...
    await writeRuntimeOpencodeConfig(offlinegpt.config, "ws_1", (current) => ({
      ...current,
      mcp: { "offlinegpt-cloud": { ...CLOUD_CONFIG, url: "https://api.offlinegptlabs.com/mcp" } },
    }));

    const body = await responseRecord(await getHealth(offlinegpt.base));
  ...
  test("GET health safely adopts a live exact match before reporting ready", async () => {
    ...
    await writeRuntimeOpencodeConfig(offlinegpt.config, "ws_1", (current) => ({
      ...current,
      mcp: { "offlinegpt-cloud": cloudConfigForOfflineGpt(offlinegpt.base) },
    }));
  ...
  test("GET health runs the direct Cloud endpoint probe only when requested", async () => {
    ...
    const defaultBody = await responseRecord(await getHealth(offlinegpt.base));
    expect(defaultBody.phase).toBe("ready");
    expect(mock.requests.filter((request) => request.pathname.endsWith("/mcp/agent")).length).toBe(0);

    const probedBody = await responseRecord(await getHealth(offlinegpt.base, "ws_1", "?probe=1"));
  ...
  test("health probe timeout is bounded", async () => {
    const previousTimeout = process.env.OFFLINEGPT_CLOUD_MCP_PROBE_TIMEOUT_MS;
    process.env.OFFLINEGPT_CLOUD_MCP_PROBE_TIMEOUT_MS = "25";
    try {
      const root = await createRoot();
      ...
      if (previousTimeout === undefined) delete process.env.OFFLINEGPT_CLOUD_MCP_PROBE_TIMEOUT_MS;
      else process.env.OFFLINEGPT_CLOUD_MCP_PROBE_TIMEOUT_MS = previousTimeout;
    }
  });

  test("unreachable engine leaves desired config persisted but not applied", async () => {
    const root = await createRoot();
    const offlinegpt = await startOfflineGpt([workspace("ws_1", root, "http://127.0.0.1:9")]);
  ...
  test("uses the exact secondary workspace directory", async () => {
    const rootA = await createRoot("offlinegpt-cloud-primary-");
    const rootB = await createRoot("offlinegpt-cloud-secondary-");
    const mock = startMockOpencode();
    const baseUrl = `http://127.0.0.1:${mock.server.port}`;
    ...
    const response = await reconcile(offlinegpt.base, "ws_2");
    const body = await responseRecord(response);
    expect(body.phase).toBe("ready");
  ...
  test("removes global desired state and disconnects every workspace directory", async () => {
    const rootA = await createRoot("offlinegpt-cloud-remove-a-");
    const rootB = await createRoot("offlinegpt-cloud-remove-b-");
    const mock = startMockOpencode();
    const baseUrl = `http://127.0.0.1:${mock.server.port}`;
    const offlinegpt = await startOfflineGpt([
      workspace("ws_1", rootA, baseUrl),
    ...
    expect((await responseRecord(await reconcile(offlinegpt.base))).phase).toBe("ready");
    const disconnectsBeforeRemoval = mock.requests
      .filter((request) => request.pathname === "/mcp/offlinegpt-cloud/disconnect").length;

    const removed = await fetch(`${offlinegpt.base}/workspace/ws_1/mcp/offlinegpt-cloud`, {
      method: "DELETE",
      headers: headers(),
    });
    ...
    const disconnectDirectories = mock.requests
      .filter((request) => request.pathname === "/mcp/offlinegpt-cloud/disconnect")
  ...
  test("uses an explicit remote workspace directory and refuses ambiguous remotes", async () => {
    ...
    const ready = await reconcile(offlinegpt.base, "ws_remote");
    ...
    const ambiguous = await reconcile(offlinegpt.base, "ws_ambiguous");
  ...
  test("connected engine with direct Cloud endpoint missing a tool reports cloud_tools_missing without re-registering", async () => {
    ...
    const body = await responseRecord(await reconcile(offlinegpt.base));
  ...
  test("current OpenCode engines that exclude MCP tool IDs use direct tools/list plus provider capability", async () => {
    const root = await createRoot();
    const mock = startMockOpencode({
      toolIds: [...OFFLINEGPT_CLOUD_PLUGIN_CANARIES],
      providerToolIds: [...OFFLINEGPT_CLOUD_PLUGIN_CANARIES],
      cloudToolsAsSse: true,
    });
    const offlinegpt = await startOfflineGpt([workspace("ws_1", root, `http://127.0.0.1:${mock.server.port}`)]);

    const body = await responseRecord(await reconcile(offlinegpt.base, "ws_1", { provider: "anthropic", model: "claude" }));
    expect(body.phase).toBe("ready");
    expect(body.usable).toBe(true);
    expect(body.usableByCurrentModel).toBe(true);
    ...
    expect(requireRecord(requireRecord(body.tools, "tools").providerProjection, "projection")).toMatchObject({
      ...
      modelExists: true,
      toolCalling: true,
      missing: [...OFFLINEGPT_CLOUD_EXPECTED_TOOLS],
    });
    expect(requireArray(requireRecord(body.tools, "tools").present, "tools.present").sort()).toEqual([...OFFLINEGPT_CLOUD_EXPECTED_TOOLS].sort());
  });
  ...
  test("reports provider projection missing when fallback provider model lacks tool calling", async () => {
    ...
    expect(firstFailure(body).recommendedAction).toBe("Choose a model that can use OfflineGPT Cloud tools");
    expect(requireRecord(requireRecord(body.tools, "tools").providerProjection, "projection")).toMatchObject({
      source: "provider_capability",
      modelExists: true,
      toolCalling: false,
  ...
  test("reports extension canary missing when docs canary is present but extension canary is absent", async () => {
    const root = await createRoot();
    const mock = startMockOpencode({ toolIds: [...OFFLINEGPT_CLOUD_EXPECTED_TOOLS, "offlinegpt_docs_search"] });
    const offlinegpt = await startOfflineGpt([workspace("ws_1", root, `http://127.0.0.1:${mock.server.port}`)]);

    const body = await responseRecord(await reconcile(offlinegpt.base));
    expect(firstFailure(body).code).toBe("extensions_plugin_missing");
    expect(requireArray(requireRecord(body.pluginCanaries, "pluginCanaries").missing, "missing")).toContain("offlinegpt_query");
  });

  test("old engines without tool.ids return Update OfflineGPT guidance", async () => {
    const root = await createRoot();
    ...
    expect(firstFailure(body).recommendedAction).toBe("Update OfflineGPT");
  });

  test("health detects project tool denies while generic MCP add remains best-effort", async () => {
    const root = await createRoot();
    await writeFile(join(root, "opencode.jsonc"), JSON.stringify({ tools: { deny: ["offlinegpt-cloud_*"] } }), "utf8");
    const mock = startMockOpencode();
    const offlinegpt = await startOfflineGpt([workspace("ws_1", root, `http://127.0.0.1:${mock.server.port}`)]);

    const strictBody = await responseRecord(await reconcile(offlinegpt.base));
    ...
    const generic = await fetch(`${offlinegpt.base}/workspace/ws_1/mcp`, {
      method: "POST",
      headers: headers(),
...
async function engineRefresh(base: string, workspaceId = "ws_1", body?: Record<string, unknown>): Promise<Response> {
  return fetch(`${base}/workspace/${workspaceId}/mcp/offlinegpt-cloud/engine-refresh`, {
    method: "POST",
    headers: headers(),
...
describe("offlinegpt-cloud MCP engine refresh", () => {
  test("disconnects the engine client, re-registers, and returns ordered refresh steps with probed health", async () => {
    ...
    const disconnects = mock.requests.filter((request) => request.pathname === "/mcp/offlinegpt-cloud/disconnect");
  ...
  test("reports desired_missing without touching the engine when no config is persisted", async () => {
    ...
    const response = await engineRefresh(offlinegpt.base);
    expect(response.status).toBe(200);
    const body = await responseRecord(response);
    ...
    expect(mock.requests.filter((request) => request.pathname === "/mcp/offlinegpt-cloud/disconnect")).toHaveLength(0);
  });
  ...
  test("rejects malformed JSON on engine refresh instead of silently ignoring it", async () => {
    ...
    const response = await fetch(`${offlinegpt.base}/workspace/ws_1/mcp/offlinegpt-cloud/engine-refresh`, {
      method: "POST",
      headers: headers(),
      body: "not-json",
    });
  ...
  test("richer engine cert/TLS error strings stay classified as connection failures, not token problems", async () => {
    ...
    // "Reconnect OfflineGPT Cloud" cannot repair a broken TLS path.
    expect(failure.code).toBe("opencode_mcp_sync_failed");
    expect(failure.recommendedAction).toBe("Retry reconcile or reconnect OfflineGPT Cloud");
  });

  test("token-expired engine errors keep their token classification", async () => {
    const root = await createRoot();
    const mock = startMockOpencode({ cloudFailedError: "offlinegpt-cloud bearer token expired" });
    const offlinegpt = await startOfflineGpt([workspace("ws_1", root, `http://127.0.0.1:${mock.server.port}`)]);
  ...
  test("keeps step-level failure detail when the engine goes down between seed and refresh", async () => {
    ...
    const response = await engineRefresh(offlinegpt.base, "ws_1", { trigger: "engine-down" });
    expect(response.status).toBe(200);
    const body = await responseRecord(response);
