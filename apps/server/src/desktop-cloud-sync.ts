...
export function readWorkspaceCloudImports(offlinegpt: Record<string, unknown>): WorkspaceCloudImports {
  const cloudImports = isRecord(offlinegpt.cloudImports) ? offlinegpt.cloudImports : {};
  return {
...
export function readDesktopCloudSyncState(offlinegpt: Record<string, unknown>): DesktopCloudSyncState {
  const raw = isRecord(offlinegpt.desktopCloudSync) ? offlinegpt.desktopCloudSync : {};
...
export function syncDesktopCloudResources(input: {
  ...
  const state = readDesktopCloudSyncState(input.offlinegpt);
  const key = contextKey(input.snapshot);
  const previousEntry = state.entries[key] ?? null;
  const changes = diffInstalledCloudResources(readWorkspaceCloudImports(input.offlinegpt), input.snapshot, now);
