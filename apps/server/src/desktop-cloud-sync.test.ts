...
describe("desktop cloud sync", () => {
  test("queues provider update and remove changes", () => {
    ...
    const result = syncDesktopCloudResources({ now: 1780442400000, offlinegpt, snapshot: baseSnapshot });
    ...
    const state = readDesktopCloudSyncState(result.offlinegpt);
  ...
  test("syncs large provider snapshots within an interactive budget", () => {
    ...
    const offlinegpt = { cloudImports: { providers: importedProviders } };

    const start = performance.now();
    const result = syncDesktopCloudResources({ now: 1780442400000, offlinegpt, snapshot });
