...
// (offlinegpt://connect?token=<JWT>), keyed by the token's `kid` header.
//
...
// OfflineGPT Cloud: the vendor's secret store) and never enter this repository.
 ...
 * embedded vendor keys. Dev mode (OFFLINEGPT_DEV_MODE=1) may add ONE ephemeral
...
export function resolveConnectLinkPublicKeys() {
  ...
  if (process.env.OFFLINEGPT_DEV_MODE === "1") {
    const testPem = process.env.OFFLINEGPT_CONNECT_TEST_PUBLIC_KEY_PEM?.trim();
    const testKid = process.env.OFFLINEGPT_CONNECT_TEST_PUBLIC_KEY_KID?.trim();
    if (testPem && testKid) {
