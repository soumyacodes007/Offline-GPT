...
const OFFLINEGPT_CONFIG_FILENAMES = [
  "server.json",
  "runtime.sqlite",
  "runtime.sqlite-wal",
  "runtime.sqlite-shm",
...
const USERDATA_WORKSPACE_FILENAMES = [
  "offlinegpt-workspaces.json",
  "workspace-state.json",
  "offlinegpt-server-tokens.json",
...
function isTruthyDevMode(env) {
  return envValue(env, "OFFLINEGPT_DEV_MODE") === "1";
}
...
function isIsolatedProfile(env) {
  return (
    isTruthyDevMode(env) ||
    envValue(env, "OFFLINEGPT_ELECTRON_APP_IDENTIFIER") !== "" ||
    envValue(env, "OFFLINEGPT_ELECTRON_USERDATA") !== ""
  );
}
...
function resolveNukeEnvironment({ env = {}, homedir, platform, userDataPath }) {
  ...
  const userDataOverride = envValue(env, "OFFLINEGPT_ELECTRON_USERDATA");
  const resolvedUserDataPath = userDataOverride || userDataPath;

  if (isTruthyDevMode(env)) {
    const root = paths.join(resolvedUserDataPath, "offlinegpt-dev-data");
...
function offlinegptServerConfigPath(env, homedir, platform, paths) {
  return resolveOfflineGptServerConfigPath({ env, homeDir: homedir, platform });
}

function envStorePath(env, homedir, platform, paths) {
  return offlinegptEnvStorePath({ env, homeDir: homedir, platform });
}

function tokenStorePath(env, serverConfigPath, homedir, paths) {
  const override = envValue(env, "OFFLINEGPT_TOKEN_STORE");
  if (override) return paths.resolve(override);
  const configDir = serverConfigPath ? paths.dirname(serverConfigPath) : paths.join(homedir, ".config", "offlinegpt");
  return paths.join(configDir, "tokens.json");
}

function runtimeDbPath(env, serverConfigPath, homedir, paths) {
  const override = envValue(env, "OFFLINEGPT_RUNTIME_DB");
  if (override) return paths.resolve(override);
  const configDir = serverConfigPath ? paths.dirname(serverConfigPath) : paths.join(homedir, ".config", "offlinegpt");
...
function orchestratorDataDir(env, homedir, paths) {
  ...
  return paths.join(homedir, ".offlinegpt", LEGACY_ORCHESTRATOR_DIR_NAME);
}

function serverDataDir(env, homedir, paths) {
  const override = envValue(env, "OFFLINEGPT_DATA_DIR");
  if (override) return override;
  return paths.join(homedir, ".offlinegpt", "offlinegpt-server");
}

/** Workspace-local state OfflineGPT owns; the rest of the workspace folder is the user's. */
function workspaceOfflineGptStatePaths(workspacePaths, paths) {
  const output = [];
  for (const workspacePath of workspacePaths) {
    const value = String(workspacePath ?? "").trim();
    if (!value) continue;
    const opencodeDir = paths.join(paths.resolve(value), ".opencode");
    output.push(paths.join(opencodeDir, "offlinegpt"), paths.join(opencodeDir, "offlinegpt.json"));
  }
  return output;
}
...
function addOfflineGptConfigFiles(deletePaths, roots, paths) {
  for (const root of roots) {
    if (!root) continue;
    for (const filename of OFFLINEGPT_CONFIG_FILENAMES) {
...
function resolveNukePlan(input) {
  ...
  const serverConfig = offlinegptServerConfigPath(env, homedir, platform, paths);
  ...
  const deletePaths = [
    ...
    ...workspaceOfflineGptStatePaths(
      Array.isArray(input.workspacePaths) ? input.workspacePaths : [],
      paths,
    ),
  ];

  const offlinegptConfigRoots = [
    paths.join(desktopConfigHome(env, homedir, platform, paths), "offlinegpt"),
    paths.dirname(serverConfig),
    paths.dirname(runtimeDb),
    paths.dirname(tokens),
    paths.dirname(envStore),
  ];
  deletePaths.push(...offlinegptConfigRoots);
  addOfflineGptConfigFiles(deletePaths, offlinegptConfigRoots, paths);
  ...
  const pendingPath = scopeToProfile
    ...
    : paths.join(desktopConfigHome(env, homedir, platform, paths), "offlinegpt", PENDING_NUKE_FILENAME);
...
function nukeWorkerPayloadPath() {
  return path.join(os.tmpdir(), `offlinegpt-nuke-worker-${Date.now()}-${randomBytes(6).toString("hex")}.json`);
}
...
function nukeWorkerSpawnEnv(env = process.env) {
  ...
  delete workerEnv["OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT"];
  return workerEnv;
}
...
async function writeReceipt(receipt) {
  const receiptPath = path.join(os.tmpdir(), `offlinegpt-nuke-receipt-${Date.now()}.json`);
...
async function quiesceForNuke({ runtimeManager, uiControlServer, removeWindowsBrandShortcut }, errors, options = {}) {
  ...
  if (!options.scopeToProfile) {
    await bestEffort(errors, "sandbox-docker-cleanup", () => runtimeManager.sandboxCleanupOfflineGptContainers(), 24_000);
  }
...
/** @returns {Promise<import("@offlinegpt/types/desktop-ipc").NukeReceipt>} */
