...
function legacyOrchestratorPath(home) {
  return path.join(home, ".offlinegpt", LEGACY_ORCHESTRATOR_DIR_NAME);
}
...
async function withTempDir(fn) {
  const root = await mkdtemp(path.join(tmpdir(), "offlinegpt-nuke-test-"));
  try {
    await fn(root);
  } finally {
...
function pendingNukePath(root) {
  return path.join(root, "xdg", "offlinegpt", ".nuke-pending.json");
}
...
function fakeRuntimeManager() {
  return {
    ...
    sandboxCleanupOfflineGptContainers: async () => ({ candidates: [], removed: [], errors: [] }),
  };
}
...
test("buildNukeManifest includes default macOS state roots and preserves bootstrap", () => {
  ...
  assert.equal(manifest.preserveBootstrapPath, "/Users/alice/.config/offlinegpt/desktop-bootstrap.json");
  assert.deepEqual(manifest.partitions, ["default", "persist:offlinegpt-browser"]);
  assert.ok(manifest.deletePaths.includes(userDataPath));
  assert.ok(manifest.deletePaths.includes("/Users/alice/.config/offlinegpt/server.json"));
  assert.ok(manifest.deletePaths.includes("/Users/alice/.config/offlinegpt/runtime.sqlite"));
  assert.ok(manifest.deletePaths.includes("/Users/alice/.config/offlinegpt/runtime.sqlite-wal"));
  assert.ok(manifest.deletePaths.includes("/Users/alice/.config/offlinegpt/runtime.sqlite-shm"));
  assert.ok(manifest.deletePaths.includes("/Users/alice/.config/offlinegpt/runtime-opencode-config.json"));
  assert.ok(manifest.deletePaths.includes("/Users/alice/.config/offlinegpt/engine-instances.json"));
  assert.ok(manifest.deletePaths.includes("/Users/alice/.config/offlinegpt/tokens.json"));
  assert.ok(manifest.deletePaths.includes("/Users/alice/.config/offlinegpt/env.json"));
...
test("buildNukeManifest wipes session state, server audit data, and workspace registries", () => {
  ...
  assert.ok(manifest.deletePaths.includes("/Users/alice/.offlinegpt/offlinegpt-server"));
  assert.ok(manifest.deletePaths.includes(`${userDataPath}/offlinegpt-workspaces.json`));
  assert.ok(manifest.deletePaths.includes(`${userDataPath}/offlinegpt-server-tokens.json`));
  assert.ok(manifest.deletePaths.includes(`${userDataPath}/offlinegpt-server-state.json`));
  assert.ok(manifest.deletePaths.includes("/Users/alice/project/.opencode/offlinegpt"));
  assert.ok(manifest.deletePaths.includes("/Users/alice/project/.opencode/offlinegpt.json"));
  assert.ok(manifest.deletePaths.includes("/Users/alice/other/.opencode/offlinegpt"));
  assert.ok(!manifest.deletePaths.includes("/Users/alice/project/.opencode"));
...
test("buildNukeManifest can include the bootstrap file in the wipe", () => {
  const bootstrapPath = "/Users/alice/.config/offlinegpt/desktop-bootstrap.json";
  const manifest = buildNukeManifest({
    env: {},
    homedir: "/Users/alice",
    platform: "darwin",
...
test("buildNukeManifest includes Windows path shapes", () => {
  ...
  assert.ok(manifest.deletePaths.includes("C:\\Users\\Alice\\AppData\\Roaming\\offlinegpt\\env.json"));
  ...
  assert.ok(manifest.deletePaths.includes("C:\\Users\\Alice\\.config\\offlinegpt\\desktop-bootstrap.json"));
});

test("buildNukeManifest honors OFFLINEGPT_ELECTRON_USERDATA override", () => {
  const manifest = buildNukeManifest({
...
test("buildNukeManifest redirects HOME/XDG paths in dev mode", () => {
  const manifest = buildNukeManifest({
    ...
    homedir: "/Users/alice",
    platform: "darwin",
    userDataPath: "/tmp/offlinegpt-dev-userdata",
  });

  assert.equal(
    manifest.preserveBootstrapPath,
    "/tmp/offlinegpt-dev-userdata/offlinegpt-dev-data/home/.config/offlinegpt/desktop-bootstrap.json",
  );
  assert.ok(manifest.deletePaths.includes("/tmp/offlinegpt-dev-userdata"));
  assert.ok(manifest.deletePaths.includes("/tmp/offlinegpt-dev-userdata/offlinegpt-dev-data/xdg/data/opencode"));
  assert.ok(manifest.deletePaths.includes("/tmp/offlinegpt-dev-userdata/offlinegpt-dev-data/config/opencode"));
  assert.ok(manifest.deletePaths.includes("/tmp/offlinegpt-dev-userdata/offlinegpt-dev-data/xdg/cache/opencode"));
...
test("buildNukeManifest never reaches production state from a non-dev isolated profile", () => {
  ...
  for (const productionPath of [
    "/Users/alice/.config/offlinegpt",
    "/Users/alice/.config/offlinegpt/tokens.json",
    "/Users/alice/.config/offlinegpt/runtime.sqlite",
...
test("buildNukeManifest still wipes a dev profile's own orchestrator data dir", () => {
  ...
  const manifest = buildNukeManifest({
    env: { OFFLINEGPT_DEV_MODE: "1", OFFLINEGPT_DATA_DIR: devOrchestratorPath },
    homedir: "/Users/alice",
    platform: "darwin",
    userDataPath: "/tmp/offlinegpt-dev-userdata",
  });
...
test("buildNukeManifest ignores an inherited XDG_CONFIG_HOME pointing at production config", () => {
  const manifest = buildNukeManifest({
    env: { OFFLINEGPT_DEV_MODE: "1", XDG_CONFIG_HOME: "/Users/alice/.config" },
    homedir: "/Users/alice",
    platform: "darwin",
    userDataPath: "/tmp/offlinegpt-dev-userdata",
  });
...
test("buildNukeManifest excludes paths that would remove ~/.opencode/bin", () => {
  const manifest = buildNukeManifest({
    ...
    homedir: "/Users/alice",
    platform: "darwin",
    userDataPath: "/tmp/offlinegpt-userdata",
  });
...
test("runPendingNukeCleanup retains the choice to remove bootstrap state", async () => {
  await withTempDir(async (root) => {
    ...
    const bootstrapPath = path.join(input.env.XDG_CONFIG_HOME, "offlinegpt", "desktop-bootstrap.json");
...
test("runPendingNukeCleanup refuses replayed paths outside an isolated profile", async () => {
  await withTempDir(async (root) => {
    ...
    const productionPath = path.join(root, "home", ".config", "offlinegpt", "tokens.json");
    const profilePath = path.join(userDataPath, "offlinegpt-server-tokens.json");
...
test("nuke worker payload only serializes safe path inputs", () => {
  const nukeInput = buildNukeWorkerNukeInput({
    env: {
      APPDATA: "C:\\Users\\Alice\\AppData\\Roaming",
      OFFLINEGPT_API_KEY: "secret-api-key",
      OFFLINEGPT_TOKEN: "secret-token",
      OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT: "9888",
      OFFLINEGPT_TOKEN_STORE: "C:\\Users\\Alice\\AppData\\Roaming\\offlinegpt\\tokens.json",
  ...
  const payload = buildNukeWorkerPayload({
    ...
    pendingPath: "/tmp/config/offlinegpt/.nuke-pending.json",
    nowMs: 1_000,
  });
  ...
  assert.equal(payload.nukeInput.env.OFFLINEGPT_TOKEN_STORE, "C:\\Users\\Alice\\AppData\\Roaming\\offlinegpt\\tokens.json");
  ...
  assert.equal(serialized.includes("OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT"), false);
  assert.equal(serialized.includes("OFFLINEGPT_API_KEY"), false);
  assert.equal(serialized.includes("OFFLINEGPT_TOKEN\""), false);
...
test("scheduleNukeCleanupWorker launches Electron as Node with a detached safe payload", async () => {
  await withTempDir(async (root) => {
    const input = pendingNukeInput(root);
    input.env.OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT = "9888";
    const plan = {
    ...
    const result = await scheduleNukeCleanupWorker({
      ...
      env: { SECRET_TOKEN: "do-not-write", XDG_CONFIG_HOME: input.env.XDG_CONFIG_HOME, OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT: "9888" },
    ...
    assert.equal(spawnOptions.env.OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT, undefined);
    assert.equal(payload.nukeInput.env.OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT, undefined);
    assert.deepEqual(payload.appArgv, []);
...
test("nuke cleanup worker waits for parent exit, clears pending path, removes payload, and launches app", async () => {
  await withTempDir(async (root) => {
    ...
    const result = await runNukeCleanupWorker(payloadPath, {
      env: { ELECTRON_RUN_AS_NODE: "1", OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT: "9888" },
    ...
    assert.equal(launchEnv.OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT, undefined);
    assert.equal(await exists(targetPath), false);
    assert.equal(await exists(pendingPath), false);
...
test("executeNukeFreshStart skips host-wide container cleanup on an isolated profile", async () => {
  await withTempDir(async (root) => {
    ...
    runtimeManager.sandboxCleanupOfflineGptContainers = async () => {
      cleanedContainers += 1;
      return { candidates: [], removed: [], errors: [] };
    };
    const input = { ...pendingNukeInput(root), env: { OFFLINEGPT_DEV_MODE: "1" } };
