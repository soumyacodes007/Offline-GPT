...
const ELECTRON_UPDATER_FEEDS = Object.freeze({
  stable: "https://github.com/soumyacodes007/offline-gpt/releases/latest/download",
  alpha: "https://github.com/soumyacodes007/offline-gpt/releases/download/alpha-macos-latest",
});
...
export function targetedStableUpdaterFeed(currentVersion, targetVersion, allowOlder = false) {
  ...
  return `https://github.com/soumyacodes007/offline-gpt/releases/download/v${normalizedTarget}`;
}
...
const SHIP_IT_DEFAULTS_DOMAIN = "com.offlinegptlabs.offlinegpt.ShipIt";
...
export function registerUpdaterIpc({
  ...
  async function resolveRecoveryArtifact(version) {
    ...
    try {
      const manifestUrl = `https://github.com/soumyacodes007/offline-gpt/releases/download/v${version}/${recoveryManifestName(platform, arch, distribution)}`;
