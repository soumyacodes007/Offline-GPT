...
import {
  OFFLINEGPT_DESKTOP_ID,
  buildOfflineGptDesktopEntry,
...
async function createHarness(options = {}) {
  const root = await mkdtemp(path.join(os.tmpdir(), "offlinegpt-appimage-integration-"));
  temporaryRoots.push(root);
  const homeDir = path.join(root, "home");
  const dataHome = path.join(root, "data");
  const configHome = path.join(root, "config");
  const resourcesPath = path.join(root, "resources");
  const appImagePath = options.appImagePath ?? path.join(root, "OfflineGPT AppImage");
...
function relaunchAt(harness, appImagePath, options = {}) {
  let defaultHandler = options.defaultHandler ?? OFFLINEGPT_DESKTOP_ID;
  const dialogs = [];
  const integration = createLinuxDesktopIntegration({
...
describe("Linux AppImage desktop integration", () => {
  ...
  it("installs its launcher, icons, and offlinegpt handler", async () => {
    const harness = await createHarness();
    const result = await harness.integration.install();
    assert.equal(result.ok, true);
    assert.equal(result.status.state, "integrated");
    assert.equal(harness.getDefaultHandler(), OFFLINEGPT_DESKTOP_ID);
    ...
    assert.match(desktopEntry, /^X-OfflineGPT-Managed=true$/m);
  ...
  it("detects a moved AppImage and repairs the launcher for the new path", async () => {
    ...
    const movedPath = path.join(first.root, "Applications", "OfflineGPT.AppImage");
    ...
    const moved = createLinuxDesktopIntegration({
      ...
      runCommand: async (command, args) => {
        if (command === "xdg-mime" && args[0] === "query") {
          return { ok: true, stdout: `${OFFLINEGPT_DESKTOP_ID}\n`, stderr: "" };
        }
        return { ok: true, stdout: "", stderr: "" };
      },
    });
  ...
  it("silently repairs an owned launcher after a self-update lands a new filename", async () => {
    ...
    const updatedPath = path.join(harness.root, "offlinegpt-linux-x86_64-0.18.8.AppImage");
    ...
    assert.match(entry, /^X-OfflineGPT-Version=0\.18\.8$/m);
  });
  ...
  it("keeps a failed silent repair quiet and leaves it to Settings", async () => {
    ...
    const movedPath = path.join(harness.root, "offlinegpt-linux-x86_64-0.18.8.AppImage");
  ...
  it("recognizes an external manager and does not duplicate its integration", async () => {
    const managerDesktopId = "it.mijorus.gearlever.offlinegpt.desktop";
...
`;
  ...
  it("registers an existing manager launcher without creating a duplicate", async () => {
    const managerDesktopId = "appimagelauncher-offlinegpt.desktop";
