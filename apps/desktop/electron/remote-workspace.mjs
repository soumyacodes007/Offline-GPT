...
export function selectOfflineGptWorkspaceForConnection(list, directory) {
...
export function offlinegptWorkspaceDisplayName(workspace) {
  return (
    trim(workspace?.displayName) ||
    trim(workspace?.offlinegptWorkspaceName) ||
    trim(workspace?.name) ||
    trim(workspace?.id) ||
    null
  );
