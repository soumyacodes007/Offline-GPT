...
function cloudThread(): AutomationExecutionThread {
  return { ...desktopThread("offlinegpt-cloud-agent-v1"), executionLocation: "cloud" }
}

describe("Automation execution thread UI", () => {
  test("uses Den's persisted thread identity for receipt navigation", () => {
    expect(automationExecutionThreadRoute(desktopThread("offlinegpt-desktop-runner-v1"))).toBe(
