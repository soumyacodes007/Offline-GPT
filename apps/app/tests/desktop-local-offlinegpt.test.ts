...
import type { OfflineGptServerInfo } from "../src/app/lib/desktop";
import {
  isReadyLocalOfflineGptServerInfo,
  LOCAL_OFFLINEGPT_READINESS_RETRY_DELAY_MS,
  offlinegptServerSettingsChanged,
  shouldAttemptDesktopLocalReconnect,
  waitForReadyLocalOfflineGptServerInfo,
  type DesktopLocalReconnectInput,
} from "../src/react-app/shell/desktop-local-offlinegpt";

describe("offlinegptServerSettingsChanged", () => {
  test("does not refresh the renderer for an unchanged healthy server", () => {
    ...
    expect(offlinegptServerSettingsChanged(settings, { ...settings })).toBe(false);
    expect(offlinegptServerSettingsChanged(settings, { ...settings, token: "new-owner" })).toBe(true);
  });
});

function serverInfo(overrides: Partial<OfflineGptServerInfo> = {}): OfflineGptServerInfo {
  return {
    running: false,
    remoteAccessEnabled: false,
    host: null,
...
describe("isReadyLocalOfflineGptServerInfo", () => {
  test("requires running, a base URL, and at least one token", () => {
    expect(isReadyLocalOfflineGptServerInfo(readyInfo())).toBe(true);
    expect(
      isReadyLocalOfflineGptServerInfo(
    ...
    expect(
      isReadyLocalOfflineGptServerInfo(serverInfo({ running: true, ownerToken: "tok_owner" })),
    ).toBe(false);
    expect(
      isReadyLocalOfflineGptServerInfo(serverInfo({ running: true, baseUrl: "http://127.0.0.1:4187" })),
    ).toBe(false);
    expect(isReadyLocalOfflineGptServerInfo(null)).toBe(false);
  });
});

describe("waitForReadyLocalOfflineGptServerInfo", () => {
  test("keeps polling through a restart gap until the server is ready", async () => {
    const responses: Array<OfflineGptServerInfo | null> = [
    ...
    const result = await waitForReadyLocalOfflineGptServerInfo({
    ...
    expect(isReadyLocalOfflineGptServerInfo(result)).toBe(true);
  });
  ...
  test("is bounded: reports the last unready observation after max attempts", async () => {
    ...
    expect(calls).toBe(4);
    expect(isReadyLocalOfflineGptServerInfo(result)).toBe(false);
