...
describe("normalizeOrganizationServerInput", () => {
  test("normalizes full URLs and bare hostnames to their origin", () => {
    expect(normalizeOrganizationServerInput("https://offlinegpt.acme.com/werpiweur")).toBe(
      "https://offlinegpt.acme.com",
    );
    expect(normalizeOrganizationServerInput("  offlinegpt.acme.com  ")).toBe(
      "https://offlinegpt.acme.com",
    );
    expect(normalizeOrganizationServerInput("http://localhost:3005/dashboard?x=1#y")).toBe(
      "http://localhost:3005",
    );
    expect(normalizeOrganizationServerInput("https://offlinegpt.acme.com:8443/path")).toBe(
      "https://offlinegpt.acme.com:8443",
    );
  });
  ...
  test("allows http only for loopback hosts", () => {
    ...
    expect(normalizeOrganizationServerInput("http://offlinegpt.acme.com")).toBeNull();
    expect(normalizeOrganizationServerInput("http://den.internal:8080")).toBeNull();
  });

  test("keeps https available for non-loopback hosts", () => {
    expect(normalizeOrganizationServerInput("https://offlinegpt.acme.com")).toBe(
      "https://offlinegpt.acme.com",
    );
  ...
  test("rejects unsupported schemes, empty values, and malformed input", () => {
    expect(normalizeOrganizationServerInput("ftp://offlinegpt.acme.com")).toBeNull();
  ...
  test("rejects URL-parser userinfo and backslash oddities", () => {
    expect(normalizeOrganizationServerInput("https://admin@offlinegpt.acme.com")).toBeNull();
    expect(normalizeOrganizationServerInput("https://admin:secret@offlinegpt.acme.com/path")).toBeNull();
    expect(normalizeOrganizationServerInput("https:\\offlinegpt.acme.com\\dashboard")).toBeNull();
    expect(normalizeOrganizationServerInput("offlinegpt.acme.com\\@attacker.example")).toBeNull();
  });
});
