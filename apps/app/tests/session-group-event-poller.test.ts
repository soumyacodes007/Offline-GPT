...
import type { OfflineGptSessionGroupEvent } from "../src/app/lib/offlinegpt-server";
import { SessionGroupEventPoller } from "../src/react-app/shell/session-group-event-poller";

function event(seq: number, workspaceId = "workspace-a"): OfflineGptSessionGroupEvent {
  return {
    id: `event-${seq}`,
    seq,
    workspaceId,
...
describe("session group event poller", () => {
  ...
  test("applies each event once across the exclusive cursor boundary", async () => {
    ...
    const apply = async (items: OfflineGptSessionGroupEvent[]) => {
