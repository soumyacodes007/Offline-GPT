...
import {
  buildOfflineGptContext,
  screenFromRoute,
} from "../src/react-app/shell/offlinegpt-context-projector";
import type { WorkbenchSnapshot } from "../src/react-app/domains/session/chat/workbench-store";

type ContextProjectorInput = Parameters<typeof buildOfflineGptContext>[0];
...
describe("OfflineGPT context projector", () => {
  test("projects the focused split session and its panel state", () => {
    const context = buildOfflineGptContext(baseInput);
  ...
  test("does not leak artifact tabs into a non-panel surface", () => {
    const context = buildOfflineGptContext({
      ...baseInput,
      ui: {
        ...baseInput.ui,
    ...
    panelSessions: {
      "session-a": {
        tabs: [{
          id: "browser-one",
          type: "browser",
          label: "OfflineGPT docs",
          url: "https://docs.offlinegpt.so",
          favicon: null,
          status: "ready",
          canGoBack: false,
          canGoForward: false,
