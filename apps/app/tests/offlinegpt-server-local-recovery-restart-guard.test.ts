    ...
    value: {
      ...
      __OFFLINEGPT_ELECTRON__: {
        invokeDesktop: async (command: string) => {
          ...
          if (command === "offlinegptServerRestart") {
...
function createStore() {
  return createOfflineGptServerStore({
    ...
    selectedWorkspaceDisplay: () => ({
      id: "workspace_local",
      name: "Local",
      path: "/tmp/offlinegpt-restart-guard",
      preset: "starter",
      workspaceType: "local",
    }),
...
describe("local offlinegpt server recovery restart guard", () => {
  test("a fresh store with a healthy live server never restarts it", async () => {
    ...
    const client = await store.ensureLocalOfflineGptServerClient();
