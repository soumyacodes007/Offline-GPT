...
function hasBuiltInBrowserPrompt(promptText) {
  return ["built-in offlinegpt browser", "offlinegpt browser", "example.com"]
...
try {
  tmpdir = await mkdtemp(path.join(os.tmpdir(), "offlinegpt-browser-entry-"));
      ...
      if (req.method === "POST" && url.pathname.endsWith("/chat/completions")) {
        ...
        if (triesBuiltInBrowser) {
          ...
          writeSse(
            res,
            createTextStream(
              "Using the built-in OfflineGPT Browser to open example.com and read the page title.",
            ),
          );
        } else {
  ...
  await step("session.create", async () => {
    const session = await client.session.create({ title: "OfflineGPT browser-entry test" });
  ...
  await step("assert.built-in-browser-quickstart", async () => {
    assert.equal(sawBuiltInBrowserPrompt, true, "Expected browser quickstart prompt to use the built-in OfflineGPT Browser");
