...
import { normalizeOfflineGptServerUrl } from "./offlinegpt-server";
...
function isSupportedDeepLinkProtocol(protocol: string): boolean {
  ...
  return normalized === "offlinegpt:"
    || normalized === "offlinegpt-dev:"
    || normalized === "https:"
    || normalized === "http:";
}
...
export function parseRemoteConnectDeepLink(rawUrl: string): RemoteWorkspaceDefaults | null {
  ...
  const hostUrlRaw = url.searchParams.get("offlinegptHostUrl") ?? url.searchParams.get("offlinegptUrl") ?? "";
  const tokenRaw = url.searchParams.get("offlinegptToken") ?? url.searchParams.get("accessToken") ?? "";
  const normalizedHostUrl = normalizeOfflineGptServerUrl(hostUrlRaw);
  ...
  return {
    offlinegptHostUrl: normalizedHostUrl,
...
export function parseConnectDeepLink(rawUrl: string): ConnectDeepLink | null {
  ...
  const protocol = url.protocol.toLowerCase();
  if (protocol !== "offlinegpt:" && protocol !== "offlinegpt-dev:") {
    return null;
  }
...
function normalizeDebugDeepLinkInput(rawValue: string): string {
  ...
  const directMatch = trimmed.match(/(?:offlinegpt-dev|offlinegpt|https?):\/\/[^\s"'<>]+/i);
