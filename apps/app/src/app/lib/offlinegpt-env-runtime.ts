const PENDING_CHANGES_KEY = "offlinegpt.settings.environment.pendingChanges";
...
export function buildOfflineGptEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "offlinegpt"}::${runtime || "runtime"}`;
}

export function readOfflineGptEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeOfflineGptEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
