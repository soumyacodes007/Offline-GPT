...
// offlinegpt-server, desktop cloud sync). Extracted from den.ts so that modules
...
export type DenOrgExtensionProjection = {
  ...
  name: string;
  description: string | null;
  sourceFormat: OfflineGPTExtensionSourceFormat;
...
export type DenOrgPlugin = {
  ...
  /** Preferred Den surface: plugins are normalized into OfflineGPT extensions. */
