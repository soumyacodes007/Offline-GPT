/**
 * Product analytics for the OfflineGPT desktop app (PostHog, zero-dependency).
 *
 ...
 *   (`window.__offlinegpt.record("analytics.<event>")`) so coded evals can
...
const ENV_POSTHOG_HOST = String(import.meta.env.VITE_OFFLINEGPT_POSTHOG_HOST ?? "").trim();
...
// unless VITE_OFFLINEGPT_POSTHOG_KEY is set. Set it to "" to disable analytics
// in any build. The inspector mirror still records events locally either way.
const POSTHOG_KEY = resolvePosthogKey(import.meta.env.VITE_OFFLINEGPT_POSTHOG_KEY, import.meta.env.DEV);
const POSTHOG_HOST = (ENV_POSTHOG_HOST || DEFAULT_POSTHOG_HOST).replace(/\/+$/, "");

const PREFS_STORAGE_KEY = "offlinegpt.preferences";
const DISTINCT_ID_STORAGE_KEY = "offlinegpt.analytics.distinctId";
