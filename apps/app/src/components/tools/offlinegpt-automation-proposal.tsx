...
import { automationProposalSchema, AUTOMATION_FREE_MODEL, type AutomationProposal } from "@offlinegpt/types/automations"
...
/**
 * Reads an `automation.propose` affordance result out of an offlinegpt_execute
...
export function isAutomationProposalToolPart(part: DynamicToolUIPart): boolean {
  return part.toolName === "offlinegpt_execute"
    && part.state === "output-available"
    && parseAutomationProposal(part.output) !== null
}

export function OfflineGPTAutomationProposalTool({ part }: { part: DynamicToolUIPart }) {
  ...
  const blocker = !automationsEnabled
    ...
    : !signedIn
      ? "Sign in to OfflineGPT Cloud to create this Automation."
      : null
  ...
  return (
    <div
      ...
      data-offlinegpt-automation-proposal
    ...
    >
      <div className="flex items-start gap-3 border-b border-dls-border px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-dls-secondary">
            {created
              ? placement === "cloud"
                ? "It runs on the schedule below in OfflineGPT Cloud."
      ...
      <div className="flex items-center justify-between gap-3 border-t border-dls-border px-4 py-3">
        <p className="min-w-0 flex-1 text-xs text-dls-secondary">
          {blocker ?? (placement === "cloud" ? "Automations run headlessly in OfflineGPT Cloud." : "Automations run on this desktop while it is connected.")}
        </p>
        {created ? (
          <Button
            type="button"
