 ...
 * localStorage.offlinegpt.debug.profilerOverlay = "1" / "0".
 *
 * In prod builds the wrapper is a pass-through (no Profiler overhead) and
 * the overlay renders null.
 *
 * Findings also land on window.__offlinegpt.slice("profiler") so external
 * tools can read them.
 */
...
//
// Explicit opt-ins:
//   - VITE_OFFLINEGPT_PROFILER=1 at `pnpm dev`
//   - window.localStorage.setItem("offlinegpt.debug.profiler", "1")
...
const PROFILER_ENABLED = (() => {
  ...
  try {
    ...
    const flag = env.VITE_OFFLINEGPT_PROFILER;
    if (flag === "1" || flag === "true" || flag === true) return true;
  } catch {
    // ignore
  }
  try {
    if (window.localStorage.getItem("offlinegpt.debug.profiler") === "1") return true;
  } catch {
    // ignore
  }
  return false;
...
// window.__offlinegpt.slice("profiler") — even for operators who aren't
...
function readOverlayStoredPreference(): boolean | null {
  ...
  try {
    const raw = window.localStorage.getItem("offlinegpt.debug.profilerOverlay");
...
function writeOverlayStoredPreference(value: boolean) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem("offlinegpt.debug.profilerOverlay", value ? "1" : "0");
  } catch {
    // ignore
  }
}
