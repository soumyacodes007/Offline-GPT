...
import type {
  OfflineGptConversationLayout,
  OfflineGptContextSnapshot,
  OfflineGptPanelTab,
  OfflineGptResourceDescriptor,
  OfflineGptScreen,
} from "@offlinegpt/types/offlinegpt-context";
...
type OfflineGptContextProjectorInput = {
  route: string;
  revision: number;
  capturedAt: string;
  ...
  availableAffordances: OfflineGptAffordanceDescriptor[];
};
...
export function screenFromRoute(route: string): OfflineGptScreen {
...
function panelTab(tab: PanelTabStore["sessions"][string]["tabs"][number]): OfflineGptPanelTab {
  if (tab.type === "browser") {
    return {
      id: tab.id,
      kind: "browser",
...
export function buildOfflineGptContext(
  input: OfflineGptContextProjectorInput,
): OfflineGptContextSnapshot {
  const primary = input.workbench.primary;
  const secondary = input.workbench.secondary;
  const layout: OfflineGptConversationLayout = primary && secondary
    ? {
        kind: "split",
  ...
  const provider: OfflineGptProviderRef = { id: "offlinegpt-ui", kind: "builtin" };
  const resources: OfflineGptResourceDescriptor[] = [{
    ref: `screen:${input.route}`,
    kind: "screen",
    title: screen.kind === "settings" ? `${screen.panel} settings` : "OfflineGPT",
    provider,
