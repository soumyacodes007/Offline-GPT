...
export function ArchitectureMismatchGate({ children }: ArchitectureMismatchGateProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    const bridge = window.__OFFLINEGPT_ELECTRON__?.system?.getArchitectureInfo;
    if (!bridge) {
  ...
  const openDownload = useCallback(() => {
    ...
    if (!url) return;
    void window.__OFFLINEGPT_ELECTRON__?.shell?.openExternal?.(url);
  }, [info?.downloadUrl, info?.releaseUrl]);

  const openRelease = useCallback(() => {
    if (!info?.releaseUrl) return;
    void window.__OFFLINEGPT_ELECTRON__?.shell?.openExternal?.(info.releaseUrl);
  }, [info?.releaseUrl]);
          ...
          <div className="grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="space-y-8 p-8 sm:p-10 lg:p-12">
              ...
              <div className="space-y-4">
                <h1 className="max-w-2xl text-4xl font-semibold tracking-[-0.04em] text-white sm:text-5xl">
                  Install the correct OfflineGPT build
                </h1>
                <p className="max-w-2xl text-base leading-7 text-white/72 sm:text-lg">
                  Your application is running the {info.appArchLabel} version of OfflineGPT, but this {platformLabel(info.platform)} system is {info.systemArchLabel}. This may cause unpredictable issues.
                </p>
              </div>
            ...
            <aside className="border-t border-white/10 bg-gradient-to-br from-emerald-300/12 via-sky-300/8 to-transparent p-8 sm:p-10 lg:border-l lg:border-t-0 lg:p-12">
              <div className="space-y-5 rounded-[28px] border border-white/10 bg-black/25 p-6 text-sm leading-6 text-white/68">
                <div className="text-lg font-semibold text-white">Why OfflineGPT stopped here</div>
                <p>
                  OfflineGPT blocks startup when the installed app architecture does not match the machine architecture. This prevents runtime sidecars, browser tooling, and update downloads from continuing on the wrong build.
                </p>
                <p>
                  After installing the correct {info.systemArchLabel} build, quit this copy and launch OfflineGPT again. Your workspaces and settings are kept in the same app data folder.
                </p>
