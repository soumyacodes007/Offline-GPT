import { getMcpServerName, isBuiltInOfflineGPTExtension, type McpDirectoryInfo } from "../../../app/constants";
...
const OFFLINEGPT_PROVIDED_SKILL_NAMES = new Set([
  "workspace-guide",
  "skill-creator",
]);

export function isOfflineGptProvidedSkill(skill: Pick<SkillCard, "name" | "path">) {
  const normalizedName = skill.name.trim().toLowerCase();
  const normalizedPath = skill.path.replace(/\\/g, "/").toLowerCase();
  return normalizedPath.includes("/.opencode/skills/") &&
    OFFLINEGPT_PROVIDED_SKILL_NAMES.has(normalizedName);
}
...
export function buildExtensionItems(input: ExtensionItemBuildInput) {
  const builtInItems = input.quickConnect.filter(isBuiltInOfflineGPTExtension).map((entry): ExtensionItem => {
  ...
  const standaloneMcpEntries = input.quickConnect.filter((entry) => {
    if (isBuiltInOfflineGPTExtension(entry)) return false;
