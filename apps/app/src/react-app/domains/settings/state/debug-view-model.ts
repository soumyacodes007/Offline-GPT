...
import {
  ...
  openDesktopUrl,
  offlinegptServerInfo as offlinegptServerInfoCmd,
  offlinegptServerRestart as offlinegptServerRestartCmd,
  pickFile,
...
import {
  ...
  type OfflineGptRuntimeConfigStatus,
...
import type { OfflineGptServerStore, OfflineGptServerStoreSnapshot } from "../../connections/offlinegpt-server-store";

type DebugViewModelProps = Omit<DebugViewProps, "agentContextDiagnostics">;

const STARTUP_PREFERENCE_KEY = "offlinegpt.startupPreference";
const ENGINE_SOURCE_KEY = "offlinegpt.engineSource";
const ENGINE_CUSTOM_BIN_KEY = "offlinegpt.engineCustomBinPath";
const OPENCODE_ENABLE_EXA_KEY = "offlinegpt.opencodeEnableExa";
...
const ONBOARDING_LOCAL_STORAGE_KEYS = [
  "offlinegpt.acknowledgedProviders",
  "offlinegpt.orgOnboardingSeen",
  "offlinegpt.reloadAfterOrgOnboarding",
  "offlinegpt.seenProviderIds",
];

type UseDebugViewModelOptions = {
  developerMode: boolean;
  offlinegptServerStore: OfflineGptServerStore;
  offlinegptServerSnapshot: OfflineGptServerStoreSnapshot;
...
function clearOfflineGptLocalStorageForReset(mode: ResetModalMode): void {
  if (typeof window === "undefined") return;
  try {
    if (mode === "all") {
    ...
    const raw = window.localStorage.getItem("offlinegpt.preferences");
    if (raw) {
      const prefs = JSON.parse(raw);
      prefs.hasCompletedOnboarding = false;
      window.localStorage.setItem("offlinegpt.preferences", JSON.stringify(prefs));
    }
  } catch {
    // ignore persistence failures
  }
...
function formatManagedOpencodeBinary(info: OfflineGptServerInfo | null) {
...
function describeOfflineGptServer(info: OfflineGptServerInfo | null) {
  const running = Boolean(info?.running);
  return {
    ...statusPill(running),
    lines: [
...
export function useDebugViewModel(options: UseDebugViewModelOptions) {
  ...
  const [runtimeConfigStatus, setRuntimeConfigStatus] = useState<OfflineGptRuntimeConfigStatus | null>(null);
  ...
  const [offlinegptServerRestarting, setOfflineGptServerRestarting] = useState(false);
  const [opencodeServiceStatus, setOpencodeServiceStatus] = useState<{
    tone: "success" | "error";
    message: string;
  } | null>(null);
  const [offlinegptServiceStatus, setOfflineGptServiceStatus] = useState<{
    tone: "success" | "error";
    message: string;
  } | null>(null);
  const [opencodeLogStatus, setOpencodeLogStatus] = useState<string | null>(null);
  const [offlinegptLogStatus, setOfflineGptLogStatus] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (!developerMode) return;
    const client = offlinegptServerSnapshot.offlinegptServerClient;
  ...
  }, [developerMode, offlinegptServerSnapshot.offlinegptServerClient, runtimeWorkspaceId]);
  ...
  const runtimeSummary = useMemo(
    () => ({
      ...
      offlinegptServerVersionLabel: offlinegptServerSnapshot.offlinegptServerDiagnostics?.version ?? "—",
    }),
    [
      appBuild?.gitSha,
      appBuild?.version,
      engineInfoState?.baseUrl,
      offlinegptServerSnapshot.offlinegptServerDiagnostics?.version,
    ],
  );
  ...
  const runtimeDebugReport = useMemo(() => {
    return {
      ...
      offlinegptServer: {
        hostInfo: offlinegptServerSnapshot.offlinegptServerHostInfo,
        diagnostics: offlinegptServerSnapshot.offlinegptServerDiagnostics,
        capabilities: offlinegptServerSnapshot.offlinegptServerCapabilities,
        settings: offlinegptServerSnapshot.offlinegptServerSettings,
        status: offlinegptServerSnapshot.offlinegptServerStatus,
        url: offlinegptServerSnapshot.offlinegptServerUrl,
      },
      runtimeWorkspaceId,
      selectedWorkspaceRoot,
    ...
    bootstrapPrepared,
    engineInfoState,
    offlinegptServerSnapshot.offlinegptServerCapabilities,
    offlinegptServerSnapshot.offlinegptServerDiagnostics,
    offlinegptServerSnapshot.offlinegptServerHostInfo,
    offlinegptServerSnapshot.offlinegptServerSettings,
    offlinegptServerSnapshot.offlinegptServerStatus,
    offlinegptServerSnapshot.offlinegptServerUrl,
    runtimeWorkspaceId,
    selectedWorkspaceRoot,
  ]);
  ...
  const offlinegptCard = useMemo(
    () => describeOfflineGptServer(offlinegptServerSnapshot.offlinegptServerHostInfo),
    [offlinegptServerSnapshot.offlinegptServerHostInfo],
  );
  ...
  const onExportRuntimeDebugReport = useCallback(async () => {
    try {
      downloadTextAsFile(
        `offlinegpt-runtime-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
        runtimeDebugReportJson,
        "application/json",
      );
  ...
  const onExportDeveloperLog = useCallback(async () => {
    try {
      downloadTextAsFile(
        `offlinegpt-developer-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
        developerLog.join("\n"),
        "text/plain",
      );
  ...
  const onRevealElectronMigrationBackup = useCallback(async () => {
    ...
    try {
      ...
      if (!appBundlePath) {
        setElectronMigrationStatus("Could not resolve the current OfflineGPT.app bundle path.");
        return;
      }
      await revealDesktopItemInDir(`${appBundlePath}.migrate-bak`);
      setElectronMigrationStatus("Requested Finder reveal for OfflineGPT.app.migrate-bak. The backup exists after an install handoff completes.");
    } catch (error) {
      setElectronMigrationStatus(error instanceof Error ? error.message : safeStringify(error));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const bridge = window.__OFFLINEGPT_ELECTRON__?.updater;
  ...
  const bootFullEngineStack = useCallback(async () => {
    ...
    // Collect ALL local workspace paths so offlinegpt-server is started with
    ...
    const info = await engineStartCmd(workspacePath, {
      ...
      offlinegptRemoteAccess:
        optionsRef.current.offlinegptServerSnapshot.offlinegptServerSettings
          .remoteAccessEnabled === true,
    });

    // engine_start restarts offlinegpt-server on a NEW port and lets that server
    // manage OpenCode. Re-read host info and persist the fresh URL/token.
    try {
      const hostInfo = (await offlinegptServerInfoCmd()) as {
        baseUrl?: string;
        ownerToken?: string;
        clientToken?: string;
        hostToken?: string;
    ...
    return info;
  }, [offlinegptServerStore, refreshEngineInfo]);
  ...
  const onRestartOfflineGptServer = useCallback(async () => {
    if (!isDesktopRuntime()) return;
    setOfflineGptServerRestarting(true);
    setOfflineGptServiceStatus(null);
    setServiceRestartError(null);
    try {
      await offlinegptServerRestartCmd({
        remoteAccessEnabled: offlinegptServerSnapshot.offlinegptServerSettings.remoteAccessEnabled === true,
      });
      setOfflineGptServiceStatus({
        tone: "success",
        message: t("settings.restart_succeeded_template", { service: "OfflineGPT server" }),
      });
      pushDeveloperLog("Restarted offlinegpt-server");
      await offlinegptServerStore.reconnectOfflineGptServer();
    } catch (error) {
      const message = error instanceof Error ? error.message : safeStringify(error);
      setOfflineGptServiceStatus({
        tone: "error",
        message: `${t("settings.restart_failed_template", { service: "OfflineGPT server" })} ${message}`,
      });
      setServiceRestartError(message);
    } finally {
      setOfflineGptServerRestarting(false);
    }
  }, [
    offlinegptServerSnapshot.offlinegptServerSettings.remoteAccessEnabled,
    offlinegptServerStore,
    pushDeveloperLog,
  ]);
  ...
  const onExportOpencodeLogs = useCallback(async () => {
    ...
    try {
      downloadTextAsFile(
        `offlinegpt-opencode-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
        text,
        "text/plain",
      );
  ...
  const onCopyOfflineGptLogs = useCallback(async () => {
    const info = offlinegptServerSnapshot.offlinegptServerHostInfo;
    const text = formatServiceLogs(info?.lastStdout, info?.lastStderr);
    if (!text) {
      setOfflineGptLogStatus(t("settings.no_logs_captured"));
      return;
    }
    try {
      await navigator.clipboard.writeText(text);
      setOfflineGptLogStatus(t("settings.copied_service_logs", { service: "OfflineGPT server" }));
    } catch (error) {
      setOfflineGptLogStatus(error instanceof Error ? error.message : safeStringify(error));
    }
  }, [formatServiceLogs, offlinegptServerSnapshot.offlinegptServerHostInfo]);

  const onExportOfflineGptLogs = useCallback(async () => {
    const info = offlinegptServerSnapshot.offlinegptServerHostInfo;
    const text = formatServiceLogs(info?.lastStdout, info?.lastStderr);
    if (!text) {
      setOfflineGptLogStatus(t("settings.no_logs_captured"));
    ...
    try {
      downloadTextAsFile(
        `offlinegpt-server-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
        text,
        "text/plain",
      );
      setOfflineGptLogStatus(t("settings.exported_developer_log"));
    } catch (error) {
      setOfflineGptLogStatus(error instanceof Error ? error.message : safeStringify(error));
    }
  }, [formatServiceLogs, offlinegptServerSnapshot.offlinegptServerHostInfo]);
  ...
  const onOpenResetModal = useCallback(
    (mode: ResetModalMode) => {
      ...
      const message =
        mode === "all"
          ? "Reset ALL OfflineGPT app data? Open sessions and workspaces will be removed."
      ...
      setResetStatus(null);
      void resetOfflineGptState(mode)
        .then(async () => {
          clearOfflineGptLocalStorageForReset(mode);
          setResetStatus(
            mode === "all"
              ? "Reset OfflineGPT state. Restart the app to see changes."
              : "Reset onboarding state. Restart the app to see changes.",
          );
          pushDeveloperLog(`reset_offlinegpt_state mode=${mode}`);
        })
        .catch((error) => {
  ...
  const onOpenNukeDialog = useCallback(async () => {
    ...
    try {
      const preview = await nukeOfflineGptAndOpencodeConfigPreview({ preserveBootstrap: true });
  ...
  const onSetNukeDeleteBootstrap = useCallback(async (deleteBootstrap: boolean) => {
    ...
    setNukeConfigStatus(null);
    try {
      const preview = await nukeOfflineGptAndOpencodeConfigPreview({ preserveBootstrap: !deleteBootstrap });
      setNukeManifestPreview(preview);
    } catch (error) {
      setNukeDeleteBootstrap(!deleteBootstrap);
      setNukeConfigStatus(error instanceof Error ? error.message : safeStringify(error));
  ...
  const onConfirmNukeOfflineGptAndOpencodeConfig = useCallback(async () => {
    ...
    try {
      ...
      await nukeOfflineGptAndOpencodeConfigAndExit({ preserveBootstrap: !nukeDeleteBootstrap });
    } catch (error) {
      setNukeConfigStatus(error instanceof Error ? error.message : safeStringify(error));
  ...
  const debugProps: DebugViewModelProps = useMemo(
    () => ({
      ...
      startupLabel:
        offlinegptServerSnapshot.offlinegptServerStatus === "connected"
          ? t("settings.offlinegpt_server_label")
      ...
      opencodeRestarting,
      offlinegptServerRestarting,
      opencodeServiceStatus,
      offlinegptServiceStatus,
      opencodeLogStatus,
      offlinegptLogStatus,
      onCopyOpencodeLogs,
      onExportOpencodeLogs,
      onCopyOfflineGptLogs,
      onExportOfflineGptLogs,
      serviceRestartError,
      onRestartOpencode,
      onRestartOfflineGptServer,
      engineCard,
      opencodeConnectCard,
      offlinegptCard,
      offlinegptServerDiagnostics: offlinegptServerSnapshot.offlinegptServerDiagnostics,
      runtimeWorkspaceId,
      offlinegptServerCapabilities: offlinegptServerSnapshot.offlinegptServerCapabilities,
      pendingPermissions: {},
      events: [],
      workspaceDebugEvents: [],
      safeStringify,
      onClearWorkspaceDebugEvents,
      offlinegptAuditEntries: offlinegptServerSnapshot.offlinegptAuditEntries,
      offlinegptAuditStatus: auditStatusPill(offlinegptServerSnapshot.offlinegptAuditStatus),
      offlinegptAuditError: offlinegptServerSnapshot.offlinegptAuditError,
      opencodeConnectStatus: null,
      opencodeDevModeEnabled: appBuild?.offlinegptDevMode === true,
      nukeConfigBusy,
      nukeConfigStatus,
      nukePreviewBusy,
      nukeDialogOpen,
      ...
      onConfirmNukeOfflineGptAndOpencodeConfig,
    }),
    [
      appBuild?.offlinegptDevMode,
      developerLog,
      developerLogStatus,
      developerMode,
      bootstrapConfigDebugJson,
      ...
      startupStatus,
      workspaceDebugEventsStatus,
      offlinegptServerSnapshot.offlinegptAuditEntries,
      offlinegptServerSnapshot.offlinegptAuditError,
      offlinegptServerSnapshot.offlinegptAuditStatus,
      offlinegptServerSnapshot.offlinegptServerCapabilities,
      offlinegptServerSnapshot.offlinegptServerDiagnostics,
      offlinegptServerSnapshot.offlinegptServerStatus,
      resetModalBusy,
