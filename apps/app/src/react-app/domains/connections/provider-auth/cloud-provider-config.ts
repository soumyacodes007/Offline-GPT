...
) => (provider.source === "offlinegpt" ? "offlinegpt" : provider.id.trim());

/**
 * A provider key in `opencode.jsonc` that is owned by the cloud-import system:
 * `lpr_*` keys (org-managed providers) and the `offlinegpt` hosted provider.
...
export const isCloudManagedProviderKey = (providerId: string) =>
  /^lpr_/i.test(providerId) || providerId.trim() === "offlinegpt";
...
export const buildCloudProviderConfig = (
  ...
  // OfflineGPT Models are catalog-backed via OPENCODE_MODELS_URL. Den provisions
  // the provider + key with zero model rows — writing `models: {}` can prevent
  // the engine from keeping catalog models, so omit an empty map for offlinegpt.
