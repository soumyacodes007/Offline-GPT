...
export type OfflineGPTConnectStateStatus = "available" | "missing" | "invalid" | "unreadable";

export type OfflineGPTConnectStateSummary = {
...
export function resolveOfflineGPTConnectStateSummary(
  status: OfflineGPTConnectStateStatus,
  connectEnabled: boolean,
): OfflineGPTConnectStateSummary {
  if (status === "missing") {
    return {
...
export type OfflineGPTConnectStatus = {
  state: "checking" | "ready" | "needs_attention";
  label: "Checking" | "Ready" | "Needs attention";
  description: string;
};

export function offlineGptConnectAttentionTitle(description: string): string {
  return `One possible issue: ${description}`;
}

export function resolveOfflineGPTConnectStatus(
  signedIn: boolean,
  maintenance: SessionCloudMcpMaintenanceState | undefined,
): OfflineGPTConnectStatus | null {
  ...
  if (maintenance.status === "failed" || maintenance.status === "skipped") {
    return {
      ...
      description: maintenance.issue?.message
        ?? "OfflineGPT Connect could not verify connected service tools. Run diagnostics for details.",
    };
  }
