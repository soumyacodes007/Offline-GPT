...
import {
  ...
  type OfflineGptCloudMcpFailure,
...
type CloudMcpMaintenanceClient = CloudMcpClient & Pick<OfflineGptServerClient, "listMcp">;
...
function genericCloudMcpMaintenanceIssue(input?: {
  ...
  return {
    ...
    message: input?.message ?? "OfflineGPT could not verify connected service tools for this workspace.",
  };
}
...
export function getSessionMcpMaintenanceTargetKey(input: {
  client: Pick<OfflineGptServerClient, "baseUrl">;
...
export async function syncCloudControlMcpInBackground(input: {
  ...
  try {
    ...
    if (error instanceof OfflineGptServerError) {
  ...
  if (result.status === "skipped") {
    ...
    if (result.skippedReason === "mint_failed") {
      return failedCloudMcpBackgroundSync({
        ...
        message: "OfflineGPT could not refresh Cloud authentication for connected service tools.",
      });
    }
  }
  return failedCloudMcpBackgroundSync({
