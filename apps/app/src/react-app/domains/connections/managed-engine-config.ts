...
export async function updateManagedDisabledProviders(
  ...
  if (options.offlinegptClient && workspaceId && options.workspaceType === "local") {
    const result = await options.offlinegptClient.setRuntimeDisabledProviders(workspaceId, disabledProviders);
