...
export type CloudMcpClient = {
  ...
  ) => Promise<OfflineGptCloudMcpHealth>;
  reconcileOfflineGptCloudMcp: (
    workspaceId: string,
    payload: OfflineGptCloudMcpReconcilePayload,
  ) => Promise<OfflineGptCloudMcpHealth>;
  refreshOfflineGptCloudMcpEngine?: (
    workspaceId: string,
    payload?: { provider?: string; model?: string; trigger?: string },
  ) => Promise<OfflineGptCloudMcpEngineRefreshResult>;
};
...
type CloudMcpReconcilerInput = {
  ...
  /**
   * Ask the OfflineGPT server to also verify the Cloud endpoint directly
...
const APP_VERSION = String(import.meta.env.VITE_OFFLINEGPT_APP_VERSION ?? "").trim();
const APP_BUILD_SHA = String(import.meta.env.VITE_OFFLINEGPT_BUILD_SHA ?? import.meta.env.VITE_OFFLINEGPT_GIT_SHA ?? "").trim();

function normalizeCode(code: string | null | undefined): string {
  return code?.trim().toLowerCase().replace(/[-.]/g, "_") ?? "";
}

function isProviderProjectionFailure(failure?: OfflineGptCloudMcpFailure | null): boolean {
...
export function buildOfflineGptCloudMcpReconcilePayload(input: {
  context: CloudMcpOperationContext;
  token: DenMcpToken;
}): OfflineGptCloudMcpReconcilePayload | null {
...
export function isCloudMcpAuthTokenFailureCode(code: string | null | undefined): boolean {
  ...
  return normalized === "offlinegpt_cloud_auth_required" ||
    normalized === "offlinegpt_cloud_auth_invalid" ||
    normalized === "offlinegpt_cloud_token_expired" ||
 ...
 * `offlinegpt_cloud_token_expired`). Remint decisions must consider both.
 */
export function isCloudMcpAuthTokenFailure(failure: Pick<OfflineGptCloudMcpFailure, "code" | "aliases"> | null | undefined): boolean {
  if (!failure) return false;
...
async function probeHealth(input: CloudMcpReconcilerInput, scope: CloudMcpScope, options?: { writeFreshnessMarker?: boolean }): Promise<CloudMcpOperationResult> {
  const health = await input.client.getOfflineGptCloudMcpHealth(
    scope.workspaceId,
    input.context.providerModel,
...
async function mintAndPost(input: CloudMcpReconcilerInput, scope: CloudMcpScope): Promise<{ health: OfflineGptCloudMcpHealth | null; token: DenMcpToken | null }> {
  ...
  const payload = buildOfflineGptCloudMcpReconcilePayload({ context: { ...input.context, ...scope }, token });
  if (!payload) return { health: null, token };
  return {
    health: await input.client.reconcileOfflineGptCloudMcp(scope.workspaceId, payload),
    token,
  };
}
...
async function repairCloudMcp(input: CloudMcpReconcilerInput, scope: CloudMcpScope): Promise<CloudMcpOperationResult> {
  ...
  if (!input.force && marker && isCloudMcpSyncMarkerFresh({
    ...
    const health = await input.client.getOfflineGptCloudMcpHealth(scope.workspaceId, input.context.providerModel);
...
export async function runOfflineGptCloudMcpReconciler(input: CloudMcpReconcilerInput): Promise<CloudMcpOperationResult> {
...
/**
 * Force the engine to drop its offlinegpt-cloud MCP client and reconnect.
 ...
 */
export async function runOfflineGptCloudMcpEngineRefresh(input: {
  client: CloudMcpClient;
  context: CloudMcpOperationContext;
  ...
  const refreshEngine = input.client.refreshOfflineGptCloudMcpEngine;
  if (!refreshEngine) {
...
export function cloudMcpFailureStageLabel(input: {
  ...
  if (code.includes("tool_ids") || code.includes("client_registration")) return "OfflineGPT components need updating";
...
export function cloudMcpRecommendedAction(input: {
  ...
  if (!input.signedIn) return "Sign in to OfflineGPT Cloud.";
  ...
  if (code.includes("scope")) return "Reconnect OfflineGPT Cloud with the required permissions.";
  ...
  if (isProviderProjectionFailure(input.health?.firstFailure)) return "Choose a model that can use OfflineGPT Cloud tools.";
  if (code.includes("tool_ids") || code.includes("client_registration")) return "Update OfflineGPT, then retry.";
  if (code === "extensions_plugin_missing") return "Reload the agent so OfflineGPT instructions are current.";
  if (code === "cloud_tools_missing") return "Reconnect OfflineGPT Cloud so the endpoint exposes search_capabilities and execute_capability.";
...
export async function cleanupOfflineGptCloudMcpAfterSignOut(input: {
  context: CloudMcpScope;
  offlinegptClient: CleanupClient | null;
  ...
  await Promise.all([
    input.offlinegptClient && scope
      ? input.offlinegptClient.removeMcp(scope.workspaceId, CLOUD_MCP_SERVER_NAME).catch(() => null)
