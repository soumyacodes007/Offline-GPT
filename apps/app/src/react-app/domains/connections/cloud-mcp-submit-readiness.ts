...
export type CloudMcpSubmissionReadinessAssessment =
  | { ready: true; health: OfflineGptCloudMcpHealth }
  | { ready: false; health: OfflineGptCloudMcpHealth | null; issue: CloudMcpSubmissionIssue };

export type CloudMcpSubmissionReadinessResult =
  | { outcome: "ready"; health: OfflineGptCloudMcpHealth; attempts: number }
  | { outcome: "bypass"; health: OfflineGptCloudMcpHealth; attempts: number; reason: "disabled" }
  | { outcome: "failed"; health: OfflineGptCloudMcpHealth | null; issue: CloudMcpSubmissionIssue; attempts: number };
...
function genericSubmissionIssue(input?: {
  ...
  return {
    ...
    message: input?.message ?? "OfflineGPT could not verify connected service tools for the selected model.",
  };
}

function failureIssue(health: OfflineGptCloudMcpHealth): CloudMcpSubmissionIssue {
...
function healthShowsExplicitDisable(health: OfflineGptCloudMcpHealth): boolean {
...
function authResolutionIssue(input?: { timedOut?: boolean }): CloudMcpSubmissionIssue {
  return genericSubmissionIssue({
    ...
    message: input?.timedOut
      ? "OfflineGPT timed out while restoring connected service access."
      : "OfflineGPT could not finish restoring connected service access.",
  ...
  if (
    ...
    return {
      ...
      issue: genericSubmissionIssue({
        ...
        message: "OfflineGPT Cloud did not prove that search_capabilities and execute_capability are available.",
      }),
    };
  }
  ...
  if (!projection.checked) {
    return {
      ...
      issue: genericSubmissionIssue({
        ...
        stage: "provider_projection",
        message: "OfflineGPT could not read tool capability information for the selected provider and model.",
      ...
      issue: genericSubmissionIssue({
        ...
        message: modelMissing
          ...
          : toolCallingUnavailable
            ...
            : "OfflineGPT could not confirm that the selected model supports tool calling.",
  ...
  if (projection.source !== "experimental_tool") {
    return {
      ...
      issue: genericSubmissionIssue({
        ...
        stage: "provider_projection",
        retryable: false,
        message: "OfflineGPT received an unsupported tool capability result for the selected provider and model.",
...
function timeoutIssue(): CloudMcpSubmissionIssue {
  return genericSubmissionIssue({
    ...
    message: "OfflineGPT timed out while preparing connected service tools.",
  });
}
  ...
  return {
    ...
    issue: timedOut
      ...
      : genericSubmissionIssue({
          ...
          message: "OfflineGPT could not check connected service tools before sending.",
        }),
  };
}
