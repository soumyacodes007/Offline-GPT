...
import {
  ...
  type OfflineGPTWebAccessCheck,
  type OfflineGPTWebAccessGateState,
} from "./offlinegpt-web-access-state";
...
export function OfflineGPTWebAccessGateScreen(props: {
  state: Exclude<OfflineGPTWebAccessGateState, "inactive" | "granted">;
  ...
  return (
    <main
      ...
      data-testid="offlinegpt-web-access-gate"
            ...
            <h1 className="mt-2 text-[26px] font-semibold leading-tight tracking-[-0.03em]">
              {checking
                ...
                : denied
                  ...
                  : "OfflineGPT Web remains locked"}
            </h1>
            <p className="mt-3 text-sm leading-6 text-muted-foreground">
              {checking
                ? `Waiting for Den to confirm access for ${organizationName}.`
                : denied
                  ? `${organizationName} does not have an active OfflineGPT Web subscription or complimentary admin grant.`
                  : `Den could not confirm OfflineGPT Web access for ${organizationName}. The workspace stays locked until it can.`}
            </p>
          </div>
        </div>
...
export function OfflineGPTWebAccessGate({ children }: { children: ReactNode }) {
  ...
  const expectedScope =
    ...
    && verifiedOrganizationId === organizationId
      ...
      : null;
  const [check, setCheck] = useState<OfflineGPTWebAccessCheck | null>(null);
  ...
  useEffect(() => {
    ...
    void client.getOfflineGPTWebAccess(organizationId).then(
      (access) => {
        if (cancelled) return;
        setCheck({
          scope: expectedScope,
  ...
  const state = resolveOfflineGPTWebAccessGateState({
    gatewayMode,
