...
type RemoteWorkspaceFormState = {
  ...
  offlinegptTokenVisible: boolean;
  directory: string;
  displayName: string;
};
...
export function CreateRemoteWorkspaceModal(
  ...
  const { offlinegptHostUrl, offlinegptToken, offlinegptTokenVisible, directory, displayName } = form;
  ...
  const canSubmit = useMemo(() => {
    ...
    return offlinegptHostUrl.trim().length > 0;
  }, [offlinegptHostUrl, submitting]);
  ...
  useEffect(() => {
    ...
    setForm({
      offlinegptHostUrl: defaults.offlinegptHostUrl?.trim() ?? "",
      offlinegptToken: defaults.offlinegptToken?.trim() ?? "",
      offlinegptTokenVisible: false,
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <RemoteWorkspaceFields
            hostUrl={offlinegptHostUrl}
            onHostUrlInput={(value) => setForm((current) => ({ ...current, offlinegptHostUrl: value }))}
            token={offlinegptToken}
            tokenVisible={offlinegptTokenVisible}
            onTokenInput={(value) => setForm((current) => ({ ...current, offlinegptToken: value }))}
            onToggleTokenVisible={() =>
              setForm((current) => ({ ...current, offlinegptTokenVisible: !current.offlinegptTokenVisible }))
            }
            ...
            description="Use the URL your OfflineGPT server shared with you. Add a token only if the server needs one."
          />
        </div>
        ...
        <DialogFooter className="shrink-0 flex-col gap-3">
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button
              ...
              onClick={() =>
                props.onConfirm({
                  offlinegptHostUrl: offlinegptHostUrl.trim(),
                  offlinegptToken: offlinegptToken.trim(),
