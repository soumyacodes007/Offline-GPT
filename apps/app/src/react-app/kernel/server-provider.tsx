...
import { normalizeOfflineGptServerUrl } from "../../app/lib/offlinegpt-server";
...
export function normalizeServerUrl(input: string): string | undefined {
  return normalizeOfflineGptServerUrl(input) ?? undefined;
}
...
function readStoredList(): string[] {
  ...
  try {
    const raw = window.localStorage.getItem("offlinegpt.server.list");
...
function readStoredActive(): string {
  if (typeof window === "undefined") return "";
  try {
    const stored = window.localStorage.getItem("offlinegpt.server.active");
...
function readOfflineGptToken(): string {
  if (getOfflineGptGatewayOrigin()) return readOfflineGptGatewayDenToken();

  if (typeof window === "undefined") return "";
  try {
    return (window.localStorage.getItem("offlinegpt.server.token") ?? "").trim();
  } catch {
    return "";
  }
}

export function buildOfflineGptHealthHeaders(url: string): Record<string, string> | undefined {
  const token = readOfflineGptToken();
  return token && url.includes("/opencode") ? { Authorization: `Bearer ${token}` } : undefined;
}

async function checkHealth(url: string): Promise<boolean> {
  if (!url) return false;
  const headers = buildOfflineGptHealthHeaders(url);
...
export function ServerProvider({ children, defaultUrl }: ServerProviderProps) {
  ...
  useEffect(() => {
    ...
    // Hosted web deployments served by OfflineGPT must reuse the OpenCode proxy
    ...
    const forceProxy =
      ...
      (!isDesktopRuntime() &&
        ...
        (import.meta.env.PROD ||
          (typeof import.meta.env?.VITE_OFFLINEGPT_URL === "string" &&
            import.meta.env.VITE_OFFLINEGPT_URL.trim().length > 0)));
  ...
  useEffect(() => {
    ...
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("offlinegpt.server.list", JSON.stringify(list));
      window.localStorage.setItem("offlinegpt.server.active", active);
    } catch {
      // ignore
    }
  }, [active, list]);
  ...
  useEffect(() => {
    ...
    if (isDesktopRuntime() && !active.includes("/opencode")) {
      // Desktop React routes now talk to OfflineGPT server workspace-mounted
