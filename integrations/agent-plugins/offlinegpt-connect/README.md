# OfflineGPT Connect Agent Plugin

This directory is the portable OfflineGPT Connect package for the published
...
- the remote OfflineGPT MCP endpoint;
...
The MCP client discovers OfflineGPT OAuth from the endpoint and opens the normal
browser sign-in flow. Access remains scoped to the selected organization and
the signed-in member's grants.

The `streamable-http` entry does not pin an MCP wire version. OfflineGPT Connect
