 ...
 * layout, checkpoint hydrate/flush, and the supervised `offlinegpt-server`
...
export type OfflineGPTCheckpointConfig = {
...
export type OfflineGPTBootstrapConfig = OfflineGPTCheckpointConfig & {
...
export function checkpointDirectory(config: Pick<OfflineGPTCheckpointConfig, "dataMountPath">) {
  return `${config.dataMountPath}/checkpoints`
}

export function checkpointRestoreMarkerPath(config: Pick<OfflineGPTCheckpointConfig, "runtimeDataPath">) {
  return `${config.runtimeDataPath}/.offlinegpt-restore-marker`
}

function checkpointStateManifest(config: OfflineGPTCheckpointConfig) {
  return `${config.runtimeDataPath} ${config.runtimeWorkspacePath}`
}

function checkpointLastFlushMarkerPath(config: OfflineGPTCheckpointConfig) {
  return `${config.sidecarDir}/checkpoint.last-flush`
}

function checkpointEnvironmentScript(config: OfflineGPTCheckpointConfig) {
  ...
  return `ENGINE_STATE_PATH=\${OFFLINEGPT_ENGINE_STATE_PATH:-\$HOME/.local/share/opencode}
OFFLINEGPT_STATE_MANIFEST="${checkpointStateManifest(config)} \$ENGINE_STATE_PATH"
...
function checkpointFlushFunctions(config: OfflineGPTCheckpointConfig, input: { failOnError: boolean }) {
  const failureReturn = input.failOnError ? "1" : "0"
  return `checkpoint_changed() {
  if [ ! -e "$LAST_FLUSH_MARKER" ]; then
    return 0
  fi
  for state_path in $OFFLINEGPT_STATE_MANIFEST; do
...
export function renderCheckpointFlushCommand(config: OfflineGPTCheckpointConfig) {
  return `set -u
...
export function renderCheckpointExistsCommand(config: Pick<OfflineGPTCheckpointConfig, "dataMountPath">) {
...
export function renderRestoreMarkerExistsCommand(config: Pick<OfflineGPTCheckpointConfig, "runtimeDataPath">) {
  return `test -s ${shellQuote(checkpointRestoreMarkerPath(config))}`
}

/** The supervised OfflineGPT server process with checkpoint hydrate and flush, as a POSIX sh script. */
export function renderOfflineGPTBootstrapScript(config: OfflineGPTBootstrapConfig) {
  const verifyRuntimeStep = [
    `if ! command -v offlinegpt-server >/dev/null 2>&1; then echo 'offlinegpt-server binary missing from ${config.imageDescription}; ${config.rebuildHint}' >&2; exit 1; fi`,
  ...
  const offlinegptServe = [
    ...
    " OFFLINEGPT_MANAGE_OPENCODE=",
    shellQuote("1"),
    " OFFLINEGPT_OPENCODE_BIN=",
    shellQuote("/usr/local/bin/opencode"),
    " OFFLINEGPT_WEB_ROOT=",
    shellQuote("/opt/offlinegpt/web"),
    ...
    " OFFLINEGPT_WEB_BOOTSTRAP_TOKEN=",
    shellQuote("0"),
    " OFFLINEGPT_EXTENSIONS_PLUGIN_DIR=",
    shellQuote("/opt/offlinegpt/opencode-plugins"),
...
hydrate_checkpoint() {
  ...
  if ! state_dirs_pristine; then
    echo "checkpoint hydrate skipped; local OfflineGPT state is not pristine or was already restored"
    return 0
  fi
  ...
  echo "checkpoint hydrate failed for $latest_checkpoint; continuing with fresh OfflineGPT state" >&2
...
checkpoint_pid=""
on_term() {
  echo "termination requested; flushing OfflineGPT checkpoint"
  flush_checkpoint
...
while [ "$attempt" -lt 3 ]; do
  ...
  echo "offlinegpt-server failed (attempt $attempt, exit $status); ${config.rebuildHint} if this persists; retrying in 3s"
  sleep 3
done
flush_checkpoint
exit 1
...
/** `renderOfflineGPTBootstrapScript` wrapped for a provider that execs one command line. */
export function renderOfflineGPTBootstrapCommand(config: OfflineGPTBootstrapConfig) {
  return `sh -lc ${shellQuote(renderOfflineGPTBootstrapScript(config))}`
}
