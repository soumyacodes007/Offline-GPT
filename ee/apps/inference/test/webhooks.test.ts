...
function webhookRequest(body: unknown) {
  return new Request("http://offlinegpt.test/webhooks/openrouter", {
    method: "POST",
    headers: {
...
function createWebhookTestServer() {
  ...
  function usagePayload(input: { requestId: string; eventId: string; generationId: string; requestModel: string; responseModel: string; includeSensitive?: boolean }) {
    const attributes = [
      ...
      attribute("trace.offlinegpt_request_id", input.requestId),
...
test("retains unknown model usage without deduction and reports bounded provider facts", async () => {
  ...
  assert.equal(report.offlinegptRequestId, "request-unknown")
...
test("deducts usage without Sentry diagnostics when OpenRouter usage reports a known model", async () => {
  ...
  assert.deepEqual(insertedEntries, [{
    offlinegptRequestId: "request-known",
