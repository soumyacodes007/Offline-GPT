  ...
  test("builds install URLs and validates signed state tokens", async () => {
    const app = await getGithubAppSummary({
      ...
      fetchFn: async () => new Response(JSON.stringify({
        html_url: "https://github.com/apps/offlinegpt-test",
        name: "OfflineGPT Test",
        slug: "offlinegpt-test",
      }), { status: 200 }),
    })
    ...
    expect(buildGithubAppInstallUrl({ app, state: token })).toBe(`https://github.com/apps/offlinegpt-test/installations/new?state=${encodeURIComponent(token)}`)
