...
test("default marketplace logos pass logo url validation", () => {
  expect(schemas.marketplaceLogoUrlSchema.safeParse(defaults.DEFAULT_OFFLINEGPT_MARKETPLACE_LOGO_URL).success).toBe(true)
...
test("marketplace logo url accepts https and root-relative paths only", () => {
  ...
  expect(schemas.marketplaceLogoUrlSchema.safeParse("/offlinegpt-mark.svg").success).toBe(true)
