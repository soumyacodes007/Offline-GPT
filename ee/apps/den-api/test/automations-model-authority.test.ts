...
const offlineGptProvider: AutomationAuthorityProvider = {
...
function authorityStore(overrides: Partial<AutomationModelAuthorityStore> = {}): AutomationModelAuthorityStore {
  return {
    ...
    async findOfflineGPTProvider() { return offlineGptProvider },
...
describe("Automation normalized model authority", () => {
  ...
  test("resolves enabled OfflineGPT aliases through the owner's managed provider", async () => {
    const result = await resolveAutomationModelAccessWithStore({
      ...base,
    ...
    expect(result).toMatchObject({
      ...
      value: {
        ...
        providerRecordId: offlineGptProvider.id,
        providerId: "offlinegpt",
        modelId: "z-ai/glm-5.2",
      },
    })
