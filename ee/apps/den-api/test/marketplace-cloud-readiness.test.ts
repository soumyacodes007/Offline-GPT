...
import { and, eq, inArray, sql } from "@offlinegpt-ee/den-db/drizzle"
import {
  AuthUserTable,
...
function seedRequiredEnv() {
  process.env.DATABASE_URL = process.env.DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/offlinegpt_test_pr6"
...
describe("marketplace cloud readiness payload", () => {
  ...
  test("connector mapping delete removes only exact mapping-owned bindings after pluginId reassignment", async () => {
    ...
    await db.insert(ConnectorTargetTable).values({
      ...
      targetConfigJson: { branch: "main", repositoryFullName: "offlinegpt/test", installationId: 1 },
      createdAt: now,
      updatedAt: now,
    })
