 ...
 *
 * Usage:
 *   pnpm --filter @offlinegpt-ee/den-api migrate:skills-to-plugins        # dry run
 *   pnpm --filter @offlinegpt-ee/den-api migrate:skills-to-plugins -- --yes
 *
 ...
 *        pnpm --filter @offlinegpt-ee/den-api migrate:skills-to-plugins -- --table-prefix recovered_ --yes
 *   4. Drop the recovered_* tables after verifying the migrated plugins.
 */
import { createDenDb, type DenDbMode } from "@offlinegpt-ee/den-db"
import { and, asc, eq, isNull, sql } from "@offlinegpt-ee/den-db/drizzle"
import {
...
async function ensureDefaultOfflineGPTMarketplace(input: {
  ...
  const existing = (await input.database
    ...
    .where(and(
      ...
      eq(MarketplaceTable.name, DEFAULT_OFFLINEGPT_MARKETPLACE_NAME),
...
async function attachPluginToDefaultMarketplace(input: {
  ...
  const marketplaceId = await ensureDefaultOfflineGPTMarketplace(input)
  const existing = (await input.database
    .select()
...
async function main() {
  ...
  const pointerVersions = await db
    ...
    .from(ConfigObjectVersionTable)
    .where(eq(ConfigObjectVersionTable.schemaVersion, "offlinegpt.den_skill.v1"))
