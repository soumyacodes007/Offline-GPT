/**
 * Organization entitlement for OfflineGPT VM execution.
 *
 ...
 * (apps/app/src/react-app/domains/cloud/offlinegpt-web-access-gate.tsx), which
 ...
 * den-gateway) has returned `403 offlinegpt_web_access_required` for an
 ...
 *
 * The den-side `403 offlinegpt_web_access_required` responses that use this
...
} from "./offlinegpt-web-access-error.js"

export type OfflineGPTWebRuntimeAccess = {
  hasAccess: boolean
}

export type OfflineGPTWebRuntimeAccessResolver = (
  organizationId: string,
) => Promise<OfflineGPTWebRuntimeAccess>

export const getOfflineGPTWebRuntimeAccess: OfflineGPTWebRuntimeAccessResolver = async (organizationId) => {
  const access = await getOfflineGPTWebAccess(
    normalizeDenTypeId("organization", organizationId),
  )
  return { hasAccess: access.hasAccess }
}

export async function requireOfflineGPTWebRuntimeAccess(
  organizationId: string,
  resolveAccess: OfflineGPTWebRuntimeAccessResolver = getOfflineGPTWebRuntimeAccess,
) {
  const access = await resolveAccess(organizationId)
  if (!access.hasAccess) {
    throw new OfflineGPTWebAccessRequiredError()
  }
  return access
}
