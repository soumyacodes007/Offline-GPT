...
import { OAuthAccessTokenTable } from "@offlinegpt-ee/den-db/schema"
    ...
    try {
      ...
      if (error instanceof McpSigningKeysUnavailable) {
        return mcpJsonResponse(503, {
          ...
          message: "OfflineGPT could not verify the token signing key. Retry shortly.",
          referenceId,
  ...
  if (grantId) {
    ...
    if (grantLiveness === "check_failed") {
      return mcpJsonResponse(503, {
        ...
        message: "OfflineGPT could not verify the token grant. Retry shortly.",
        referenceId,
      }, undefined, { "retry-after": "10" })
    }
    ...
    if (sessionLiveness === "check_failed") {
      return mcpJsonResponse(503, {
        ...
        message: "OfflineGPT could not verify the token session. Retry shortly.",
        referenceId,
      }, undefined, { "retry-after": "10" })
    }
