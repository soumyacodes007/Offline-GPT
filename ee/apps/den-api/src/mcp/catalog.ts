...
// `<serverName>_<name>` (e.g. `offlinegpt-cloud_` adds 15 chars), so the raw
...
export const MAX_CLIENT_PREFIX = "offlinegpt-cloud_".length // 15
...
export function buildMcpCatalog(document: OpenApiDocument): McpToolOperation[] {
  ...
  for (const [path, pathItem] of Object.entries(document.paths ?? {})) {
    for (const [method, operation] of Object.entries(pathItem)) {
      ...
      // prefix budget (e.g. `offlinegpt-cloud_` + name > 64) is rejected by AWS
