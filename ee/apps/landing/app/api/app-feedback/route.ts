...
import { EmailSendError, sendEmail, type FeedbackEmailProps } from "@offlinegpt/email";
...
const DEFAULT_INTERNAL_FEEDBACK_EMAIL = "team@offlinegptlabs.com";
...
function sanitizeContext(input: FeedbackContext | undefined) {
  return {
    ...
    offlinegptServerVersion: sanitizeValue(input?.offlinegptServerVersion),
...
function formatDiagnosticsSummary(context: ReturnType<typeof sanitizeContext>) {
  ...
  const lines = [
    ...
    ["OfflineGPT server", context.offlinegptServerVersion],
...
export async function POST(request: Request) {
  ...
  const internalEmail =
    process.env.OFFLINEGPT_FEEDBACK_EMAIL?.trim() ||
