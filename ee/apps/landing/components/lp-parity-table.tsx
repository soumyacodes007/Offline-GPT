...
type OfflineGPTSupport = "check" | "soon";

type ParityRow = {
  capability: string;
  cowork: CoworkSupport;
  offlinegpt?: OfflineGPTSupport;
  badge?: "alpha" | "offlinegpt";
  highlighted?: boolean;
};
...
function OfflineGPTCell({ support }: { support?: OfflineGPTSupport }) {
  if (support === "soon") {
    return (
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 rounded-[12px] bg-[var(--lp-tonal)] p-3">
              <div>
                <div className="mb-2 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--lp-muted)]">
                  <OfflineGPTMark className="h-3.5 w-3.5 object-contain" />
                  OfflineGPT
                </div>
                <OfflineGPTCell support={row.offlinegpt} />
              </div>
              <div>
      ...
      <div className="hidden md:block">
        <div className="flex items-center border-b border-[var(--lp-border)] px-5 py-3.5">
          ...
          <div className="flex w-40 items-center justify-center gap-2 text-[13px] font-semibold text-[var(--lp-ink)]">
            <OfflineGPTMark className="h-5 w-5 object-contain" />
            OfflineGPT
          </div>
