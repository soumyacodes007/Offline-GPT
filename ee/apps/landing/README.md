...
1. Install deps from repo root:
   `pnpm install`
2. Run the app:
   `pnpm --filter @offlinegpt-ee/landing dev`

### Optional env vars

- `NEXT_PUBLIC_CAL_URL` - enterprise booking link
- `EMAIL_FROM` - sender for feedback emails (for example `OfflineGPT <team@offlinegptlabs.com>`)
...
- `OFFLINEGPT_FEEDBACK_EMAIL` - optional override for the internal feedback recipient (defaults to `team@offlinegptlabs.com`)
- `LOOPS_API_KEY` - Loops API key for enterprise contact submissions
- `LOOPS_INTERNAL_FEEDBACK_EMAIL` - legacy feedback recipient override, used only when `OFFLINEGPT_FEEDBACK_EMAIL` is not set
...
2. Build command: `pnpm --filter @offlinegpt-ee/landing build`
3. Output: `.next`
4. Start command: `pnpm --filter @offlinegpt-ee/landing start`
5. Enable Vercel BotID for the project so protected form routes can reject automated submissions.

### Self-hosted

1. Build: `pnpm --filter @offlinegpt-ee/landing build`
2. Start: `pnpm --filter @offlinegpt-ee/landing start`
