...
export const getGithubData = async () => {
  ...
  const isElectronDesktopAsset = (name: string) =>
    name.startsWith("offlinegpt-mac-") ||
    name.startsWith("offlinegpt-win-") ||
    name.startsWith("offlinegpt-linux-");
  ...
  const hasWindowsDesktopAsset = (release: Release) => {
    ...
    return assets.some((asset) => {
      const name = String(asset?.name || "").toLowerCase();
      return name.startsWith("offlinegpt-win-x64-") && name.endsWith(".exe");
    });
  };
  ...
  const exe = selectAsset(windowsAssets, [".exe"], ["offlinegpt-win-"]);
