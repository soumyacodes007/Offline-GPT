...
declare global {
  var __offlinegptConnectDebugProxyFlakyWindows: Map<string, FlakyWindow> | undefined
}

const flakyWindows = globalThis.__offlinegptConnectDebugProxyFlakyWindows ??= new Map()
...
export function connectDebugProxyFault(input: {
  ...
  if (input.scenario.fault === "auth-expired") {
    return {
      kind: "response",
      label: "auth-expired",
      response: jsonError(401, "invalid_token", "The debug scenario expired the OfflineGPT Connect token.", {
        "www-authenticate": "Bearer error=\"invalid_token\", error_description=\"OfflineGPT Connect debug token expired\"",
      }),
    }
  }
