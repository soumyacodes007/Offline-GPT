...
describe("Den flow stale session cleanup", () => {
  test("clears Better Auth session cookies after /v1/me returns unauthorized", async () => {
    setDenApiOriginOverride("https://api.offlinegptlabs.com");
    globalThis.window = {
      location: { origin: "https://app.offlinegptlabs.com" },
      localStorage: { getItem: () => null },
    };
    const calls = [];
    globalThis.fetch = async (input, init) => {
      calls.push({ input: String(input), credentials: init?.credentials, method: init?.method });
      if (String(input) === "https://api.offlinegptlabs.com/v1/me") {
    ...
    expect(calls).toEqual([
      { input: "https://api.offlinegptlabs.com/v1/me", credentials: "include", method: "GET" },
