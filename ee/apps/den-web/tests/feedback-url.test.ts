...
import { buildDenFeedbackUrl, OFFLINEGPT_FEEDBACK_URL } from "../app/(den)/_lib/feedback";
...
describe("Den feedback links", () => {
  test("point to the public landing feedback form with dashboard context", () => {
    ...
    expect(url.startsWith(`${OFFLINEGPT_FEEDBACK_URL}?`)).toBe(true);
    expect(url).toContain("source=offlinegpt-web-app");
