...
describe("OfflineGPT Web billing data", () => {
  ...
  test("billing presents plan, quantity definition, unit-total math, lifecycle, and management", () => {
    ...
    expect(OFFLINEGPT_WEB_QUANTITY_EXPLANATION).toContain("Pending invitations are not billed");
    expect(source).toContain('data-testid="billing-offlinegpt-web-card"');
    expect(source).toContain('label="Plan" value="OfflineGPT Web"');
    ...
    expect(source).toContain('data-testid="billing-offlinegpt-web-lifecycle"');
  ...
  test("platform admin UI exposes an audited complimentary access action separate from capabilities", () => {
    ...
    expect(source).toContain('data-testid="admin-offlinegpt-web-access"');
    expect(source).toContain("OfflineGPT Web billing access");
    ...
    expect(source).toContain("/offlinegpt-web-access");
  });
});
