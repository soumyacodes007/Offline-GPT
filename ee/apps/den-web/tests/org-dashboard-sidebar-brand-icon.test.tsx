...
describe("Den dashboard sidebar brand icon", () => {
  ...
  test("renders the canonical managed square icon without first rendering the OfflineGPT mark", () => {
    const markup = renderToStaticMarkup(
  ...
  test("uses the OfflineGPT mark when no managed square icon exists", () => {
    const markup = renderToStaticMarkup(
      <SidebarBrandMark metadata={null} organizationName="Acme" />,
    );

    expect(markup).toContain("<svg");
    expect(markup).toContain('aria-label="OfflineGPT"');
