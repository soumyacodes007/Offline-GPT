# OfflineGPT Cloud App (`ee/apps/den-web`)

Frontend for `app.offlinegptlabs.com`.
...
- Offers desktop handoff actions so users can open the generated worker directly in OfflineGPT or copy the connect credentials manually.
- Calls the Den API directly at the matching `api.*` origin (for example, `app.offlinegptlabs.com` -> `api.app.offlinegptlabs.com`), including Better Auth traffic.
...
1. Install workspace deps from repo root:
   `pnpm install`
2. Run the app:
   `pnpm --filter @offlinegpt-ee/den-web dev`
3. Open:
   `http://localhost:3005`
...
- `DEN_WEB_OFFLINEGPT_APP_CONNECT_URL` (runtime): Base URL for "Open in App" links.
  - Example: `https://offlinegptlabs.com/app`
  - The web panel appends `/connect-remote` and injects worker URL/token params automatically.
- `DEN_WEB_OFFLINEGPT_WEB_URL` (runtime): URL opened by the dashboard Web tab.
  - default: `https://web.offlinegptlabs.com`
- `DEN_WEB_OFFLINEGPT_AUTH_CALLBACK_URL` (runtime): Canonical URL where the app returns after auth completes.
...
- `DEN_MCP_CLAIM_NAMESPACE`: namespace used for MCP token claim URIs. Leave blank to use `BETTER_AUTH_URL`; set a stable value before issuing tokens if hosts may change. Use `https://offlinegptlabs.com` to preserve the original hosted MCP claim names.
...
- Build command: `cd ../../.. && pnpm --filter @offlinegpt-ee/den-web build`
- Output directory: `.next`
- Install command: use the command committed in `vercel.json`

The filtered install includes Den Web's workspace dependency closure, so shared packages like `@offlinegpt-ee/utils` remain available without installing unrelated native desktop and server dependencies.

Then assign custom domain:

- `app.offlinegptlabs.com`
