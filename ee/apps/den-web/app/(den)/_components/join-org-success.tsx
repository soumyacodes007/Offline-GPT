...
import { detectPlatform, type DetectedPlatform } from "@offlinegpt/ui/react";
...
const OFFLINEGPT_DOWNLOAD_URL = "https://offlinegptlabs.com/download";

function ReturnToOfflineGPTStatus({
  offlinegptUrl,
  grant,
  organizationName,
}: {
  ...
  if (status === "consumed") {
    return (
      <div className="rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700" data-testid="desktop-connected" aria-live="polite">
        Connected — {organizationName} is ready in OfflineGPT.
      </div>
    );
  }
  ...
  if (timedOut || status === "unknown") {
    return (
      <div className="grid gap-3 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600" data-testid="desktop-handoff-troubleshoot" aria-live="polite">
        <p className="m-0">
          Nothing opened?{" "}
          <button type="button" className="font-medium text-slate-950 underline-offset-4 hover:underline" onClick={() => window.location.assign(offlinegptUrl)}>
            Return to OfflineGPT again
          </button>
        </p>
        <div className="grid gap-2">
          <p className="m-0">Still stuck? Copy this sign-in link into OfflineGPT:</p>
          <div className="flex flex-col gap-2 sm:flex-row">
            <input className="den-input min-w-0 flex-1 text-xs" value={offlinegptUrl} readOnly onFocus={(event) => event.currentTarget.select()} />
...
export function JoinOrgSuccess({
  ...
  const [desktopOfflineGptUrl, setDesktopOfflineGptUrl] = useState<string | null>(null);
  ...
  async function handleReturnToOfflineGPT() {
    setHandoffBusy(true);
    setActionError(null);
    ...
    try {
      ...
      if (!response.ok) {
        setActionError(getErrorMessage(payload, `Could not return to OfflineGPT (${response.status}).`));
        return;
      }

      const offlinegptUrl = getDesktopHandoffOfflineGptUrl(payload);
      if (!offlinegptUrl) {
        setActionError("OfflineGPT sign-in was prepared, but no app link was returned.");
        return;
      }

      const grant = getDesktopHandoffGrant(payload, offlinegptUrl);
      rememberDesktopHandoffGrant(grant);
      setDesktopOfflineGptUrl(offlinegptUrl);
      setDesktopGrant(grant);
      window.location.assign(offlinegptUrl);
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "Could not return to OfflineGPT.");
    } finally {
          ...
          <div className="grid gap-3">
            <h1 className="m-0 grid max-w-full gap-1 text-[30px] font-semibold leading-[38px] tracking-[-0.03em] text-slate-950 sm:text-[38px] sm:leading-[46px]">
              ...
              {/* No horizontal gap: the possessive must hug the org identity
                  ("Acme Robotics's OfflineGPT", not "Acme Robotics 's"). */}
            ...
            <p className="m-0 max-w-2xl text-[15px] leading-[23px] text-slate-600">
              {desktopAuthRequested
                ? "Your team setup is ready. Return to OfflineGPT to continue where you left off."
                : "The desktop app is where OfflineGPT runs on your computer and puts your team's setup to work."}
            </p>
          </div>
          ...
          {isMobile === null ? (
            ...
            <div className="grid gap-3">
              <div className="grid gap-2 rounded-2xl bg-slate-50 p-4" data-testid="join-org-mobile-note">
                <p className="m-0 text-sm font-medium text-slate-950">OfflineGPT runs on your computer.</p>
            ...
            desktopOfflineGptUrl ? (
              <ReturnToOfflineGPTStatus offlinegptUrl={desktopOfflineGptUrl} grant={desktopGrant} organizationName={organizationName} />
            ) : (
              <button
                type="button"
                className="den-button-primary min-h-12 w-full"
                onClick={() => void handleReturnToOfflineGPT()}
                disabled={handoffBusy}
                data-testid="join-org-return-offlinegpt"
              >
                {handoffBusy ? "Returning to OfflineGPT..." : "Return to OfflineGPT"}
              </button>
            )
          ) : !platformReady ? (
            <p className="m-0 text-sm text-slate-500">Preparing your next step...</p>
            ...
            <div className="grid gap-3">
              ...
              >
                {handoffBusy ? "Opening OfflineGPT..." : "Already have OfflineGPT? Open it."}
              </button>
            </div>
          )}

          {desktopAuthRequested && desktopOfflineGptUrl ? null : (
            <div className="rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700" data-testid="join-org-connected">
              Connected — OfflineGPT is set up for {organizationName}
            </div>
          )}
          ...
          {actionError ? (
            <div className="grid gap-3">
              ...
              {desktopAuthRequested ? null : (
                <a href={OFFLINEGPT_DOWNLOAD_URL} className="den-button-secondary min-h-12 w-full">
