...
function lastBoundaryFromDiagnostic(diagnostic: ExternalMcpDiagnostic | null): string | undefined {
  if (diagnostic?.highestPassed === "operation_ready") return "OfflineGPT started remote tool execution";
  if (diagnostic?.highestPassed === "catalog_ready") return "OfflineGPT loaded the remote MCP tool catalog";
  if (diagnostic?.highestPassed === "protocol_ready") return "OfflineGPT initialized the remote MCP session";
  if (diagnostic?.highestPassed === "authorized") return "Remote MCP accepted the connection credential";
  if (diagnostic?.highestPassed === "reachable") return "OfflineGPT reached the remote MCP endpoint";
  if (diagnostic?.highestPassed === "configured") return "OfflineGPT loaded the MCP connection configuration";
  return undefined;
}
...
export function attributeExternalMcpToolFailure(input: {
  ...
  if (browserTimeout) {
    ...
    return {
      summary: `OfflineGPT stopped waiting after ${duration} seconds. The operation’s outcome is unknown.`,
      lastConfirmedBoundary: "OfflineGPT dashboard sent the request",
      likelySource: "Source unclear after OfflineGPT timeout",
      confidence: "Inferred",
  ...
  if (blockedBeforeSend) {
    return {
      ...
      retryGuidance: diagnostic?.operatorAction ?? "Resolve the OfflineGPT policy or connection configuration, then run the tool again.",
      outcome: "failed",
      ...details,
    };
  }
  ...
  if (inspection?.request && !inspection.response) {
    return {
      summary: "OfflineGPT started the outbound request, but no HTTP response was captured. This does not prove the remote MCP caused the failure.",
      lastConfirmedBoundary: "OfflineGPT started the outbound tools/call",
  ...
  return {
    summary: inspection?.diagnosis?.summary
      ?? diagnostic?.message
      ?? "The request failed before OfflineGPT received a tool result.",
    lastConfirmedBoundary: lastBoundaryFromDiagnostic(diagnostic)
      ?? (diagnostic ? "OfflineGPT returned a structured diagnostic" : "OfflineGPT dashboard sent the request"),
    likelySource: networkSetup ? "Connection path or remote MCP" : "OfflineGPT or MCP setup",
    confidence: "Inferred",
    retryGuidance: diagnostic?.operatorAction ?? "Use the diagnostic reference to inspect OfflineGPT and MCP connection health before retrying.",
    outcome: "failed",
    ...details,
  };
}
