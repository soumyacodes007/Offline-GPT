...
<html lang="en">
  <head>
    ...
    <title>Connecting — OfflineGPT</title>
    <style>
${authorizationDocumentStyles}
  ...
  <body>
    <main class="card connect-card" role="status" aria-live="polite">
      <div class="brand"><span class="brand-mark">OW</span>OfflineGPT Connect</div>
      <h1>Connect your account</h1>
      <p>OfflineGPT is preparing the secure provider sign-in.</p>
...
<html lang="en">
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <meta name="color-scheme" content="light" />
    <title>Connection failed — OfflineGPT</title>
    <style>
${authorizationDocumentStyles}
    ...
    <main class="card error-card" role="alert" aria-live="assertive">
      <div class="error-header">
        ...
        <div class="status">
          ...
          <p>OfflineGPT couldn’t start the provider sign-in.</p>
        </div>
...
export function openMcpAuthorizationWindow(): Window {
  const popupName = `offlinegpt-mcp-authorization-${crypto.randomUUID()}`
  const popup = window.open("", popupName, "popup,width=960,height=720")
  if (!popup) {
    throw new Error("OfflineGPT could not open the sign-in window. Allow popups for OfflineGPT, then try again.")
  }
  try {
    popup.opener = null
    popup.document.open()
