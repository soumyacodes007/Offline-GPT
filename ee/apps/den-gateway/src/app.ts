...
import { createJsonStdoutLogger, type JsonObject, type JsonStdoutLogger } from "@offlinegpt-ee/utils/observability"
...
export function shouldProxyToInstance(request: Request) {
  ...
  // /workspace/ is ambiguous: offlinegpt-server owns API routes here, while the
...
function injectGatewayMarker(html: string, buildVersion: string | undefined) {
  ...
  const script = `<script>window.__OFFLINEGPT_GATEWAY__ = ${marker}</script>`
...
function upstreamRequestHeaders(headers: Headers, clientToken: string, hostToken: string) {
  ...
  upstream.delete("x-offlinegpt-host-token")
  ...
  upstream.set("x-offlinegpt-host-token", hostToken)
  return upstream
}
...
function denApiRequestHeaders(request: Request) {
  ...
  request.headers.forEach((value, name) => {
    ...
    if (normalized === "x-offlinegpt-host-token") return
