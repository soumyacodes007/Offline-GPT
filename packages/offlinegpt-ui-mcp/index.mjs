 ...
 * offlinegpt-ui-mcp
 *
 * MCP server that exposes OfflineGPT's semantic app context and UI control
 * surface as an MCP resource and tools.
 * Speaks MCP stdio and proxies to the OfflineGPT desktop bridge HTTP API.
 *
 * Requires OfflineGPT desktop running with the local UI control bridge active.
 *
 * Usage:
...
const DISCOVERY_FILE = "offlinegpt-ui-control.json";
...
async function bridgeRequest(path, options = {}) {
  ...
  if (!bridge) {
    return {
      ok: false,
      error: "OfflineGPT is not running. Launch the OfflineGPT desktop app first.",
      hint: "The MCP server connects to a running OfflineGPT instance via its local bridge.",
    };
  }
...
// ── offlinegpt://context/current ──
server.resource(
  "current_offlinegpt_context",
  "offlinegpt://context/current",
  async (uri) => {
...
server.tool(
  "ui_context",
  "Read OfflineGPT's semantic app context: current screen, all open conversation tabs, split-screen layout, focused pane, sidebar, side panel, settings, and currently available queries and commands. Reads app state without focusing the window.",
  {},
  async () => {
    const result = await bridgeRequest("/context");
...
server.tool(
  "ui_snapshot",
  "Get a snapshot of the current OfflineGPT UI state: active route, narration, visible actions, and status. Use this before taking action to understand what the user sees.",
  {},
  async () => {
    ...
    return { content: [{ type: "text", text: lines.join("\n") || "OfflineGPT is reachable, but it did not return visible UI state." }] };
  }
);
...
server.tool(
  "ui_list_actions",
  "List all UI control actions currently available in OfflineGPT: session navigation, composer control, transcript access, and more. Each action has an id you can pass to ui_execute_action.",
  {},
  async () => {
    ...
    if (!Array.isArray(result.actions) || result.actions.length === 0) {
      return { content: [{ type: "text", text: "No actions available. Is OfflineGPT on the main screen?" }] };
    }
...
server.tool(
  "ui_execute_action",
  "Execute an OfflineGPT UI action by its id without activating the desktop window. Use ui_list_actions first to see available actions and their required arguments.",
  {
...
server.tool(
  "ui_query",
  "Run a side-effect-free OfflineGPT query by id. Use ui_context to inspect currently available queries. Queries never focus or navigate the app.",
  {
...
server.tool(
  "ui_command",
  "Execute a semantic OfflineGPT command by id while OfflineGPT remains in the background. Use ui_context first and pass its revision to prevent acting on stale UI state.",
  {
...
server.tool(
  "ui_status",
  "Check if OfflineGPT is running and the bridge is reachable. Returns connection status and app info.",
  {},
  async () => {
    const bridge = await discoverBridge();
    if (!bridge) {
      return { content: [{ type: "text", text: "OfflineGPT is not running.\nLaunch the OfflineGPT desktop app to enable UI control." }], isError: true };
    }
    try {
      const response = await fetch(`${bridge.baseUrl}/health`, { signal: AbortSignal.timeout(3000) });
      const data = await response.json();
      return { content: [{ type: "text", text: `Connected to ${data.app || "OfflineGPT"}\nBridge: ${bridge.baseUrl}\nVersion: ${data.version ?? "?"}` }] };
    } catch (error) {
      clearBridgeCache();
      return { content: [{ type: "text", text: `Bridge file found but not reachable: ${error.message}\nOfflineGPT may have quit. Relaunch it.` }], isError: true };
    }
  }
);
