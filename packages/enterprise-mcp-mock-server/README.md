# `@offlinegpt/enterprise-mcp-mock-server`
...
This package owns the mock **data plane**. It deliberately does not import Den, an OfflineGPT client, or the Enterprise Mock Lab. The lab consumes this public API and supplies the protected control plane.
...
```bash
pnpm --filter @offlinegpt/enterprise-mcp-mock-server check
```
