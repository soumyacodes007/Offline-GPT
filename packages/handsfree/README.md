# OfflineGPT Computer Use

Native macOS computer-use runtime for OfflineGPT.
Packaged OfflineGPT builds wrap this runtime in a bundled `Computer Use.app`
...
```bash
pnpm --filter @offlinegpt/handsfree check:native
```

Run it as an MCP-compatible adapter:

```bash
pnpm --filter @offlinegpt/handsfree exec offlinegpt-handsfree-computer-use mcp
```
