---
title: "Roadmap"
description: "The roadmap for the OfflineGPT desktop app, portable capabilities, hosted workspaces, and every surface where work happens."
---
...
OfflineGPT implements the [MCP authorization specification](https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization) on both sides of the wire: as an MCP client (the desktop app and OfflineGPT Cloud connections) and as an MCP server with its own authorization server (OfflineGPT Cloud). Items marked Building or Next track the 2026-07-28 MCP specification release.
...
| Client ID Metadata Documents (SEP-991) | A single, stable HTTPS client identity for OfflineGPT that enterprises allowlist once, replacing per-connection registrations and wildcard redirect allowlists. | Building |
...
| URL-based client IDs in OfflineGPT Cloud | Accept Client ID Metadata Document identities in the OfflineGPT Cloud authorization server. | Next |

> Following the spec is what makes OfflineGPT drop-in compatible with enterprise MCP gateways: paste the gateway URL, sign in with your identity provider, and every downstream system it fronts becomes available.
...
| Anthropic-compatible plugins | Import Claude-compatible plugin and marketplace manifests and normalize their skills, MCPs, commands, and tools into OfflineGPT extensions. | Live |
...
| OpenTelemetry coverage | Extend OTLP traces, metrics, and logs across OfflineGPT services and deployment paths. | Building |
...
| Surface | Status |
| --- | --- |
| OfflineGPT desktop | Live |
| Existing AI agents through MCP | Live |
| OfflineGPT Web — the full workspace in your browser, nothing to install | Preview |
| Slack | Next |
| Mobile | Next |
...
[Share feedback](https://offlinegptlabs.com/feedback?source=roadmap)
