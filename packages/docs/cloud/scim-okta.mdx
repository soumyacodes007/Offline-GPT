...
description: "Provision OfflineGPT members and teams from Okta"
---

Use Okta Lifecycle Management to create, update, deactivate, and reactivate
OfflineGPT members. Okta groups can also become SCIM-managed OfflineGPT teams.
...
test pass. Make sure at least one OfflineGPT owner can sign in through Okta and
that the current SSO configuration has passed OfflineGPT's **Test SSO before
enabling it** flow.

## 1. Create the OfflineGPT SCIM connector

In OfflineGPT, select the organization, then open **Settings → SCIM**. Confirm
the page reports that SAML/SSO is active, then:

1. Select **Create connector**.
2. Copy the **SCIM base URL**.
3. Copy the bearer token immediately. OfflineGPT displays the full token only
...
<Note>
Use the base URL OfflineGPT displays. In a split-host Helm deployment it may use
...
1. Open **Applications → Applications → OfflineGPT**.
...
| Okta field | Value |
| --- | --- |
| SCIM connector base URL | OfflineGPT **SCIM base URL** |
...
| Authorization | OfflineGPT SCIM bearer token |
...
## 3. Configure provisioning to OfflineGPT
...
does not create an OfflineGPT password credential.
...
can create a duplicate OfflineGPT member instead of linking SSO and SCIM.
...
5. In OfflineGPT, open **Members** and verify the same member appears without a
...
1. Changing the profile name in Okta updates the OfflineGPT member.
...
3. The user cannot regain access through password sign-up while SCIM marks the
   identity inactive.
4. Reactivating and reassigning the user restores the same OfflineGPT identity.
5. An unrelated OfflineGPT member remains unchanged throughout the test.

OfflineGPT retains a disconnected member record for organization history. The
global user is deleted only after their final active organization membership
is removed.

## 6. Push Okta groups as OfflineGPT teams

After user provisioning works:

1. In OfflineGPT **Settings → SCIM**, enable
...
5. In OfflineGPT, open **Members → Teams** and verify a team labeled
   **Managed by SCIM** appears with the expected members.
6. Add and remove one member in Okta and verify the OfflineGPT team follows.

Manage pushed groups in Okta, not in OfflineGPT. Manually created OfflineGPT teams
remain independent.

### Optional: grant organization Admin through an Okta group

After verifying the pushed membership, an OfflineGPT owner or super-admin can
...
Okta **System Log** and OfflineGPT SCIM health if the change has not arrived.
...
Assignments page and System Log for outbound provisioning. When OfflineGPT has no
...
| --- | --- | --- |
| **Create connector** is disabled | OfflineGPT does not see an enabled SSO provider | Complete and test Okta SAML first; if SAML returns `invalid_saml_configuration`, upgrade and save the connection again |
| **Test API Credentials** fails | Incorrect base URL, stale token, whitespace, or a rewritten hostname | Copy both values again from OfflineGPT and use the advertised base URL unchanged |
| Okta reports `409 User already exists` | The user existed before SCIM, or more than one create job ran for the same assignment | Inspect the System Log first. If a successful create immediately precedes the conflict and the exact member exists in OfflineGPT, stop triggering **Provision User**. Use a never-before-provisioned user for the clean lifecycle test |
...
| **Run reconciliation** does not provision an assigned user | Reconciliation only checks OfflineGPT's local SCIM identity state | Trigger and inspect outbound provisioning in Okta instead |
| Deactivation does not remove access | **Deactivate Users** is disabled or Okta has not sent the event | Enable it and inspect the Okta System Log and OfflineGPT SCIM health |
| A pushed group does not create a team | OfflineGPT team synchronization is off | Enable **Create teams from SCIM groups**, then retry the Okta group push |
