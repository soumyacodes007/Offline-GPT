---
title: "Get started with OfflineGPT Cloud"
...
<Note>
OfflineGPT Cloud is optional. The local desktop app stays free, requires no Cloud sign-in, and supports your own providers, skills, plugins, and custom MCP servers. Use Cloud when an organization needs to assign and manage those resources for a team.
</Note>

OfflineGPT Cloud is available at [app.offlinegptlabs.com](https://app.offlinegptlabs.com/). It is for teams that want shared identity, Collections, managed connections and models, desktop policies, or hosted workspaces.

First, create or join an organization and connect the desktop app to it.

<Frame>
  ![OfflineGPT Cloud org settings with organization identity, allowed email domains, and desktop restrictions](/images/cloud-org-settings.png)
</Frame>

1. Open [app.offlinegptlabs.com](https://app.offlinegptlabs.com/) in your browser.
2. Click `Create organization`.
3. Enter your org name.
4. Finish checkout if OfflineGPT Cloud asks for billing.
...
1. Open the `dashboard/org_settings` [page](https://app.offlinegptlabs.com/dashboard/org-settings) in the cloud app.
...
4. If the browser does not return to OfflineGPT automatically, click `Paste sign-in code`, paste the `offlinegpt://den-auth?...` link or one-time code from OfflineGPT Cloud, then click `Finish sign-in`.

## Cloud URL used by the desktop app

The desktop app uses a single Cloud URL: the Den web `baseUrl`. The hosted
default is `https://app.offlinegptlabs.com`.
...
- OfflineGPT Cloud MCP requests go to `<baseUrl>/api/den/mcp/...`.
...
back to `https://app.offlinegptlabs.com`. When you change the Cloud URL in
`Settings > Account`, OfflineGPT writes the new `baseUrl` back to the bootstrap
...
- Use [OfflineGPT Web](/cloud/run-in-the-cloud/shared-workspace) to give members a hosted OfflineGPT workspace in the browser, with nothing to install.
