...
description: "Provision OfflineGPT members and teams from Microsoft Entra ID"
---

Use Microsoft Entra ID provisioning to create, update, and deactivate OfflineGPT
members automatically. Entra security groups can also become OfflineGPT teams
...
- An OfflineGPT organization with Enterprise SSO and SCIM.
- An OfflineGPT owner or super-admin.
- A working, domain-verified, tested, and enabled OfflineGPT SAML connection.
  Complete and test
...
SCIM. Make sure at least one OfflineGPT owner can sign in through SAML and that
the current SSO configuration has passed OfflineGPT's **Test SSO before enabling
it** flow before relying on Entra as the identity source.

## 1. Get the OfflineGPT connection values

In OfflineGPT, select the organization you want to connect, then open
**Settings → SCIM**.
...
3. Copy the **SCIM bearer token** immediately. OfflineGPT shows the full token
   only after creating or rotating it.
4. To synchronize Entra groups as OfflineGPT teams, select **Enable team sync**
...
| --- | --- |
| Tenant URL | OfflineGPT **Settings → SCIM → SCIM base URL**. It normally ends in `/api/auth/scim/v2`. |
...
2. Select the same enterprise application used for OfflineGPT SAML SSO.
3. Open **Provisioning**.
...
6. Paste the OfflineGPT **SCIM base URL** into **Tenant URL**.
7. Paste the OfflineGPT **SCIM bearer token** into **Secret Token**.
...
principal names use an `onmicrosoft.com` domain but users sign in to OfflineGPT
...
create a second OfflineGPT member for the same person.
...
not enough; assign it to the OfflineGPT enterprise application as well.
...
3. Confirm that import, scope evaluation, matching, and the final action all
   succeed.
4. In OfflineGPT, open **Members** and confirm that the member appears with the
   expected email.
...
6. In OfflineGPT, open **Members → Teams** and confirm that the group appears as a
...
sends a group-create request to OfflineGPT. Add members and select them explicitly
...
5. Refresh OfflineGPT **Members** and **Teams** to verify the final state.
...
1. Remove a test user from the Entra group and confirm that OfflineGPT removes the
   user from the SCIM-managed team.
2. Disable or unassign a test user and confirm the expected OfflineGPT
   deprovisioning behavior.
3. Add another assigned group and confirm that OfflineGPT creates a second
   SCIM-managed team.

Change SCIM-managed teams in Entra, not in OfflineGPT. OfflineGPT keeps manually
created teams separate from identity-provider-managed teams.

### Optional: grant organization Admin through an Entra group

After verifying the synchronized membership, an OfflineGPT owner or super-admin
...
| --- | --- | --- |
| **Test Connection** fails | Incorrect Tenant URL, expired or rotated token, or copied whitespace | Copy the base URL and a current token again from OfflineGPT, then retest |
...
| On-demand group provisioning reports an internal server error without an OfflineGPT request | The group is empty or no members were selected for the on-demand run | Add members, select up to five under **Selected users**, and retry |
| A group provisions but no OfflineGPT team appears | OfflineGPT team synchronization is disabled | Open **Settings → SCIM** and enable **Create teams from SCIM groups** |
| Entra creates a duplicate OfflineGPT member | SAML and SCIM use different email identifiers | Align the SAML NameID and the SCIM `userName` mapping |
| Entra reports success but OfflineGPT still shows the old state | The UI or incremental cycle has not refreshed | Refresh OfflineGPT, check Entra **Provisioning logs**, and review the SCIM health status in OfflineGPT |
