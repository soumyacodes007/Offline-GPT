...
The runnable example at [`examples/litellm-per-member-keys`](https://github.com/different-ai/offlinegpt/tree/dev/examples/litellm-per-member-keys) uses LiteLLM virtual keys. Configure the Den and LiteLLM URLs, admin tokens, provider ID, and model IDs listed in its README, then run:
