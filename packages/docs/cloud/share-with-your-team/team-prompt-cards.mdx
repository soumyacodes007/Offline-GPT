...
Organization prompt cards are an OfflineGPT Cloud desktop-policy feature. Admins manage them in Cloud, and assigned members see them in the desktop composer as task suggestions. Managing desktop policies requires an Enterprise plan; the API returns `402` when the org is not entitled.
...
<Frame>
  ![OfflineGPT desktop composer showing organization prompt cards assigned through a Cloud desktop policy](/images/desktop-team-prompt-cards.png)
</Frame>
