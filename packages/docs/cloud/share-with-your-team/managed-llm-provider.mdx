...
description: "Create an org-managed provider in OfflineGPT Cloud and import it into the desktop app"
---
...
## Create the provider in OfflineGPT Cloud
...
<Frame>
  ![Managed LLM provider detail in OfflineGPT Cloud](/images/cloud-managed-llm-provider-detail.png)
</Frame>
...
OfflineGPT writes the imported provider into the workspace `opencode.jsonc` and stores the org credential for that workspace.

## Update or remove it later

- Use `Sync` after changing models or access in OfflineGPT Cloud.
