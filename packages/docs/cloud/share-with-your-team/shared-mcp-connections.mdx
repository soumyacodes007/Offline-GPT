...
description: "Share Notion, Linear, Stripe, Exa, or any MCP server in OfflineGPT Cloud with Individual accounts by default, or One org account when everyone should act as the same account"
---
...
## Publish the connection in OfflineGPT Cloud

1. Open `Connectors` in the OfflineGPT Cloud dashboard.
...
<Frame>
  ![MCP Connections admin screen in OfflineGPT Cloud](/images/cloud-mcp-connections-admin.png)
</Frame>
...
For the hosted OfflineGPT Cloud instance, the dashboard is
`https://app.offlinegptlabs.com` and its public Den API origin is
`https://api.offlinegptlabs.com`. Register this exact redirect URL with the OAuth
provider:

```text
https://api.offlinegptlabs.com/v1/mcp-connections/oauth/callback
```

For a self-hosted instance whose public Den API is
`https://api.offlinegpt.example.com`, register:

```text
https://api.offlinegpt.example.com/v1/mcp-connections/oauth/callback
```
...
Slack's MCP server does not support automatic OAuth client registration, so a Slack admin creates a Slack app once and OfflineGPT Cloud uses its credentials for every member.
...
6. In OfflineGPT Cloud, add the Slack connection and paste the app's `Client ID` and `Client Secret` from `Basic Information` > `App Credentials`.
...
Granted members find the connection in `Settings` > `OfflineGPT Connect`, grouped by what
they need to do:
...
1. Sign in to OfflineGPT, then open `Settings` > `OfflineGPT Connect`.
...
4. The browser shows `Connected` — return to OfflineGPT.
5. The connection updates by itself, no reload needed, and moves to
   `Ready to use`.

Your sign-in is stored in OfflineGPT Cloud, not on this machine — connect once
...
The agent searches the live capabilities available through OfflineGPT Connect,
...
action and direct you to `Settings` > `OfflineGPT Connect`.

## Expose a connection directly as an MCP server

By default the agent reaches every connection through OfflineGPT Connect's two
...
that reads the OfflineGPT Connect server index.

What stays the same:

- Sign-in still happens in OfflineGPT Cloud. The desktop app never receives the
  provider's URL or token; it talks to Den's per-connection endpoint with the
  member's own OfflineGPT credential.
...
Directly exposed connections appear in OpenCode as `offlinegpt-direct-<name>-…`.
...
services managed through OfflineGPT Cloud.
