---
title: "OfflineGPT MCP for VS Code"
sidebarTitle: "VS Code"
description: "VS Code MCP setup for OfflineGPT Connect, with OAuth access to your org's Gmail, Calendar, Drive, shared connections, skills, and Cloud resources."
---

The OfflineGPT MCP server ([OfflineGPT Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables VS Code to use the OfflineGPT capabilities your organization has made
...
```sh
code --add-mcp '{"name":"offlinegpt","type":"http","url":"https://api.offlinegptlabs.com/mcp/agent"}'
```

Then open the Command Palette and run `MCP: List Servers` > `offlinegpt` >
...
Then run `MCP: List Servers` > `offlinegpt` > `Start Server`, and follow the
sign-in prompt.

## Switch organizations

Remove the stored MCP auth token for `offlinegpt`, then start the server again
