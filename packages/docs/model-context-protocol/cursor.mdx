---
title: "OfflineGPT MCP for Cursor"
sidebarTitle: "Cursor"
description: "Cursor MCP setup for OfflineGPT Connect on Cursor Desktop, Cursor Web, and Cursor Agents."
---

The OfflineGPT MCP server ([OfflineGPT Connect](/cloud/run-in-the-cloud/cloud-mcp))
enables Cursor to use the OfflineGPT capabilities your
...
Add OfflineGPT as a remote MCP server in Cursor's MCP settings, paste the server
URL, and start OAuth from Cursor's connection prompt. Sign in to OfflineGPT and
pick the organization Cursor should use.

Cursor Desktop's OAuth callback is the private-use URI
`cursor://anysphere.cursor-mcp/oauth/callback`. OfflineGPT accepts exactly this
...
Other private-use callback schemes remain rejected: OfflineGPT otherwise accepts
...
OfflineGPT, and pick the organization Cursor should use.

Cursor Web and Cursor Agents use HTTPS OAuth callbacks, which OfflineGPT
accepts.
...
steps match OfflineGPT Connect's OAuth profile: HTTPS or HTTP loopback callbacks
