...
description: "Run the OfflineGPT control plane from published images without cloning or building the repository."
---

Use the pull-only Docker Compose stack to evaluate Den web, Den API, and organization management on one machine. The stack downloads published OfflineGPT images and MySQL, runs database migrations, and starts the browser experience at `http://localhost:3005`.
...
| --- | --- | --- |
| `OFFLINEGPT_WEB_PORT` | `3005` | Host port for Den web. |
| `OFFLINEGPT_API_PORT` | `8788` | Host port for Den API. |
| `OFFLINEGPT_AUTH_SECRET` | Required | Better Auth signing secret of at least 32 random characters. |
| `OFFLINEGPT_DB_ENCRYPTION_KEY` | Required | Key of at least 32 random characters used to encrypt selected database values. |
| `OFFLINEGPT_ORG_NAME` | `OfflineGPT Evaluation` | Name of the single organization. |
| `OFFLINEGPT_ALLOW_SIGNUP` | `true` | Whether anyone who can reach the site can create an account. |
