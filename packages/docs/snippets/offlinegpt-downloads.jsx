export const OfflineGPTDownloads = () => {
    ...
    return [
      {
        label: "macOS",
        downloads: [
          { label: "Mac (Apple Silicon)", asset: findAsset(`offlinegpt-mac-arm64-${version}.dmg`) },
          { label: "Mac (Intel)", asset: findAsset(`offlinegpt-mac-x64-${version}.dmg`) },
        ].filter((download) => download.asset),
      },
      {
        label: "Windows",
        downloads: [
          { label: "Windows (x64)", asset: findAsset(`offlinegpt-win-x64-${version}.exe`) },
          { label: "Windows (ARM64)", asset: findAsset(`offlinegpt-win-arm64-${version}.exe`) },
        ].filter((download) => download.asset),
      },
      {
        label: "Linux",
        downloads: [
          { label: "AppImage (x64)", asset: findAsset(`offlinegpt-linux-x86_64-${version}.AppImage`) },
          { label: "AppImage (ARM64)", asset: findAsset(`offlinegpt-linux-arm64-${version}.AppImage`) },
          { label: "tar.gz (x64)", asset: findAsset(`offlinegpt-linux-x64-${version}.tar.gz`) },
          { label: "tar.gz (ARM64)", asset: findAsset(`offlinegpt-linux-arm64-${version}.tar.gz`) },
        ].filter((download) => download.asset),
      },
    ];
  };
  const detectPlatform = () => {
    if (typeof navigator === "undefined") {
      return { label: "macOS", assetName: (version) => `offlinegpt-mac-arm64-${version}.dmg` };
    }
    ...
    if (platform.includes("win")) {
      return { label: "Windows", assetName: (version) => `offlinegpt-win-x64-${version}.exe` };
    }
    if (platform.includes("linux")) {
      return { label: "Linux", assetName: (version) => `offlinegpt-linux-x86_64-${version}.AppImage` };
    }
    return { label: "macOS", assetName: (version) => `offlinegpt-mac-arm64-${version}.dmg` };
  };
  ...
  const loadAlpha = async () => {
    ...
    try {
      ...
      release.assets.forEach((asset) => {
        if (!asset.name.startsWith("offlinegpt-mac-arm64-") || (!asset.name.endsWith(".dmg") && !asset.name.endsWith(".zip"))) return;
      ...
      setAlphaBuilds(
        Object.values(buildsByRun)
          ...
          .map((build) => ({
            ...build,
            version: build.dmg.name.replace("offlinegpt-mac-arm64-", "").replace(/\.dmg$/, ""),
          }))
  ...
  return (
    <div id="offlinegpt-downloads" className="not-prose my-8">
      {latestStable ? (
      ...
      <div className="mb-5 overflow-x-auto border-b border-gray-200 dark:border-white/10" role="tablist" aria-label="OfflineGPT download channels">
