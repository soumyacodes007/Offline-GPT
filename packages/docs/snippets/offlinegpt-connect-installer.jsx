export const OfflineGPTConnectInstaller = () => {
  const MCP_SERVER_URL = "https://api.offlinegptlabs.com/mcp/agent";
  ...
  const CODEX_LOGIN_COMMAND = "codex mcp login offlinegpt";
  const CODEX_RECONNECT_COMMAND = `codex mcp logout offlinegpt
codex mcp login offlinegpt`;
  const OPENCODE_AUTH_COMMAND = "opencode mcp auth offlinegpt";
  const OPENCODE_RECONNECT_COMMAND = `opencode mcp logout offlinegpt
opencode mcp auth offlinegpt`;
  const installs = [
    ...
    },
    {
      ...
      helper: "Add OfflineGPT once, then sign in with Codex's MCP login command.",
      supportStatus: "Setup only",
      supportExplanation: "Setup guide only: add OfflineGPT, run codex mcp login offlinegpt, and reconnect with logout then login. Native proof must be rerun on this exact branch.",
      copyText: `codex mcp add offlinegpt --url ${MCP_SERVER_URL}`,
    ...
    },
    {
      ...
      copyText: `claude mcp add --transport http offlinegpt ${MCP_SERVER_URL}`,
    },
    {
      id: "opencode",
      label: "OpenCode",
  ...
  "mcp": {
    ...
    },
    {
      ...
      copyText: `code --add-mcp '{"name":"offlinegpt","type":"http","url":"${MCP_SERVER_URL}"}'`,
    },
    {
      id: "any-client",
      label: "Any client",
  ...
  return (
    <div id="connect-mcp-install" className="not-prose my-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-950">
      ...
      <div className="overflow-x-auto border-b border-gray-100 px-4 pt-4 dark:border-white/10" role="tablist" aria-label="OfflineGPT MCP client install options">
      ...
      <div id={`connect-mcp-panel-${activeInstall.id}`} role="tabpanel" aria-labelledby={`connect-mcp-install-${activeInstall.id}`} className="p-5">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <p className="m-0 text-xs text-gray-500">Works with your OfflineGPT account — <a href="https://app.offlinegptlabs.com?mode=sign-up" className="font-medium underline">create one free</a>.</p>
          <button type="button" aria-label="Copy the OfflineGPT MCP install command" onClick={() => copy(activeInstall.id, activeInstall.copyText)} className="shrink-0 rounded-lg bg-[#011627] px-4 py-2 text-xs font-medium text-white">
      ...
      <div className="flex flex-col gap-2 border-t border-gray-100 px-5 py-4 text-xs sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <button type="button" aria-label="Copy the OfflineGPT MCP server URL" onClick={() => copy("server-url", MCP_SERVER_URL)} className="shrink-0 font-medium underline">
