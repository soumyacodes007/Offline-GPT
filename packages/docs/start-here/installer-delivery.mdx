...
description: "Choose how organization install links deliver standard OfflineGPT desktop installers."
---

Organization install links help members install the standard OfflineGPT desktop and connect it to the correct Den organization. Den does not build a customer-specific installer or wrap the installer in a ZIP. The implementation and security details remain in the repository's [Organization Install Links guide](https://github.com/soumyacodes007/offline-gpt/blob/dev/docs/org-install-links.md#installer-delivery).

## Delivery matrix

| Deployment style | Download behavior | Required client access |
| --- | --- | --- |
| Internet-connected | Den validates the organization install token, then redirects the browser to the exact standard GitHub release asset under `OFFLINEGPT_INSTALLER_RELEASE_REPO` and `OFFLINEGPT_INSTALLER_RELEASE_TAG`. The organization token is not forwarded. | Den web/API, `github.com`, `release-assets.githubusercontent.com`, and `objects.githubusercontent.com` for legacy or rollback paths. |
| Semi-air-gapped installer delivery | Den streams the matching standard installer from `OFFLINEGPT_INSTALLER_ARTIFACTS_DIR`. | Den web/API only for installer download. Desktop runtime dependencies may still need internet unless mirrored or disabled. |
...
When `OFFLINEGPT_INSTALLER_ARTIFACTS_DIR` points at a mounted directory and the expected file exists, Den streams that file directly. It does not fetch from GitHub, cache on first request, rewrap, ZIP, or hold the whole installer in memory.

`OFFLINEGPT_INSTALLER_RELEASE_TAG` selects the expected release version. The standard filenames use the release tag without a leading `v`:
...
`OFFLINEGPT_INSTALLER_RELEASE_REPO` controls the GitHub redirect path when Den does not find a mounted artifact. It is not an internal artifact registry setting.
