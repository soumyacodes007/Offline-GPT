...
description: "How to connect a custom provider with OfflineGPT"
---

<Note>
**Local provider setup.** A custom provider can stay entirely in your workspace. OfflineGPT remains free, and you do not need to sign in to OfflineGPT Cloud. For centrally managed team access, see [Adding a managed LLM provider](/cloud/share-with-your-team/managed-llm-provider).
</Note>

Because `OfflineGPT` is built on `OpenCode` primitives, we support everything that you could modify in `.opencode.json`, like adding a [model](https://opencode.ai/docs/models/). However, our recommendation is to add it to `/path-to-your-workspace/.config/opencode/opencode.json` instead of modifying `~/.config/opencode/opencode.json`.
...
For teams, manage the provider in OfflineGPT Cloud under [Models > Bring your Own Keys](/cloud/share-with-your-team/managed-llm-provider), then make it available from `Settings > Cloud Providers` in the desktop app.
