...
description: "Paste-able prompts that let your agent debug OfflineGPT issues on its own."
---

When something in OfflineGPT looks wrong, the fastest debugger is usually the agent itself: it can read its own configuration, probe servers, and verify fixes in-band. This page collects paste-able diagnostic prompts. Copy the whole block for your situation into a chat and let the agent work through it.
...
Your agent's `offlinegpt-cloud` MCP entry is a snapshot: it stores a server URL and token from the moment it was connected. If it was created against a local development server or an old deployment — including via a leftover `desktop-bootstrap.json` file that overrides server URLs beneath the Settings UI — the agent keeps querying that old world forever, while the dashboard shows the current one.

**Paste this to your agent**

````text
Debug and fix my OfflineGPT Cloud MCP connection. It may be pointed at a stale or local server instead of my real OfflineGPT Cloud server. Work through ALL phases, show your findings after each, and ask before anything destructive. Never print bearer tokens or Authorization headers — redact them.

My expected cloud web URL: https://app.offlinegptlabs.com (replace with your self-hosted Den web URL if you run your own).

## Phase 1 — Evidence: what am I actually connected to?

1. Print the `offlinegpt-cloud` MCP entry's `url` (redact headers) from every config you can find:
...
2. Inspect these files if they exist (they can silently override server URLs beneath the Settings UI), but report only `baseUrl`, `apiBaseUrl`, `requireSignin`, and `writtenAt`. Never print `handoff`, `claimLinks`, grants, tokens, or other credential-bearing fields:
   - `~/.config/offlinegpt/desktop-bootstrap.json`
   - `~/Library/Application Support/com.offlinegptlabs.offlinegpt/desktop-bootstrap.json`
...
- offlinegpt-cloud url is localhost/127.0.0.1, OR search returned only gmail-drafts, OR execute returned unknown_capability → you are bound to a STALE server, not the expected cloud.
...
10. Tell me to do these steps in OfflineGPT, then wait:
    - Restart the OfflineGPT app.
    - In the affected workspace, open Settings → Library and click Refresh (re-mints the token and rewrites the Cloud MCP config).
    - Click Show hidden, then verify OfflineGPT Cloud Control under Your apps.
...
- The `offlinegpt-cloud` entry's `url` points at your real server and ends in `/mcp/agent` (either a direct API host or the web app's `/api/den` proxy).
...
If the agent completes all phases and verification still fails, [open an issue](https://github.com/soumyacodes007/offline-gpt/issues) and paste the agent's final report (it is already redacted).

## 2. Cloud MCP is configured, but its tools are missing

**Symptoms**

- Settings shows an `offlinegpt-cloud` MCP entry, but the agent calls `offlinegpt_docs_search` for requests involving connected services.
...
- A connected service such as ServiceNow appears ready in OfflineGPT Cloud, but the task does not expose `offlinegpt-cloud_search_capabilities` or `offlinegpt-cloud_execute_capability`.

**What this diagnostic checks**

The prompt below runs OfflineGPT's bundled OpenCode executable against its generated runtime config and checks whether a fresh OpenCode process can connect to `offlinegpt-cloud`. It does not attach to the OpenCode server already running inside OfflineGPT, so it combines that result with the live status shown in OfflineGPT and a forced tool call from a new task.

**Paste this to your agent**

````text
Diagnose whether OfflineGPT Cloud MCP tools are available in this workspace. Gather evidence only: do not modify configuration, authenticate, log out, remove anything, restart services, or print configuration-file contents.

## Phase 1 — Test the task's actual tools

1. Attempt to call `offlinegpt-cloud_search_capabilities` with `{"query":"ServiceNow incident","type":"mcp","limit":10}`. Do not use documentation or explain alternatives. Record whether the runtime accepts the tool call and its exact non-secret result or rejection.
2. Attempt to call `offlinegpt_extension_list_actions`. Record whether the runtime accepts the call. Do not infer tool availability from an answer to "list your tools"; models can omit or invent tools.

## Phase 2 — Find OfflineGPT's exact OpenCode runtime

3. Detect whether this machine is Windows, macOS, or Linux.
4. Find the exact OpenCode executable used by the OfflineGPT desktop app. Prefer the bundled executable in OfflineGPT's `resources/sidecars` directory; do not assume an unrelated `opencode` on `PATH` is the same binary. If multiple OfflineGPT builds are running, use the process associated with this task when that can be established safely; otherwise report the ambiguity.
5. Find OfflineGPT's generated `runtime-opencode-config.json` beside the active `server.json`. Respect discovered `OFFLINEGPT_SERVER_CONFIG`, `OFFLINEGPT_RUNTIME_DB`, or XDG overrides. Typical locations are:
   - Windows: `%APPDATA%\offlinegpt\runtime-opencode-config.json`
   - macOS/Linux: `${XDG_CONFIG_HOME:-$HOME/.config}/offlinegpt/runtime-opencode-config.json`
...
9. Report:
   ...
   - whether `offlinegpt-cloud` is listed
...
Important: `opencode mcp list` starts a fresh process. Do not claim that it attaches to or tests the OpenCode server already running inside OfflineGPT. Describe it only as a fresh-process probe using the same binary and generated config.

## Phase 4 — Classify the result

- Fresh probe omits `offlinegpt-cloud`: the generated config is missing, the wrong runtime config was selected, or this is a secondary workspace whose MCPs are registered dynamically.
...
**Verify inside OfflineGPT**

1. In the affected workspace, open **Settings → Library** and click **Refresh**.
2. Click **Show hidden**, then find **OfflineGPT Cloud Control** under **Your apps**. This row shows live OpenCode MCP status; the catalog card only proves that the entry is configured.
...
| Fresh `opencode mcp list` | OfflineGPT status / forced call | Likely boundary |
| --- | --- | --- |
