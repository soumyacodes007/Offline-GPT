...
2. Inspect these files if they exist (they can silently override server URLs beneath the Settings UI), but report only `baseUrl`, `apiBaseUrl`, `requireSignin`, and `writtenAt`. Never print `handoff`, `claimLinks`, grants, tokens, or other credential-bearing fields:
   ...
   - `~/Library/Application Support/com.differentai.offlinegpt/desktop-bootstrap.json`
...
If the agent completes all phases and verification still fails, [open an issue](https://github.com/different-ai/offlinegpt/issues) and paste the agent's final report (it is already redacted).
