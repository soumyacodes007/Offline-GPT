...
description: "Run OfflineGPT when laptops have internet and VPN access while Den stays inside a private network."
---

Most enterprise OfflineGPT installs are not fully air-gapped. The common shape is:
...
External MCP clients such as Cursor, Claude Code, VS Code, ChatGPT, and OpenCode are separate from the desktop. They must reach the MCP endpoint you configure for them, usually `<DEN_API_PUBLIC_URL>/mcp/agent`. See [OfflineGPT Connect MCP](/cloud/run-in-the-cloud/cloud-mcp) for the external-client flow.
...
- The OfflineGPT chart and images at deploy or image-pull time, unless you mirror the Helm chart and enabled images into an internal registry.
...
**Why:** `DEN_DIAGNOSTICS_ORIGIN` defaults to the public OfflineGPT diagnostic service.
...
| Cloud MCP works in a real chat | In a chat, the agent can search available OfflineGPT capabilities and execute one that the user is allowed to run. | `Settings → Connect` for provider sign-in or admin setup. Use `Settings → Debug` to interpret diagnostics, especially a skipped catalog probe. |
