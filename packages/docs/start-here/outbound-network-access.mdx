...
description: "Customer IT allowlist guidance for OfflineGPT desktop, Den, deployment artifacts, and opt-in providers."
---

OfflineGPT works best when customer IT approves outbound access by component. The canonical machine-readable source is [`docs/enterprise/outbound-access.json`](https://github.com/soumyacodes007/offline-gpt/blob/dev/docs/enterprise/outbound-access.json). CI guards that file with `node scripts/check-outbound-access.mjs`; this page is the published human-readable summary.

## Minimum hosted desktop allowlist

For a practical OfflineGPT Cloud desktop install, allow TCP 443 to these hosts:

```text
app.offlinegptlabs.com
api.app.offlinegptlabs.com
github.com
release-assets.githubusercontent.com
objects.githubusercontent.com
registry.npmjs.org
models.offlinegptlabs.com
```
...
- `registry.npmjs.org` is used when packaged desktop builds start `npx -y offlinegpt-ui-mcp`. If it is blocked, the desktop can still open, but UI-control MCP is unavailable.
- `models.offlinegptlabs.com` is the default OpenCode model catalog. Set `OPENCODE_MODELS_URL` to an approved internal mirror if the public catalog is blocked.

## Desktop and local runtime destinations

| Host | Requirement | Why it is reached | What breaks when blocked |
| --- | --- | --- | --- |
| `app.offlinegptlabs.com` | Required for OfflineGPT Cloud | Hosted Cloud web origin. The desktop uses one base URL and derives Den API/MCP calls through `/api/den/...`. | Cloud sign-in, org settings, Collections, and remote workspace flows cannot load. |
| `api.app.offlinegptlabs.com` | Required for hosted install and OfflineGPT Connect | Hosted public API and OfflineGPT Connect MCP endpoint used by install and external-client flows. | Hosted install checks and public OfflineGPT Connect fail. |
| `app.offlinegpt.software`, `api.offlinegpt.software` | Required only for legacy hosted configurations | Older desktop configurations may still derive Cloud and OfflineGPT Connect traffic from these legacy hosted origins. | Legacy configurations cannot reconcile OfflineGPT Connect or native MCP Apps until moved to the current hosted origins. |
| Your self-hosted Den web origin | Required for self-hosted desktop | Replaces `app.offlinegptlabs.com` for signed-in desktop traffic. In the single-origin shape, API and MCP use `/api/den/...` on the same origin. | The desktop cannot sign in or reach Den. |
...
| `registry.npmjs.org` | Required in practice unless npm is mirrored | `npx` package resolution for `offlinegpt-ui-mcp` and development-only helper commands. | OfflineGPT UI-control MCP is unavailable. |
| `models.offlinegptlabs.com` | Required in practice unless mirrored | Default OpenCode model catalog passed as `OPENCODE_MODELS_URL`. | Model selection can be stale, incomplete, or unavailable. |
...
| `offlinegptlabs.com`, `offlinegpt.dev`, `discord.gg`, `ollama.com`, `opencode.ai`, `linear.app`, `mail.google.com` | Optional or link-only | Docs, feedback, community, provider help, local model links, sample links, and generated Gmail links. | Links do not open; core app behavior is unaffected. |

`code.claude.com` and `schemas.agentskills.io` appear in source or schema metadata. They are findable in the manifest, but OfflineGPT does not fetch them for normal runtime behavior.
...
| Host | Requirement | Why it is reached | What breaks when blocked |
| --- | --- | --- | --- |
| `ghcr.io` | Required unless mirrored | OfflineGPT Helm chart and container images. | Kubernetes cannot pull the chart or `offlinegpt-den-api`, `offlinegpt-den-web`, and optional `offlinegpt-inference` images. |
