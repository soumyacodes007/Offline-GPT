...
description: "Diagnose DNS, TLS, proxy, catalog, and Den outbound issues in locked-down OfflineGPT deployments."
---

Use this page when OfflineGPT reaches the wrong server, cannot complete TLS, is blocked by a proxy, or needs proof that Den can reach an approved diagnostic origin. For non-network runtime, MCP, and stale-tool problems, use [Diagnostic prompts](/start-here/troubleshooting/diagnostic-prompts).
...
If raw download is blocked, copy `scripts/support/offlinegpt-doctor.ps1` to the machine through your approved offline channel and run:
...
| --- | --- | --- |
| `LIKELY TLS INTERCEPTION` | The leaf issuer is an internal or proxy CA instead of the expected public issuer. | Allowlist the OfflineGPT hosts through the proxy, or install the enterprise root in every OfflineGPT trust surface that needs it. |
...
Settings → Debug includes a cloud catalog diagnostic that probes the managed `offlinegpt-cloud` MCP entry and verifies that Cloud exposes exactly `search_capabilities` and `execute_capability`.

For self-hosted Den origins, set `OFFLINEGPT_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS` before OfflineGPT launches:

```bash
OFFLINEGPT_AGENT_DIAGNOSTICS_TRUSTED_ORIGINS=https://offlinegpt.example.com
```
...
- The default diagnostics origin is `https://diagnostic.offlinegptlabs.com`.
