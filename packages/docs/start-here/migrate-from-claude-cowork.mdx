...
description: "Move your files, skills, plugins, MCP servers, and team setup to OfflineGPT."
---

You do not need to rebuild your setup to move from Claude Cowork to OfflineGPT. Start with one real task, bring over the capabilities it needs, and keep Cowork available until that task works end to end.

## What carries over

| From Claude Cowork | In OfflineGPT | What to do |
| --- | --- | --- |
| Local files and folders | Workspace files | Open the same folder or repository as an OfflineGPT workspace. |
| `SKILL.md` skills | Skills | Copy the skill folder into your workspace or install it as a plugin. |
| Claude-compatible plugins | Plugins | Import the plugin or marketplace manifest; OfflineGPT normalizes its skills, commands, and MCP dependencies. |
| MCP servers | Connectors and custom apps | Reconnect the server through OfflineGPT Connect or add its URL as a custom app. |
| Claude models | Anthropic provider | Add an Anthropic API key, or provision Anthropic centrally through OfflineGPT Cloud. |
...
## 1. Install OfflineGPT and open the same work

1. [Download OfflineGPT](https://offlinegptlabs.com/download) for macOS, Windows, or Linux.
2. Create a workspace from the folder or repository you used with Cowork.
3. Ask OfflineGPT to summarize the workspace before changing anything. This confirms it can read the files you expect.

Your files stay where they already are. OfflineGPT does not require you to upload a duplicate copy before you can work with them.

## 2. Choose your models

OfflineGPT is model-agnostic. You can keep using Anthropic models, add another provider, or let your organization provision models centrally.
...
OfflineGPT uses the same `SKILL.md` format. Copy a skill directory into the workspace, or import the plugin that contains it.
...
See [Connectors, skills, and plugins](/start-here/do-work-with-it/skills-plugins-and-mcp) for how OfflineGPT organizes these pieces. For team distribution, see [Publish and copy a skill](/start-here/do-work-with-it/publish-and-copy-a-skill).

## 4. Reconnect apps and MCP servers

Credentials do not move between products. Reconnect each service so OfflineGPT receives its own authorized access.
...
Do not ask every teammate to repeat your local setup by hand. In OfflineGPT Cloud, publish approved skills and connections once, assign them to the right teams, and apply desktop policies centrally.
...
- [ ] Cowork remains available until the representative workflow passes in OfflineGPT.

Once the checklist passes, make OfflineGPT the default for that workflow and migrate the next one.
