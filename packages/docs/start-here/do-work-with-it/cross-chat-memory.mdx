...
description: "How OfflineGPT can answer questions about past sessions."
---

OfflineGPT's cross-chat memory currently comes from saved session history. It is not a separate long-term memory database, and it does not automatically inject every old chat into every new prompt.

When an agent is running inside the OfflineGPT app, it can use OfflineGPT's semantic UI actions to look up past chats:
...
Because this uses the OfflineGPT UI, the agent may briefly navigate away from your current session to inspect another one. You can ask it to return to the current session afterward.

## Current limits

- Session lookup is based on the sessions OfflineGPT has loaded for the current app and workspace context.
...
- The agent can only read sessions available to the current OfflineGPT app and user.

## How to ask

Give the agent whatever clue you have: a session ID, title, person name, project name, workspace, or rough topic. For example: `Use the OfflineGPT app UI to find the customer migration chat and tell me what I said about customer references.`
