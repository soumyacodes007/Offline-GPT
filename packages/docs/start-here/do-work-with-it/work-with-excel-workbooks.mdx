...
description: "Attach, read, and produce .xlsx workbooks with the agent without leaving OfflineGPT."
---

OfflineGPT treats Excel workbooks as first-class files. The agent reads any sheet of a workbook in your workspace, and it writes new workbooks that open in Excel, Numbers, Google Sheets, and the OfflineGPT artifact preview.

## Attach a workbook

1. Drop a `.xlsx` file into the composer or reference one already in the workspace.
2. OfflineGPT keeps the original bytes out of the model request. The file is copied under `.opencode/offlinegpt/inbox/chat-attachments/` and the agent receives a compact preview instead: every sheet name, its used range and formula count, and the first rows of each sheet as a grid with real row numbers and column letters.
