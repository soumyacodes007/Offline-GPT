---
title: "Self-host"
description: "Run OfflineGPT on your own server or infrastructure."
---

Most users just use the [desktop app](https://offlinegptlabs.com/download). Self-hosting is for teams that want OfflineGPT to run on their own server, VPC, or private cloud and then connect the desktop app to it remotely.

Want hosted workspaces, team Collections, or shared providers without running infra? Use [OfflineGPT Cloud](/cloud/get-started) instead. Rolling this out across a company? [Book a call](https://calendar.app.google/86QpCENvhfEzDFLu5) about [Enterprise](/cloud/enterprise).
...
OfflineGPT is split into normal services:

- **OfflineGPT app**: the desktop or web client people use.
...
- **Inference service**: optional OfflineGPT Models proxy and metering service.

If you only need one private remote workspace, deploy the worker runtime. If you want OfflineGPT Cloud-style accounts, teams, and organization management, deploy Den web, Den controller, and optionally the inference service.
...
The chart pulls the matching GHCR images for `offlinegpt-den-api`, `offlinegpt-den-web`, and optional `offlinegpt-inference` unless you mirror them into an internal registry.
...
For containerized services, start from the [Docker packaging](https://github.com/soumyacodes007/offline-gpt/tree/dev/packaging/docker) in the repo.
...
- Hosted OfflineGPT uses PlanetScale for Den control-plane state. PlanetScale production clusters provide one primary and at least two replicas across three availability zones, managed failover, and automated backups.
...
- Keep the local Docker MySQL URL (`mysql://root:password@127.0.0.1:3306/offlinegpt_den`) for development only; it is not a production security posture.
- Set a unique `DEN_DB_ENCRYPTION_KEY` of at least 32 characters. OfflineGPT uses it to encrypt selected sensitive database columns, but it does not replace infrastructure-level encryption at rest.
...
Private deployments should keep public signup disabled and use the one-time [first-administrator setup flow](/self-host/deploy-to-your-cloud/first-administrator). Configuring an owner or platform-admin email does not create an account or password, and OfflineGPT does not ship a default administrator password.
...
If you do not deploy Den, worker access is token-based with `OFFLINEGPT_TOKEN` and `OFFLINEGPT_HOST_TOKEN`.

## Minimal working stack

### OfflineGPT Cloud and the desktop app

OfflineGPT cloud-style self-hosting is composed of three main pieces:
...
OfflineGPT does not require many third-party services for the core runtime. These are optional or deployment-specific:
...
Skip the server. [**OfflineGPT Cloud**](/cloud/get-started) runs hosted workers for your team.

Cloud also gives you org-wide primitives you'd otherwise have to build: [Collections](/cloud/share-with-your-team/collections), [shared LLM providers](/cloud/share-with-your-team/managed-llm-provider), [RBAC](/cloud/members-and-rbac), and [OfflineGPT Web](/cloud/run-in-the-cloud/shared-workspace).

## Looking for support for a larger rollout?

If you're rolling OfflineGPT out across a company of 150\+ employees, a single self-hosted server VM might be harder to manage.
