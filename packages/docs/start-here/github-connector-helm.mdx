...
description: "Create a GitHub App and configure the OfflineGPT EE Helm chart for repository connectors."
---

The GitHub connector lets a self-hosted OfflineGPT deployment install a GitHub App, list repositories the installation can read, discover plugin or marketplace manifests, import selected repositories, and keep imported content fresh from GitHub webhooks.
...
config:
  public:
    ...
    apiOrigin: "https://api.offlinegpt.example.com"
```

The GitHub App needs these OfflineGPT URLs:

| GitHub App field | Value |
| --- | --- |
| Homepage URL | `https://offlinegpt.example.com` |
...
## What OfflineGPT does with it

When an admin clicks **Connect GitHub** in OfflineGPT:
...
```text
https://offlinegpt.example.com/dashboard/integrations
```

Choose GitHub, install the GitHub App on a test account or organization, select a repository, and start discovery. A working setup should return to OfflineGPT, show repositories visible to the installation, and allow discovery on a selected repository.
