...
Shared UI primitives for OfflineGPT apps.

This package ships one entrypoint: `@offlinegpt/ui/react`, used by `apps/app`
...
The first shared components live under the `paper` namespace and wrap Paper Design shaders with OfflineGPT-specific defaults and deterministic seed support.
...
1. OfflineGPT defaults
2. Seed-derived values from `seed`
3. Explicit props passed by the caller

## Roadmap component

`OfflineGPTRoadmap` is the shared visual roadmap used by the landing and docs
routes. Its typed sections are exported as `roadmapSections` so other OfflineGPT
