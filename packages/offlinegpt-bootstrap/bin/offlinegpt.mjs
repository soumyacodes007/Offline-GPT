...
// bootstrap actions from other OfflineGPT commands a user may already have.
const COMMAND_NAME = "offlinegpt-bootstrap"
const DEFAULT_OFFLINEGPT_MARKETPLACE_NAME = "OfflineGPT Marketplace"
...
function printHelp() {
  console.log([
    "offlinegpt-bootstrap",
    "",
    "Usage:",
    "  offlinegpt-bootstrap install [--bin-dir <path>] [--install-dir <path>] [--source <path>] [--json]",
    "  offlinegpt-bootstrap install app --manifest <url-or-file> [--app-dir <path>] [--json]",
    "  offlinegpt-bootstrap doctor [--bin-dir <path>] [--install-dir <path>] [--base-url <url>] [--desktop-bootstrap] [--json]",
    "  OFFLINEGPT_OWNER_PASSWORD=<password> offlinegpt-bootstrap cloud onboard --base-url <url> --owner-email <email> --org-name <name> --invite-email <email> [--skill-name <name>] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  offlinegpt-bootstrap cloud bootstrap-workspace --base-url <url> --workspace-name <name> [--skill-name <name>] [--owner-email <email>] [--teammate-emails a@x.com,b@y.com] [--claim-roles owner,member] [--web-base-url <url>] [--prepare-desktop] [--json]",
    "  offlinegpt-bootstrap cloud claim-link [--role owner] [--desktop-bootstrap-path <path>] [--json]",
    "",
    "Commands:",
    "  install          Install the offlinegpt-bootstrap CLI into a user bin dir",
    ...
    "                   links). Defaults to https://app.offlinegptlabs.com when",
    "                   --base-url is the hosted API (api.offlinegptlabs.com);",
...
function defaultInstallDir() {
  return process.env.OFFLINEGPT_INSTALL_DIR || join(process.env.HOME || process.cwd(), ".offlinegpt", "bootstrap")
}

function defaultBinDir() {
  return process.env.OFFLINEGPT_BIN_DIR || join(process.env.HOME || process.cwd(), ".local", "bin")
}

function defaultAppDir() {
  return process.env.OFFLINEGPT_APP_DIR || (process.platform === "darwin"
    ? join(process.env.HOME || process.cwd(), "Applications")
    : process.platform === "win32"
      ? join(process.env.LOCALAPPDATA || join(process.env.HOME || process.cwd(), "AppData", "Local"), "OfflineGPT")
      : join(process.env.HOME || process.cwd(), ".local", "share", "offlinegpt"))
}
...
function defaultDesktopBootstrapPath() {
  return process.env.OFFLINEGPT_DESKTOP_BOOTSTRAP_PATH || join(configHomeDir(), "offlinegpt", "desktop-bootstrap.json")
}

function defaultSkillsDir() {
  return process.env.OFFLINEGPT_SKILLS_DIR || join(configHomeDir(), "opencode", "skills")
}

function defaultDeviceKeyPath() {
  return process.env.OFFLINEGPT_DEVICE_KEY_PATH || join(configHomeDir(), "offlinegpt", "bootstrap-device-key.json")
}
...
// browser opens `https://api.offlinegptlabs.com/?mode=sign-in...` and shows raw
...
function deriveWebBaseUrl(apiBaseUrl) {
  try {
    ...
    if (url.hostname === "api.offlinegptlabs.com") {
      return "https://app.offlinegptlabs.com"
    }
...
function slugifySkillName(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "offlinegpt-bootstrap-skill"
}
...
function runInstall(args) {
  ...
  const installedCli = join(installDir, "offlinegpt.mjs")
  ...
  jsonOut({ ok: true, message: `OfflineGPT CLI installed at ${executable}`, install: manifest }, json)
}
...
function defaultInstalledName(type, manifest, artifact) {
  ...
  if (type === "dmg") return "OfflineGPT.app"
  if (type === "appimage") return "OfflineGPT.AppImage"
  if (type === "exe") return "OfflineGPT.exe"
  if (type === "msi") return "OfflineGPT.msi"
  if (process.platform === "darwin") return "OfflineGPT.app"
  if (process.platform === "win32") return "OfflineGPT.exe"
  return "offlinegpt"
}
...
function installDmg(input) {
  ...
  try {
    ...
    mounted = true
    const appName = input.appName || "OfflineGPT.app"
...
async function runInstallApp(args) {
  const json = hasFlag(args.flags, "json")
  const manifestLocation = getFlag(args.flags, "manifest") || process.env.OFFLINEGPT_INSTALL_MANIFEST
  ...
  const workDir = join(tmpdir(), `offlinegpt-app-install-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  try {
    ...
    const artifactPath = join(workDir, artifact.fileName || "OfflineGPT.dmg")
    ...
    writeFileSync(join(appDir, "offlinegpt-app-install.json"), JSON.stringify(install, null, 2))
    jsonOut({ ok: true, message: `OfflineGPT app installed at ${appPath}`, install }, json)
  } finally {
...
async function runDoctor(args) {
  ...
  checks.push({ name: "offlinegptExecutable", ok: executableOk, value: executable })
  ...
  if (hasFlag(args.flags, "app") || args.flags.has("app-dir")) {
    const appManifest = join(appDir, "offlinegpt-app-install.json")
    let appPath = process.platform === "darwin"
      ? join(appDir, "OfflineGPT.app")
      : process.platform === "win32"
        ? join(appDir, "OfflineGPT.exe")
        : join(appDir, "offlinegpt")
    ...
    checks.push({ name: "offlinegptApp", ok: existsSync(appPath), value: appPath })
  ...
  jsonOut({ ok, message: ok ? "OfflineGPT doctor: ok" : "OfflineGPT doctor: failed", version: VERSION, manifest, checks }, json)
...
function skillText(name, output) {
  return `---\nname: ${name}\ndescription: Starter skill created by offlinegpt bootstrap.\nofflinegptBootstrapTrigger: bootstrap.verify\nofflinegptBootstrapOutput: ${JSON.stringify(output)}\n---\n\n# ${name}\n\nWhen triggered with \`bootstrap.verify\`, output exactly:\n\n\`${output}\`\n\nUse this skill to confirm OfflineGPT cloud onboarding can create and trigger a deterministic skill.`
}
...
async function createCloudSkillPlugin(baseUrl, auth, input) {
  ...
  const marketplace = marketplaces.body.items.find((item) => item?.name === DEFAULT_OFFLINEGPT_MARKETPLACE_NAME) || marketplaces.body.items[0]
  if (!marketplace?.id) {
...
function runBootstrapSkill(skill, input) {
  const trigger = readFrontmatterValue(skill.skillText, "offlinegptBootstrapTrigger")
  const output = readFrontmatterValue(skill.skillText, "offlinegptBootstrapOutput")
    ...
    ...(input.claimLinks
      ? {
          claimLinks: input.claimLinks.map((link) => ({
            ...
            url: `redacted: run "offlinegpt-bootstrap cloud claim-link --role ${link.role}" to view`,
          })),
        }
      : {}),
  }
...
async function resolveOwnerPassword(flags) {
  ...
  const envName = getFlag(flags, "owner-password-env", "OFFLINEGPT_OWNER_PASSWORD")
...
async function runCloudOnboard(args) {
  ...
  const skillName = getFlag(args.flags, "skill-name", "First OfflineGPT Skill")
  const skillOutput = getFlag(args.flags, "skill-output", "OFFLINEGPT_BOOTSTRAP_SKILL_TRIGGERED")
  ...
  jsonOut({
    ok: true,
    message: "OfflineGPT cloud onboarding complete",
...
async function runCloudBootstrapWorkspace(args) {
  const json = hasFlag(args.flags, "json")
  const baseUrl = getFlag(args.flags, "base-url", "https://api.offlinegptlabs.com")?.replace(/\/$/, "")
  ...
  const skill = {
    ...response.body.skill,
    skillText: skillText(response.body.skill.title, response.body.skill.output || "OFFLINEGPT_BOOTSTRAP_SKILL_TRIGGERED"),
  }

  const skillRun = runBootstrapSkill(skill, { trigger: "bootstrap.verify" })
  if (!skillRun.triggered || skillRun.output !== "OFFLINEGPT_BOOTSTRAP_SKILL_TRIGGERED") {
    throw new Error(`skill_trigger_failed: ${JSON.stringify(skillRun)}`)
  }
  ...
  jsonOut({
    ok: true,
    message: "OfflineGPT workspace bootstrap complete",
    organization: response.body.organization,
    ...
    claimLinks: response.body.claimLinks.map((link) => ({
      ...
      expiresAt: link.expiresAt,
      url: prepareDesktop
        ? `redacted: run "offlinegpt-bootstrap cloud claim-link --role ${link.role}" to view`
