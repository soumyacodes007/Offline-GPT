...
const cli = join(root, "bin", "offlinegpt.mjs")
const temp = mkdtempSync(join(tmpdir(), "offlinegpt-bootstrap-test-"))
...
try {
  ...
  const executableName = process.platform === "win32" ? "offlinegpt-bootstrap.cmd" : "offlinegpt-bootstrap"
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      apiBaseUrl: "https://api.offlinegptlabs.com",
      requireSignin: false,
      prepared: { orgId: "org_test", orgName: "Test Org", skillId: "cob_test", skillTitle: "Test Skill", skillPath: "/tmp/skill.md" },
      claimLinks: [
        { id: "wcl_owner", role: "owner", token: "test-owner-token", url: "https://app.offlinegptlabs.com/workspace-claim?token=test-owner-token", expiresAt: "2030-01-01T00:00:00.000Z" },
        { id: "wcl_member", role: "member", token: "test-member-token", url: "https://app.offlinegptlabs.com/workspace-claim?token=test-member-token", expiresAt: "2030-01-01T00:00:00.000Z" },
      ],
    }),
    "utf8",
  )
  ...
  assert.equal(claimLinkByRoleJson.claimLinks[0].url, "https://app.offlinegptlabs.com/workspace-claim?token=test-owner-token")
  ...
  await withStubDenApi(
    () => ({
      ...
      skill: { id: "cob_test", title: "First OfflineGPT Skill", output: "OFFLINEGPT_BOOTSTRAP_SKILL_TRIGGERED" },
  ...
  // and `apiBaseUrl`, so a normal `--base-url https://api.offlinegptlabs.com`
  // run broke the desktop app's Sign In flow (it opened
  // `https://api.offlinegptlabs.com/?mode=sign-in...` and showed raw API JSON).
  await withStubDenApi(
    () => ({
      ok: true,
      organization: { id: "org_test", name: "Stub Org", slug: "org_test", status: "provisional" },
    ...
    async (baseUrl) => {
      ...
      writeFileSync(
        ...
        `${JSON.stringify({ brandAppName: "OfflineGPT Demo A" })}\n`,
        "utf8",
      )
      ...
      assert.equal(writtenExplicit.brandAppName, "OfflineGPT Demo A", "prepared desktop setup must preserve its profile identity")
    },
  )
