...
You are an agent helping a user install and set up OfflineGPT.

Goal: install OfflineGPT, create a provisional workspace without requiring email
...
offer to connect the OfflineGPT MCP to the agent the user is already using.

> The bootstrap command is `offlinegpt-bootstrap`. Use that exact command for
> these setup steps so the bootstrap flow stays distinct from other OfflineGPT
> tooling a user may already have on their PATH.

## 1. Install CLI

Install the `offlinegpt-bootstrap` CLI into a user-writable bin dir.

If you already have this repository locally:

```bash
node packages/offlinegpt-bootstrap/bin/offlinegpt.mjs install \
  --install-dir "$HOME/.offlinegpt/bootstrap" \
...
`offlinegpt-bootstrap` CLI (a single dependency-free Node file) and installs it
...
```bash
offlinegpt-bootstrap install app \
  --manifest https://offlinegptlabs.com/install-manifest.json \
  --json
```
...
```bash
offlinegpt-bootstrap cloud bootstrap-workspace \
  --base-url https://api.offlinegptlabs.com \
  --workspace-name "<workspace-name>" \
  --skill-name "First OfflineGPT Skill" \
  --claim-roles owner \
  --prepare-desktop \
...
1. Confirm in one line that OfflineGPT is installed and their workspace is ready
   (use the workspace name).
2. Point them at ONE concrete first task they can run right now, e.g. "OfflineGPT
...
4. End with this single question: "Want me to connect the OfflineGPT MCP to this
...
1. A provisional workspace must be claimed before its owner can authenticate
   the OfflineGPT MCP. If it has not been claimed yet, explain that dependency and
...
3. For Codex, run:
   ...
   If an `offlinegpt` entry already exists, do not add a duplicate. Authenticate
   the existing entry instead. To switch organizations or recover stale auth,
   run `codex mcp logout offlinegpt` before `codex mcp login offlinegpt`.
4. For another agent, use its current instructions from
   `https://offlinegptlabs.com/docs/cloud/run-in-the-cloud/cloud-mcp`; do not guess
...
5. Tell the user to restart or reopen the current agent after setup so the new
   ...
   client can see OfflineGPT's `search_capabilities` and `execute_capability`
   tools.
...
```bash
offlinegpt-bootstrap cloud claim-link --role owner --json
```
...
- `offlinegpt-bootstrap doctor --json` returns `ok: true`
- `offlinegpt-bootstrap doctor --app --json` returns `ok: true`
- `offlinegpt-bootstrap cloud bootstrap-workspace ... --json` returns:
  - `ok: true`
  - `organization.id` is present
  - `setup.id` is present
  - `skill.id` is present
  - `skillRun.triggered` is `true`
  - `skillRun.output` is `OFFLINEGPT_BOOTSTRAP_SKILL_TRIGGERED`
  - `claimLinks[0].id` is present
  - `desktop.prepared` is `true`
  - `desktop.bootstrapPath` is present
  - `desktop.skillPath` is present
- `offlinegpt-bootstrap doctor --desktop-bootstrap --json` returns `ok: true`
...
- If the `offlinegpt-bootstrap` command is not found after install: ensure
  `$HOME/.local/bin` is on PATH, or call the binary by its full path
  (`$HOME/.local/bin/offlinegpt-bootstrap`).
- If app install fails: run `offlinegpt-bootstrap doctor --app --json` and report failed checks.
