// Live E2E test evidence for the script-installable OfflineGPT bootstrap CLI.
//
...
//   DEN_API_E2E_BASE_URL=... pnpm exec node evals/agent-test-evidence-offlinegpt-bootstrap.mjs
...
const temp = mkdtempSync(join(tmpdir(), "offlinegpt-bootstrap-e2e-"))
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const installedOfflineGpt = join(binDir, process.platform === "win32" ? "offlinegpt-bootstrap.cmd" : "offlinegpt-bootstrap")
const outDir = join(repoRoot, "evals", "results", "offlinegpt-bootstrap-cli")
...
try {
  ...
  prove("A bootstrap script can install the offlinegpt CLI", {
  ...
  }, install.status === 0 && install.json?.ok === true && install.json?.install?.executable === installedOfflineGpt)

  const doctor = run(installedOfflineGpt, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--base-url", baseUrl, "--json"])
  prove("The installed CLI can doctor itself and the Den API", {
    action: "offlinegpt doctor --base-url <live-den-api> --json",
  ...
  const onboard = run(installedOfflineGpt, [
    "cloud",
    "onboard",
    "--base-url",
    baseUrl,
    ...
    skillName,
    "--json",
  ], { env: { ...process.env, OFFLINEGPT_OWNER_PASSWORD: randomPassword() }, timeout: 30_000 })
  prove("The installed CLI can onboard a user, org, invite, and skill end-to-end", {
    action: "offlinegpt cloud onboard --base-url <live-den-api> --owner-email ... --org-name ... --invite-email ... --skill-name ... --json",
  ...
  }, onboard.status === 0 && onboard.json?.ok === true && onboard.json?.organization?.id && onboard.json?.invitation?.invitationId && onboard.json?.skill?.id && onboard.json?.skill?.title === skillName && onboard.json?.skillRun?.triggered === true && onboard.json?.skillRun?.output === "OFFLINEGPT_BOOTSTRAP_SKILL_TRIGGERED")
} finally {
...
<title>OfflineGPT Bootstrap CLI — live E2E test evidence</title>
...
<h1>OfflineGPT Bootstrap CLI — live E2E test evidence</h1>
