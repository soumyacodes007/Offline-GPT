// Linux E2E test evidence for OfflineGPT bootstrap without shipping to production.
//
// This runs inside a real Linux container and proves:
// - the bootstrap CLI installs on Linux,
// - `offlinegpt install app` installs a Linux tar.gz app artifact from a manifest,
// - `offlinegpt doctor --app` verifies the installed app,
// - `offlinegpt cloud onboard` can hit a live Den API from inside Linux.
//
// Required host setup:
//   DEN_API_E2E_BASE_URL=http://host.docker.internal:<den-api-port>
//
// Run from packages/offlinegpt-bootstrap:
...
const outDir = join(repoRoot, "evals", "results", "offlinegpt-linux-container")
...
const artifactPath = join(fixtureDir, "offlinegpt-linux.tar.gz")
...
function redactSecrets(value) {
  return value.replace(/OFFLINEGPT_OWNER_PASSWORD=[^\s]+/g, "OFFLINEGPT_OWNER_PASSWORD=[redacted]")
}
...
try {
  const appBinary = join(artifactRoot, "offlinegpt")
  writeFileSync(appBinary, "#!/usr/bin/env sh\necho OfflineGPT Linux fixture\n")
  chmodSync(appBinary, 0o755)
  execFileSync("tar", ["-czf", artifactPath, "-C", artifactRoot, "offlinegpt"])
  const digest = sha256(artifactPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-linux-fixture",
    artifacts: {
      linux: {
        x64: { type: "tar.gz", url: "file:///fixture/offlinegpt-linux.tar.gz", sha256: digest, appName: "offlinegpt" },
        arm64: { type: "tar.gz", url: "file:///fixture/offlinegpt-linux.tar.gz", sha256: digest, appName: "offlinegpt" },
      },
    },
  }, null, 2))
  prove("A Linux app artifact manifest is available without shipping to production", {
    action: "Create local offlinegpt tar.gz artifact and manifest with SHA-256",
  ...
  const installAndDoctor = dockerRun([
    "set -euo pipefail",
    "node /package/bin/offlinegpt.mjs install --install-dir /tmp/offlinegpt/install --bin-dir /tmp/offlinegpt/bin --json >/tmp/install.json",
    "/tmp/offlinegpt/bin/offlinegpt-bootstrap install app --manifest /fixture/install-manifest.json --app-dir /tmp/offlinegpt/apps --json >/tmp/app-install.json",
    "/tmp/offlinegpt/bin/offlinegpt-bootstrap doctor --install-dir /tmp/offlinegpt/install --bin-dir /tmp/offlinegpt/bin --app --app-dir /tmp/offlinegpt/apps --json",
  ].join(" && "), { timeout: 60_000 })
  prove("Linux can install the CLI and app artifact, then doctor the app", {
    action: "Run offlinegpt install, offlinegpt install app, and offlinegpt doctor --app inside Linux",
  ...
  const onboard = dockerRun([
    "set -euo pipefail",
    "node /package/bin/offlinegpt.mjs install --install-dir /tmp/offlinegpt/install --bin-dir /tmp/offlinegpt/bin --json >/tmp/install.json",
    `/tmp/offlinegpt/bin/offlinegpt-bootstrap cloud onboard --base-url "$DEN_API_E2E_BASE_URL" --owner-email ${JSON.stringify(ownerEmail)} --org-name ${JSON.stringify(`Linux CLI Org ${runId}`)} --invite-email ${JSON.stringify(inviteEmail)} --skill-name ${JSON.stringify(skillName)} --prepare-desktop --desktop-bootstrap-path /tmp/offlinegpt/desktop-bootstrap.json --skills-dir /tmp/offlinegpt/skills --json >/tmp/onboard.json`,
    "/tmp/offlinegpt/bin/offlinegpt-bootstrap doctor --install-dir /tmp/offlinegpt/install --bin-dir /tmp/offlinegpt/bin --desktop-bootstrap --desktop-bootstrap-path /tmp/offlinegpt/desktop-bootstrap.json --json >/tmp/doctor.json",
    ...
    env: ["-e", `OFFLINEGPT_OWNER_PASSWORD=${randomPassword()}`],
    timeout: 90_000,
  })
  prove("Linux installed CLI can complete live cloud onboarding", {
    action: "Run offlinegpt cloud onboard from inside Linux against live Den API",
    ...
    evidence: onboard,
  }, onboard.status === 0 && onboard.json?.onboard?.ok === true && onboard.json?.onboard?.organization?.id && onboard.json?.onboard?.invitation?.invitationId && onboard.json?.onboard?.skill?.id && onboard.json?.onboard?.skill?.title === skillName && onboard.json?.onboard?.skillRun?.triggered === true && onboard.json?.onboard?.skillRun?.output === "OFFLINEGPT_BOOTSTRAP_SKILL_TRIGGERED" && onboard.json?.onboard?.desktop?.prepared === true && onboard.json?.onboard?.desktop?.bootstrapPath && onboard.json?.onboard?.desktop?.skillPath && onboard.json?.doctor?.ok === true && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrap" && check.ok === true) && onboard.json?.doctor?.checks?.some((check) => check.name === "desktopBootstrapHandoff" && check.ok === true))
} finally {
...
<title>OfflineGPT Linux Bootstrap — test evidence</title>
...
</head><body><main><h1>OfflineGPT Linux Bootstrap — test evidence</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Live Den API: <code>${esc(denBaseUrl)}</code> · Screenshots: ${frames.length}</div>
...
writeFileSync(join(outDir, "report.json"), JSON.stringify({
  flow: "offlinegpt-linux-container",
