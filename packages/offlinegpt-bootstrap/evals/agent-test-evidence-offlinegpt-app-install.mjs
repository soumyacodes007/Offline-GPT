// Live local E2E test evidence for `offlinegpt install app` using a real macOS DMG.
//
// The flow creates a tiny OfflineGPT.app fixture, packages it as a .dmg, writes an
// install manifest with a SHA-256 digest, installs the bootstrap CLI into a temp
// bin dir, then runs the installed `offlinegpt install app` command against the
// manifest and verifies it with `offlinegpt doctor --app`.
...
if (process.platform !== "darwin") {
  throw new Error("offlinegpt install app DMG test evidence requires macOS")
}

const here = dirname(fileURLToPath(import.meta.url))
const packageRoot = resolve(here, "..")
const repoRoot = resolve(packageRoot, "..", "..")
const cli = join(packageRoot, "bin", "offlinegpt.mjs")
const outDir = join(repoRoot, "evals", "results", "offlinegpt-app-install-dmg")
const temp = join(tmpdir(), `offlinegpt-app-install-dmg-${Date.now()}-${Math.random().toString(36).slice(2)}`)
const sourceDir = join(temp, "source")
const appFixture = join(sourceDir, "OfflineGPT.app")
const installDir = join(temp, "install")
const binDir = join(temp, "bin")
const appDir = join(temp, "Applications")
const dmgPath = join(temp, "OfflineGPT.dmg")
const manifestPath = join(temp, "offlinegpt-install-manifest.json")
const installedOfflineGpt = join(binDir, "offlinegpt-bootstrap")
...
<key>CFBundleExecutable</key><string>OfflineGPT</string>
<key>CFBundleIdentifier</key><string>com.offlinegpt.fixture</string>
<key>CFBundleName</key><string>OfflineGPT</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>CFBundleShortVersionString</key><string>0.0.0-fixture</string>
</dict></plist>
`)
  writeFileSync(join(appFixture, "Contents", "MacOS", "OfflineGPT"), "#!/usr/bin/env sh\necho OfflineGPT fixture\n")
  chmodSync(join(appFixture, "Contents", "MacOS", "OfflineGPT"), 0o755)

  execFileSync("hdiutil", ["create", "-quiet", "-volname", "OfflineGPT", "-srcfolder", sourceDir, "-ov", "-format", "UDZO", dmgPath])
  const digest = sha256(dmgPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-fixture",
    appName: "OfflineGPT.app",
    artifacts: {
      darwin: {
        [process.arch]: {
          type: "dmg",
  ...
  prove("A real macOS DMG install manifest is available", {
    action: "Create OfflineGPT.app fixture, package it with hdiutil, and write manifest JSON",
  ...
  prove("The bootstrap CLI can be installed from a script", {
    action: "node bin/offlinegpt.mjs install --install-dir <tmp> --bin-dir <tmp>/bin --json",
    assert: "exit 0 and an offlinegpt executable exists",
    evidence: { status: installCli.status, body: installCli.json },
  }, installCli.status === 0 && existsSync(installedOfflineGpt))

  const installApp = run(installedOfflineGpt, ["install", "app", "--manifest", manifestPath, "--app-dir", appDir, "--json"], { timeout: 30_000 })
  const appPath = join(appDir, "OfflineGPT.app")
  prove("The installed CLI can download, verify, mount, and install OfflineGPT.app from a DMG", {
    action: "offlinegpt install app --manifest <fixture-manifest> --app-dir <tmp>/Applications --json",
    assert: "exit 0, checksum recorded, and OfflineGPT.app copied into app dir",
  ...
  const doctor = run(installedOfflineGpt, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--app", "--app-dir", appDir, "--json"])
  prove("doctor verifies the installed desktop app", {
    action: "offlinegpt doctor --app --app-dir <tmp>/Applications --json",
    assert: "exit 0 with offlinegptApp and appInstallManifest checks passing",
    evidence: { status: doctor.status, body: doctor.json, stderr: doctor.stderr },
  }, doctor.status === 0 && doctor.json?.ok === true && doctor.json?.checks?.some((check) => check.name === "offlinegptApp" && check.ok) && doctor.json?.checks?.some((check) => check.name === "appInstallManifest" && check.ok))
} finally {
...
<title>OfflineGPT App DMG Install — test evidence</title>
...
</head><body><main><h1>OfflineGPT App DMG Install — test evidence</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Screenshots: ${frames.length}</div>
...
writeFileSync(join(outDir, "report.json"), JSON.stringify({
  flow: "offlinegpt-app-install-dmg",
