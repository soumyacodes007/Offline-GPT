...
export const emailSubjects: { [Template in EmailTemplate]: (props: EmailTemplateProps[Template]) => string } = {
  verification: ({ verificationCode }) => `Your OfflineGPT verification code is ${verificationCode}`,
  passwordReset: () => "Reset your OfflineGPT password",
  organizationInvite: ({ organizationName }) => `You're invited to join ${organizationName} on OfflineGPT`,
  downloadLink: () => "Your OfflineGPT download link",
  feedback: ({ name, source }) => `OfflineGPT feedback from ${name}${source ? ` (${source})` : ""}`,
}
