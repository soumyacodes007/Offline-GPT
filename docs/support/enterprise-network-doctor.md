...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\offlinegpt-doctor.ps1 -WebUrl 'https://offlinegpt.example.com' -ApiUrl 'https://api.offlinegpt.example.com' -ExpectedIssuerMatch 'DigiCert'
```

Common outcome patterns:

- `LIKELY TLS INTERCEPTION`: the leaf issuer is an internal/proxy CA instead of DigiCert. The corporate proxy is re-signing TLS; allowlist OfflineGPT hosts or ensure the app runtime trusts that enterprise root.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\setup-offlinegpt-tls-repro.ps1 -Cleanup
```
