# Deploy OfflineGPT EE on Azure with AKS and Helm

Status: self-host operator guide
Related: `packaging/helm/offlinegpt-ee`, `packaging/helm/offlinegpt-ee/examples/values.azure-ingress.yaml`

This is the recommended Azure path for a first production-like OfflineGPT EE
...
add-on's managed NGINX Ingress controller so OfflineGPT gets stable HTTPS host
...
production workloads through November 2026. The current OfflineGPT chart emits
...
OfflineGPT. Azure Load Balancer is a layer 4 load balancer. It is useful for TCP
...
identities, and network security groups. The OfflineGPT Helm chart owns OfflineGPT
...
The OfflineGPT EE release artifact is already a Helm chart, and AKS gives the
...
not a different OfflineGPT packaging format.
...
export RESOURCE_GROUP=offlinegpt-ee-rg
export AKS_CLUSTER=offlinegpt-ee
export VNET_NAME=offlinegpt-ee-vnet
...
database. This is an Azure subscription/region eligibility issue, not OfflineGPT
quota usage.
...
subscription cannot use, not that OfflineGPT has consumed quota. Pick an allowed
...
- Region capacity varies. If AKS Automatic reports SKU/capacity failures, retry
  in a known-good region before debugging the OfflineGPT chart.
...
export MYSQL_ADMIN_USER=offlinegpt
...
indicate an Azure MySQL subscription/region eligibility issue before OfflineGPT or
...
```text
mysql://offlinegpt:<password>@<server>.mysql.database.azure.com:3306/offlinegpt_den?sslaccept=accept
```

Use `?sslaccept=accept` for the simple private-MySQL smoke path. This keeps TLS
on without requiring a cloud CA bundle to be mounted into the OfflineGPT image.
...
```bash
cp packaging/helm/offlinegpt-ee/examples/values.azure-ingress.yaml values.azure.yaml
```
...
helm template offlinegpt-ee oci://ghcr.io/different-ai/charts/offlinegpt-ee \
  ...
  -f values.azure.yaml > /tmp/offlinegpt-rendered.yaml
...
ingress:
  tls:
    - secretName: offlinegpt-ee-tls
```

Create that secret from a certificate that covers both OfflineGPT hosts:

```bash
kubectl create namespace offlinegpt-ee

kubectl create secret tls offlinegpt-ee-tls \
  --namespace offlinegpt-ee \
...
- `offlinegpt.example.com` -> the application routing public IP.
- `api.offlinegpt.example.com` -> the same application routing public IP.
...
**Identifier (Entity ID)** is the OfflineGPT auth origin and the certificate saved
in OfflineGPT is the active Entra token-signing certificate.
...
| Browser shows certificate warnings | TLS secret is missing, wrong, or does not cover both hosts | Recreate `offlinegpt-ee-tls` or configure the platform certificate automation |
