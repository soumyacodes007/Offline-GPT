...
Every plugin published to an org marketplace is automatically discoverable via `search_capabilities` and reachable via `execute_capability` on the existing `offlinegpt-cloud` connection — the same rail as External MCP Connections.
...
| `hook` | searchable metadata only | returns definition plus an unsupported hint | Hooks are not supported anywhere yet: `apps/server/src/claude-plugin-bundle.ts` warns that OfflineGPT does not support hooks, and local install skips loading them. |
...
Provision an External MCP Connection from a plugin's `mcp` spec; add nullable `sourcePluginId` linkage column as the first schema change; add inline connect cards; add desktop provenance / "Available via Cloud — no install needed" states in the Extensions UI; add search-side hints for already-installed dedupe; add one-line teaching in the OfflineGPT agent prompt and `/mcp/agent` tool descriptions; add command argument schemas.

### Phase 3: retire the requirement to copy

Add sandboxed `tool` execution via Den Worker Runtime; add `tools/searchText` caching if scale demands; add LOCAL rail parity so the local OfflineGPT server (`apps/server`) grows the same search/execute surface over locally-known catalogs for signed-out users; reposition copy-install as "pin locally / offline".
