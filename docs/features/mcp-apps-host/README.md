...
OfflineGPT Desktop can render the standard MCP Apps UI attached to a completed
...
2. OfflineGPT's bundled engine plugin preserves that call's standard `content`,
   `structuredContent`, and result `_meta` fields in the completed tool part.
3. The desktop asks its local OfflineGPT server to find the original projected
...
resources directly from command/stdio MCP entries. OfflineGPT-managed OAuth
