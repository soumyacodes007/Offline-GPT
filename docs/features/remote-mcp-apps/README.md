...
OfflineGPT supports MCP Apps delivered by standard MCP servers connected through
OfflineGPT Connect. A server advertises the stable
...
per-member credentials, and tool policy. The OfflineGPT Cloud control server
publishes a member-scoped resource at:

```text
offlinegpt://connect/mcp-servers/index.json
```
...
an audience or capability header. Desktop never writes `offlinegpt-connect-*`
...
central `offlinegpt-cloud` `search_capabilities` and `execute_capability` tools.
...
OfflineGPT access grants, disabled-tool policy, and approval rules still apply at
...
metadata (`kind: mcp_app`, `mcpApp.resourceUri`, `offlinegpt/mcpApp` meta) is an
...
disconnects stale
`offlinegpt-connect-*` OpenCode entries while preserving user-authored MCPs and
...
- no `ui://offlinegpt/library-apps/...` resources are registered;
