# Control OfflineGPT from any MCP client

OfflineGPT exposes its UI as an MCP server so any MCP-capable app can read what's on screen and run actions — no DOM scraping, no coordinates, no accessibility hacks.
...
OfflineGPT takes a different approach: the app itself tells you what actions are available, what the current state is, and lets you execute actions by name. The MCP server wraps that surface so any MCP client gets a first-class, semantic control experience out of the box.

This means:

- **HandsFree** can drive OfflineGPT sessions, composer, navigation, and transcript without guessing pixels.
- **OpenCode** can automate OfflineGPT as part of a larger coding workflow.
- **Claude Desktop, Codex, Cursor**, or any MCP-compatible tool can add OfflineGPT control with a single config line.
- Your own app can do the same.

> Want to control OfflineGPT Cloud workers and server APIs instead of the desktop UI? Check out the **OfflineGPT Cloud MCP** (separate package, coming soon).

## Quick start with HandsFree

HandsFree auto-discovers the OfflineGPT MCP server when both apps are running on the same machine. No config needed.

1. Launch **OfflineGPT** (desktop app).
2. Launch **HandsFree**.
3. Open the HandsFree connector panel — you should see **OfflineGPT** with a green "Connected" status and an action count.
...
- `ui_status` — check if OfflineGPT is running and the bridge is reachable.

### Cross-session memory

OfflineGPT's cross-session memory currently comes from saved session history exposed through the UI control surface. It is not a separate long-term memory database.
...
This may navigate OfflineGPT away from the user's current session while the lookup runs. If multiple sessions match, ask which one to inspect.

### OfflineGPT agents

Inside OfflineGPT, agents control the app through the semantic tools (`offlinegpt_context`, `offlinegpt_query`, `offlinegpt_execute`) using affordance ids from context. External MCP clients can also use the hidden **OfflineGPT UI Control** MCP via **Settings -> Extensions -> Show hidden**.

## Install

```bash
npm install -g offlinegpt-ui-mcp
```

Or run without installing:

```bash
npx offlinegpt-ui-mcp
```

> The package is [`offlinegpt-ui-mcp` on npm](https://www.npmjs.com/package/offlinegpt-ui-mcp).
...
```
> Use ui_snapshot to see what's on screen in OfflineGPT, then list the available sessions.
```
...
If you're building an app that speaks MCP, you can connect to the OfflineGPT UI server the same way:
...
// Check if OfflineGPT is running
...
Check if OfflineGPT is running and reachable. Returns connection status and app info.
...
Get the current OfflineGPT UI state: active route, narration, visible actions, and status. Call this before acting to understand what the user sees.
...
Execute an OfflineGPT UI action by its id.
...
The exact list depends on the current OfflineGPT route and state. Common actions include:
...
│  MCP client  │ ←────────────────→ │  offlinegpt-ui-mcp │ ←───────────────────→ │  OfflineGPT app │
...
1. OfflineGPT desktop starts a private localhost HTTP bridge on a random port, protected by a bearer token.
2. It writes a discovery file with the port and token so `offlinegpt-ui-mcp` can find it.
3. `offlinegpt-ui-mcp` reads the discovery file, proxies MCP tool calls to the bridge, and returns structured results.
4. The bridge calls `window.__offlinegptControl` inside the Electron renderer to snapshot state and execute actions.

The bridge and discovery file are implementation details — you never need to touch them directly. Just point your MCP client at `offlinegpt-ui-mcp`.
