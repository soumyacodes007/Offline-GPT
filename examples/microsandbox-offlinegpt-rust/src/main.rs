...
async fn main() -> Result<()> {
    let image = env::var("OFFLINEGPT_MICROSANDBOX_IMAGE")
        .unwrap_or_else(|_| "offlinegpt-microsandbox:dev".to_string());
    let name = env::var("OFFLINEGPT_MICROSANDBOX_NAME")
        .unwrap_or_else(|_| "offlinegpt-microsandbox-rust".to_string());
    let workspace_dir = env::var("OFFLINEGPT_MICROSANDBOX_WORKSPACE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| default_bind_dir(&name, "workspace"));
    let data_dir = env::var("OFFLINEGPT_MICROSANDBOX_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| default_bind_dir(&name, "data"));
    let replace = env_flag("OFFLINEGPT_MICROSANDBOX_REPLACE");
    let host_port = env::var("OFFLINEGPT_MICROSANDBOX_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(8787);
    let connect_host =
        env::var("OFFLINEGPT_CONNECT_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
    let client_token =
        env::var("OFFLINEGPT_TOKEN").unwrap_or_else(|_| "microsandbox-token".to_string());
    let host_token =
        env::var("OFFLINEGPT_HOST_TOKEN").unwrap_or_else(|_| "microsandbox-host-token".to_string());
    ...
    let mut builder = Sandbox::builder(&name)
        ...
        .memory(2048)
        .cpus(2)
        .env("OFFLINEGPT_CONNECT_HOST", &connect_host)
        .env("OFFLINEGPT_TOKEN", &client_token)
        .env("OFFLINEGPT_HOST_TOKEN", &host_token)
        .env("OFFLINEGPT_APPROVAL_MODE", "auto")
        .port(host_port, 8787)
        .volume("/workspace", |v| {
    ...
    let server = sandbox
        ...
        .await
        .context("failed to start the OfflineGPT microsandbox entrypoint inside the VM")?;
...
mod tests {
    ...
    async fn rust_example_smoke_test_checks_health_and_session_endpoints() -> Result<()> {
        let image = env::var("OFFLINEGPT_MICROSANDBOX_IMAGE")
            .unwrap_or_else(|_| "ttl.sh/offlinegpt-microsandbox-11559:1d".to_string());
