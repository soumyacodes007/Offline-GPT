...
**Base:** `dev` (`9f4425725`)  
**PR:** none yet. Open from https://github.com/different-ai/offlinegpt/pull/new/feat/library-composer-connections

```bash
git fetch origin feat/library-composer-connections
git checkout feat/library-composer-connections
# or: git worktree add ../offlinegpt-library-composer origin/feat/library-composer-connections
```
...
- Model select and model picker no longer show the OfflineGPT Models subscribe promo (composer Configure still goes to `/settings/ai` for providers).
