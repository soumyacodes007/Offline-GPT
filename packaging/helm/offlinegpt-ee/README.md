# OfflineGPT EE Helm Chart

Initial Helm chart for the OfflineGPT EE Den stack:
...
```yaml
image:
  tag: "REPLACE_OFFLINEGPT_VERSION"

config:
  tenancy:
    # Default chart behavior is single-org for private/self-hosted installs.
    # Hosted OfflineGPT Cloud should set this to "multi_org" explicitly.
    mode: "single_org"
  ...
  public:
    ...
    webOrigin: "https://offlinegpt.example.com"
    ...
    apiOrigin: ""
    mcpResourceUrl: ""
    mcpClaimNamespace: "https://offlinegpt.example.com"
    desktopDenBaseUrl: ""
    corsOrigins: ""
    ...
    authCallbackUrl: "https://offlinegpt.example.com"
  githubConnector:
    appId: ""
    clientId: ""
...
secret:
  values:
    ...
    databaseUrl: "mysql://offlinegpt:REPLACE_ME@mysql.example.internal:3306/offlinegpt_den?sslaccept=accept"
...
ingress:
  ...
  className: nginx
  web:
    host: offlinegpt.example.com
  api:
    host: api.offlinegpt.example.com
```
...
```bash
helm template offlinegpt-ee ./packaging/helm/offlinegpt-ee -f values.prod.yaml
helm upgrade --install offlinegpt-ee ./packaging/helm/offlinegpt-ee -f values.prod.yaml
```
...
unambiguous. Hosted OfflineGPT Cloud explicitly enables availability.
...
### OfflineGPT Web rollout

OfflineGPT Web is unavailable by default for self-hosted and customer-managed
deployments. OfflineGPT Cloud enables it in its deployment values with:
...
The chart renders this value as `DEN_OFFLINEGPT_WEB_ENABLED`. A raw environment
...
configure `STRIPE_SECRET_KEY` and `STRIPE_OFFLINEGPT_WEB_PRICE_ID` before the
...
secret:
  create: false
  existingSecret: offlinegpt-ee-secrets
```
...
Set optional `DATABASE_REDIS_URL` to enable Den API Redis-backed session and query caching. Set `DAYTONA_API_KEY` when `config.provisioner.mode` is `daytona`. Set `POLAR_ACCESS_TOKEN` when Polar feature gating is enabled. Set `OPENROUTER_MANAGEMENT_API_KEY` when enabling OfflineGPT Models management.
...
secret:
  values:
    databaseRedisUrl: "rediss://redis-master.offlinegpt.svc.cluster.local:6379"
```
...
Use `customCa` when OfflineGPT must trust a private certificate authority for
...
```bash
kubectl create secret generic offlinegpt-custom-ca \
  --namespace offlinegpt \
...
customCa:
  enabled: true
  existingSecret: offlinegpt-custom-ca
  existingConfigMap: ""
  key: ca.crt
```
...
```bash
kubectl create configmap offlinegpt-custom-ca \
  --namespace offlinegpt \
  --from-file=ca.crt=./corp-root-ca.pem
```
...
customCa:
  enabled: true
  existingSecret: ""
  existingConfigMap: offlinegpt-custom-ca
  key: ca.crt
```
...
`/etc/offlinegpt/custom-ca/ca-bundle.pem` and sets `NODE_EXTRA_CA_CERTS` to that
...
secret:
  values:
    databaseUrl: "mysql://offlinegpt:REPLACE_DB_PASSWORD@mysql.example.internal:3306/offlinegpt_den?sslmode=verify-full"
```
...
roots your OfflineGPT deployment should trust. On CA rotation, update the existing
Secret or ConfigMap and restart the running workloads so Node reloads the CA
file, for example:

```bash
kubectl rollout restart deployment/offlinegpt-ee-den-api --namespace offlinegpt
kubectl rollout restart deployment/offlinegpt-ee-den-web --namespace offlinegpt
kubectl rollout restart deployment/offlinegpt-ee-inference --namespace offlinegpt
```
...
The chart configures telemetry export from OfflineGPT; it does not install an
...
`localhost`, because that would refer to the OfflineGPT container itself.

First create the namespace used by this example:

```bash
kubectl create namespace offlinegpt
```
...
namespace as OfflineGPT**:

```bash
kubectl create secret generic offlinegpt-otel-headers \
  --namespace offlinegpt \
...
observability:
  backend: otel
  serviceNames:
    denApi: offlinegpt-den-api
    denWeb: offlinegpt-den-web
  otel:
    ...
    tracesSamplerArg: ""
    headers:
      existingSecret: offlinegpt-otel-headers
...
Install or upgrade OfflineGPT with the values file:
...
The commands below assume the Helm release is named `offlinegpt-ee`. If you use a
different release name, run `kubectl get deployments,services --namespace
offlinegpt` to find the generated resource names.

Confirm that the workloads are ready:

```bash
kubectl get pods --namespace offlinegpt
kubectl rollout status deployment/offlinegpt-ee-den-api --namespace offlinegpt
kubectl rollout status deployment/offlinegpt-ee-den-web --namespace offlinegpt
```
...
```bash
kubectl describe deployment/offlinegpt-ee-den-api --namespace offlinegpt
kubectl describe deployment/offlinegpt-ee-den-web --namespace offlinegpt
```
...
`offlinegpt-otel-headers`.

Generate a request that crosses both services. Keep this port-forward running:

```bash
kubectl port-forward service/offlinegpt-ee-den-web 3005:3005 --namespace offlinegpt
```
...
Your observability backend should show `offlinegpt-den-web` and
`offlinegpt-den-api`, with one connected trace for the request. Logs from both
...
- `observability.otel.endpoint` is a base endpoint. OfflineGPT appends
...
- The Secret must be in the OfflineGPT release namespace, and its key must match
...
observability:
  ...
  sentry:
    dsnSecret:
      existingSecret: offlinegpt-sentry-runtime
      key: SENTRY_DSN
...
- Setup URL: `https://offlinegpt.example.com/dashboard/integrations/github`
- Webhook URL: `https://api.offlinegpt.example.com/v1/webhooks/connectors/github`
...
- `DEN_API_BASE=http://<release>-offlinegpt-ee-den-api:8788`
- `DEN_AUTH_FALLBACK_BASE=http://<release>-offlinegpt-ee-den-api:8788`
- `INFERENCE_PROXY_BASE_URL=http://<release>-offlinegpt-ee-inference:8791` when `inference.enabled=true`
...
installerArtifacts:
  enabled: true
  existingClaim: offlinegpt-desktop-artifacts
  mountPath: /var/lib/offlinegpt/installer-artifacts
```
...
The organization download page hands the normal OfflineGPT app its Den
...
setup stays in the **Open OfflineGPT** deep-link step after installation.
