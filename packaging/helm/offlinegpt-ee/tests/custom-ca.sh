...
assert_failure() {
  ...
  if helm template offlinegpt-ee "$chart_dir" -f "$values_file" > "$output_file" 2> "$error_file"; then
...
helm template offlinegpt-ee "$chart_dir" --set inference.enabled=true > "$default_rendered"
...
customCa:
  ...
  key: ignored.pem
YAML
helm template offlinegpt-ee "$chart_dir" -f "$disabled_values" > "$disabled_rendered"
cmp -s "$default_rendered" "$disabled_rendered"
assert_not_contains "$default_rendered" 'name: NODE_EXTRA_CA_CERTS'
assert_not_contains "$default_rendered" '/etc/offlinegpt/custom-ca'
...
customCa:
  enabled: true
  existingSecret: offlinegpt-ca-secret
  key: corp-root.pem
YAML
helm template offlinegpt-ee "$chart_dir" -f "$secret_values" > "$secret_rendered"
assert_count "$secret_rendered" 'name: NODE_EXTRA_CA_CERTS' 4
assert_count "$secret_rendered" 'value: "/etc/offlinegpt/custom-ca/ca-bundle.pem"' 5
assert_count "$secret_rendered" 'name: custom-ca' 8
assert_count "$secret_rendered" 'secretName: "offlinegpt-ca-secret"' 4
assert_count "$secret_rendered" 'key: "corp-root.pem"' 4
assert_count "$secret_rendered" 'path: ca-bundle.pem' 4
assert_count "$secret_rendered" 'mountPath: "/etc/offlinegpt/custom-ca"' 4
assert_not_contains "$secret_rendered" 'subPath:'
assert_not_contains "$secret_rendered" 'name: "offlinegpt-ca-config"'
assert_not_contains "$secret_rendered" 'name: offlinegpt-ee-inference'
...
customCa:
  enabled: true
  existingConfigMap: offlinegpt-ca-config
  key: ca.crt
YAML
helm template offlinegpt-ee "$chart_dir" -f "$configmap_values" > "$configmap_rendered"
assert_contains "$configmap_rendered" 'name: offlinegpt-ee-den-api'
assert_contains "$configmap_rendered" 'name: offlinegpt-ee-den-web'
assert_contains "$configmap_rendered" 'name: offlinegpt-ee-inference'
assert_contains "$configmap_rendered" 'name: offlinegpt-ee-migrate'
assert_count "$configmap_rendered" 'name: NODE_EXTRA_CA_CERTS' 5
assert_count "$configmap_rendered" 'value: "/etc/offlinegpt/custom-ca/ca-bundle.pem"' 6
assert_count "$configmap_rendered" 'name: custom-ca' 10
assert_count "$configmap_rendered" 'name: "offlinegpt-ca-config"' 5
assert_count "$configmap_rendered" 'key: "ca.crt"' 5
assert_count "$configmap_rendered" 'path: ca-bundle.pem' 5
assert_count "$configmap_rendered" 'mountPath: "/etc/offlinegpt/custom-ca"' 5
assert_not_contains "$configmap_rendered" 'subPath:'
assert_not_contains "$configmap_rendered" 'secretName: "offlinegpt-ca-secret"'
...
YAML
helm template offlinegpt-ee "$chart_dir" -f "$installer_values" > "$installer_rendered"
...
assert_contains "$installer_rendered" 'name: OFFLINEGPT_INSTALLER_ARTIFACTS_DIR'
