...
const image = process.env.OFFLINEGPT_EVAL_MYSQL_IMAGE?.trim() || "mysql:8.4";
const containerName = `offlinegpt-custom-ca-mysql-${process.pid}-${Date.now()}`;
const tmp = mkdtempSync(join(tmpdir(), "offlinegpt-custom-ca-mysql-"));
...
const databaseName = "offlinegpt_den";
const databaseUser = "offlinegpt";
const databasePassword = "offlinegpt_tls_test_password";
...
function ensureBuildArtifacts() {
  ...
  console.log("Built @offlinegpt-ee/den-db migration artifacts");
}
...
`,
  ...
  runOpenSsl(["req", "-x509", "-newkey", "rsa:2048", "-nodes", "-days", "1", "-subj", "/CN=OfflineGPT Test MySQL CA", "-keyout", caKey, "-out", caCert]);
...
function startMysqlContainer() {
  const result = docker([
    ...
    "127.0.0.1::3306",
    "--env",
    "MYSQL_ROOT_PASSWORD=offlinegpt_root_password",
    "--env",
    "MYSQL_ROOT_HOST=%",
    "--env",
...
function runOfflineGPTMysqlQuery(databaseUrl, query, extraEnv = {}) {
  const source = `
import { createRequire } from "node:module";
...
function runBootstrap(databaseUrl) {
  return run(process.execPath, [bootstrapPath], {
    ...
    env: childEnv({
      ...
      DEN_DB_ENCRYPTION_KEY: "offlinegpt-custom-ca-mysql-tls-test-key-1234567890",
      NODE_EXTRA_CA_CERTS: caCert,
      OFFLINEGPT_DEN_DB_ENV_PATH: join(tmp, "does-not-exist.env"),
    }),
    timeout: 180_000,
  });
}
...
try {
  ...
  const withoutCa = runOfflineGPTMysqlQuery(databaseUrl, "select 1 as ok");
  if (withoutCa.status === 0) {
    throw new Error("Strict OfflineGPT/mysql2 unexpectedly connected without NODE_EXTRA_CA_CERTS");
  }
  const withoutCaError = `${withoutCa.stdout}\n${withoutCa.stderr}`.trim();
  if (!/certificate|verify|self-signed|unable/i.test(withoutCaError)) {
    throw new Error(`Strict OfflineGPT/mysql2 failed for a non-certificate reason:\n${withoutCaError}`);
  }
  ...
  console.log(`Strict OfflineGPT/mysql2 without custom CA: rejected (${certificateError})`);

  const bootstrap = runBootstrap(databaseUrl);
  ensureSuccess(bootstrap, "OfflineGPT migration bootstrap with NODE_EXTRA_CA_CERTS");
  console.log("OfflineGPT migration bootstrap completed with NODE_EXTRA_CA_CERTS and strict DATABASE_URL");
  console.log(bootstrap.stdout.trim());

  const query = runOfflineGPTMysqlQuery(
    databaseUrl,
  ...
  ensureSuccess(query, "OfflineGPT/mysql2 strict post-migration query with NODE_EXTRA_CA_CERTS");
  ...
  if (tableCount <= 0 || migrationCount <= 0) {
    throw new Error(`Expected migrated OfflineGPT tables and migration ledger rows, got ${query.stdout}`);
  }
  console.log(`OfflineGPT/mysql2 strict query succeeded after migration: tables=${tableCount}, migrations=${migrationCount}`);
} finally {
  cleanup();
}
