...
const tmp = mkdtempSync(join(tmpdir(), "offlinegpt-custom-ca-"));
...
https.get(process.argv[1], (response) => {
  ...
  response.on("end", () => {
    if (response.statusCode === 200 && body === "offlinegpt-custom-ca-ok") {
      console.log(body);
      process.exit(0);
    }
...
`,
  ...
  runOpenSsl(["req", "-x509", "-newkey", "rsa:2048", "-nodes", "-days", "1", "-subj", "/CN=OfflineGPT Test CA", "-keyout", caKey, "-out", caCert]);
  ...
  const server = createServer(
    ...
    (_request, response) => {
      response.end("offlinegpt-custom-ca-ok");
    },
  );
