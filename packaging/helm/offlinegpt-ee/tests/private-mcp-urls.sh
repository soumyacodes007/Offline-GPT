...
assert_source_contains() {
  ...
  while IFS= read -r line; do
    if [[ "$line" == '# Source: offlinegpt-ee/templates/'* ]]; then
      if [[ "$line" == "# Source: offlinegpt-ee/templates/$source" ]]; then
        in_source=1
      else
        in_source=0
      fi
...
helm template offlinegpt-ee "$chart_dir" > "$default_rendered"
assert_count "$default_rendered" 'DEN_ALLOW_PRIVATE_MCP_URLS' 0
assert_not_contains "$default_rendered" 'offlinegpt-ee-den-api-private-mcp-urls'
...
YAML
helm template offlinegpt-ee "$chart_dir" -f "$enabled_values" > "$enabled_rendered"
...
assert_not_contains "$enabled_rendered" 'offlinegpt-ee-den-api-private-mcp-urls'
...
YAML
helm template offlinegpt-ee "$chart_dir" -f "$false_values" > "$false_rendered"
...
config:
  public:
    allowPrivateMcpUrls: ""
YAML
helm template offlinegpt-ee "$chart_dir" -f "$blank_values" > "$blank_rendered"
