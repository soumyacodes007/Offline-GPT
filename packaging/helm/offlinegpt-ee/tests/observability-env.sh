...
YAML
helm template offlinegpt-ee "$chart_dir" -f "$otel_values" > "$otel_rendered"
...
YAML
helm template offlinegpt-ee "$chart_dir" -f "$legacy_values" > "$legacy_rendered"
...
observability:
  ...
  sentry:
    ...
    dist: web
YAML
helm template offlinegpt-ee "$chart_dir" -f "$sentry_values" > "$sentry_rendered"
...
assert_file_contains "$repo_root/.devcontainer/start-daytona-server.sh" 'pnpm --filter @offlinegpt-ee/den-api exec tsx watch src/main.ts'
