...
async function killPidfile(userDataDir) {
  const pidPath = join(userDataDir, "offlinegpt-eval-chrome.pid");
  let pid = 0;
  try {
...
async function ensureProfile(port) {
  const userDataDir = `/tmp/offlinegpt-evals-web-${port}`;
  ...
  child.unref();
  await writeFile(join(userDataDir, "offlinegpt-eval-chrome.pid"), `${child.pid}\n`);
...
for (const [name, port] of PROFILES) {
  console.log(`export OFFLINEGPT_EVAL_WEB_CDP_${name}=http://127.0.0.1:${port}`);
}
