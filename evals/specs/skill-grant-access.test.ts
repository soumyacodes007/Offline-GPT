...
import { denFetch, ensureMemberSession, signIn } from "@offlinegpt/behaviors";
...
const title = apiUrl
  ...
  : "skill grant access skipped: set OFFLINEGPT_EVAL_DEN_API_URL";
let requestId = 0;
...
test.skipIf(!apiUrl)(title, async () => {
  const den = {
    apiUrl,
    webUrl: (process.env.OFFLINEGPT_EVAL_DEN_WEB_URL?.trim() || apiUrl.replace("127.0.0.1", "localhost")).replace(/\/+$/, ""),
  };
  ...
  const creator = await ensureMemberSession(den, admin, {
    email: process.env.OFFLINEGPT_EVAL_CREATOR_EMAIL?.trim() || "casey.spec@acme.test",
    password: process.env.OFFLINEGPT_EVAL_MEMBER_PASSWORD?.trim() || "OfflineGPTDemo123!",
    name: "Casey Spec",
    markVerifiedCmd: process.env.OFFLINEGPT_EVAL_MARK_VERIFIED_CMD?.trim(),
  });
  await selectOrganization(creator, orgId);
  const deniedEmail = process.env.OFFLINEGPT_EVAL_MEMBER_EMAIL?.trim() || "nova.spec@acme.test";
  ...
  const thirdEmail = process.env.OFFLINEGPT_EVAL_THIRD_MEMBER_EMAIL?.trim() || "riley.spec@acme.test";
