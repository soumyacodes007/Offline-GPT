...
test("the composer model pickers keep their controls without the OfflineGPT Models subscribe promo", async ({ user, step }) => {
  ...
  await step("the full Models dialog keeps controls without subscribe promotion", async () => {
    ...
    await user.notSee({ role: "button", label: "Hide OfflineGPT Models" });
