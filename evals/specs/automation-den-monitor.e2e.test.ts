 ...
 * Desktop work and OfflineGPT Web creates Cloud work — so Den shows what is
...
const test = spec.world(desktopAutomationOnDenMonitor, {
  needs: { optIn: ["OFFLINEGPT_EVAL_AUTOMATIONS_E2E_TEST"] },
  timeout: 420_000,
});
...
test("Den lists Automations as a read-only monitor that routes management to Web and Desktop", async ({ world, user, step, evidence }) => {
  await step("inspect the Den Automation monitor", async () => {
    ...
    await user.see({ text: /Create and edit Cloud Automations in OfflineGPT Web/ });
    await user.see({ text: "Open in OfflineGPT Web" });
    ...
    evidence.recordAssertionEvidence(
      ...
      "The Den Automations page lists Den-scheduled work grouped by attention and routes creation to OfflineGPT Web instead of offering its own form.",
      true,
    );
  });
  ...
  await step("inspect the Desktop Automation detail", async () => {
    ...
    await user.see({ text: "Manage in OfflineGPT Desktop" });
    ...
    await user.looks([
      ...
      "A note that the Automation is managed in OfflineGPT Desktop",
