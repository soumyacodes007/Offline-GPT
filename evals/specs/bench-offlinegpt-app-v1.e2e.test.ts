...
import { clickButton, createAndSelectWorkspace, evalIn, readComposerState } from "@offlinegpt/behaviors";
import type { Surface } from "@offlinegpt/cdp";
import { desktop } from "@offlinegpt/hosts";
import type { DesktopHandle } from "@offlinegpt/hosts";
...
const benchEngine = process.env.OFFLINEGPT_BENCH_ENGINE === "v2" ? "v2" : "v1";
const title = enabled
  ? `OfflineGPT Electron ${benchEngine} engine completes the CDP benchmark with faithful witness traffic`
  : `OfflineGPT Electron ${benchEngine} benchmark skipped — needs: set OFFLINEGPT_EVAL_E2E_TESTS=1`;
...
async function startWitness(nonce: string): Promise<Witness> {
  ...
  const server = createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      const requestId = `chatcmpl-offlinegpt-v1-bench-${requestNumber}`;
...
async function createSecondWorkspaceViaUi(app: Surface, firstWorkspaceId: string, workspacePath: string): Promise<string> {
  ...
  const workspaceId = await eventually(async () => {
    const value = await evalIn(app, () => (localStorage.getItem("offlinegpt.react.activeWorkspace") ?? ""));
    return typeof value === "string" ? value : "";
  }, {
    within: 120_000,
    intervalMs: pollResolutionMs,
...
test.skipIf(!enabled)(title, { timeout: 900_000 }, async ({ evidence, place }) => {
  ...
  expect(Number.isInteger(iterations) && iterations > 0, "OFFLINEGPT_BENCH_ITERATIONS must be a positive integer").toBe(true);
  const opencodeV2Bin = benchEngine === "v2" && place.kind === "local" ? process.env.OFFLINEGPT_EVAL_OPENCODE2_BIN?.trim() || undefined : undefined;
  ...
  let uiCompactionAvailable = false;
    ...
    for (let coldIndex = 0; coldIndex < iterations; coldIndex += 1) {
      ...
      const profileDir = await mkdtemp(join(tmpdir(), `offlinegpt-bench-${benchEngine}-profile-`));
      const workspaceAPath = await mkdtemp(join(tmpdir(), `offlinegpt-bench-${benchEngine}-workspace-a-`));
      const workspaceBPath = coldIndex === iterations - 1
        ? await mkdtemp(join(tmpdir(), `offlinegpt-bench-${benchEngine}-workspace-b-`))
        : null;
      ...
      try {
        const appName = `bench-offlinegpt-app-${benchEngine}-${coldIndex}`;
        ...
        app = await desktop({
          ...
          env: {
            ...
            ...(opencodeV2Bin === undefined ? {} : { OFFLINEGPT_OPENCODE2_BIN: opencodeV2Bin }),
          },
        });
    ...
    const resultsPath = join(resultsDir, `bench-offlinegpt-app-${benchEngine}-${Date.now()}.json`);
    await writeFile(resultsPath, `${JSON.stringify(report, null, 2)}\n`);
    console.info(`[bench-offlinegpt-app-${benchEngine}] medians\n${medianTable(results)}`);
    console.info(`[bench-offlinegpt-app-${benchEngine}] results=${resultsPath} uiCompactionAvailable=${uiCompactionAvailable}`);
});
