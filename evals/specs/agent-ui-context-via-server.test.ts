...
function completedContextOutput(value: unknown): string | null {
  ...
  for (const message of value) {
    ...
    for (const part of message.parts) {
      if (!isRecord(part) || part.type !== "tool" || part.tool !== "offlinegpt_context" || !isRecord(part.state)) continue;
...
test("the in-app agent reads renderer context through offlinegpt-server and gets an explicit result without a window", async ({ world, probe, step, evidence }) => {
  await step("without a connected window the tool reports that no OfflineGPT window is connected and the turn completes", async () => {
    ...
    const output = await probe.eventually(
      ...
      {
        within: 30_000,
        label: "completed offlinegpt_context tool without a window",
        until: (value) => value !== null,
      },
    );
    if (output === null) throw new Error("offlinegpt_context did not complete");
    ...
    expect(error).toContain("No OfflineGPT window is connected");
    expect(error).not.toContain("desktop app may not be running");
    expect(replyText(result)).toBe(MOCK_REPLY);
    evidence.recordAssertionEvidence(
      "Without a polling OfflineGPT window, the agent receives the server's explicit no-window result and still completes its turn",
      `offlinegpt_context returned context=null and ui.error=${JSON.stringify(error)}; the model's reply was ${JSON.stringify(replyText(result))}.`,
      true,
    );
  });
  ...
  await step("a connected window's split layout reaches both the tool transcript and the model", async () => {
    ...
    try {
      ...
      const output = await probe.eventually(
        ...
        {
          within: 30_000,
          label: "completed offlinegpt_context tool with a connected window",
          until: (value) => value !== null,
        },
      );
      if (output === null) throw new Error("offlinegpt_context did not complete");
      ...
      expect(replyText(result)).toBe(MOCK_REPLY);
      evidence.recordAssertionEvidence(
        "A renderer window's split layout crosses offlinegpt-server into the agent tool result and the model's follow-up request",
  ...
  await step("only authenticated clients can exchange requests, and each command is delivered once", async () => {
    ...
    const hostHeaders = { ...headers, "x-offlinegpt-host-token": world.hostToken };
