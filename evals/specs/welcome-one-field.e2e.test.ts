...
test("desktop opens directly while the non-desktop welcome join field accepts a server URL or web invite", async ({ world, user: desktopUser, probe, step }) => {
  ...
  await step("Non-desktop welcome keeps its optional entry paths", async () => {
    ...
    await user.notSee({ text: /Using OfflineGPT on-premises\?/ });
    await user.looks([
      "The Welcome to OfflineGPT heading is visible",
      "Sign in to OfflineGPT Cloud and Use Without Cloud are offered",
      "Join your organization says to paste an invite link, install link, or server URL",
      "The page does not say Using OfflineGPT on-premises",
    ]);
  });
  ...
  await step("A server URL becomes the control plane", async () => {
    await user.type(joinInput, "https://offlinegpt.acme.test");
    await user.click("Connect");
    await user.see({ text: /Connected to offlinegpt\.acme\.test\. Sign in to continue\./ }, { timeoutMs: 20_000 });
    expect(await readBootstrapBaseUrl()).toBe("https://offlinegpt.acme.test");
    await user.looks([
      "The join dialog field label mentions invite link, install link, or server URL",
      "The dialog confirms it connected to offlinegpt.acme.test",
    ]);
  });
  ...
  await step("A web invite requests trust before browser handoff", async () => {
    ...
    expect(await probe.hash()).toMatch(/^#\/workspace\/[^/]+\/session$/);
    expect(await probe.storage("offlinegpt.den.baseUrl")).not.toBe("http://localhost:59991");
  });
});
