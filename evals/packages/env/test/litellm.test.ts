...
const MODEL_ID = "offlinegpt-litellm-unit-model";
...
function makeFake(options: FakeOptions = {}): {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    if (url.pathname === "/__offlinegpt_litellm/requests") {
...
test("LiteLLM Daytona sandbox names are safe and unique", () => {
  ...
  assert.match(first, /^offlinegpt-litellm-eval-[0-9]+-[a-z0-9]+-[0-9a-f]{8}$/);
