...
test("resolvePlace selects local unless OFFLINEGPT_EVAL_DAYTONA is exactly 1", () => {
  const local = resolvePlace({});
  const falseyDaytona = resolvePlace({ OFFLINEGPT_EVAL_DAYTONA: "0" });
  const daytona = resolvePlace({ OFFLINEGPT_EVAL_DAYTONA: "1", OFFLINEGPT_EVAL_REF: "feature-ref" });
...
test("needs accepts a tool-capable model and provider key", () => {
  assert.doesNotThrow(() => checkNeeds(
    ...
    {
      OFFLINEGPT_EVAL_MODEL: "openai/gpt-5",
...
test("needs throws a named SkipError for every unsatisfied resource", () => {
  assert.throws(
    ...
    (error) => {
      ...
      assert.match(error.message, /set OFFLINEGPT_EVAL_MODEL/);
      assert.match(error.message, /set OPENAI_API_KEY or ANTHROPIC_API_KEY/);
      assert.match(error.message, /set OFFLINEGPT_EVAL_DAYTONA=1/);
      return true;
    },
  );
});
...
test("needs reports an unavailable command", () => {
  const command = "offlinegpt-impossible-command-for-testkit-test";
  assert.throws(
...
test("needs rejects a local-only test on Daytona or an attached Den", () => {
  ...
  assert.throws(() => checkNeeds({ placement: "local" }, { OFFLINEGPT_EVAL_DAYTONA: "1" }), SkipError);
  assert.throws(
    () => checkNeeds({ placement: "local" }, { OFFLINEGPT_EVAL_DEN_API_URL: "https://den.example.test" }),
    SkipError,
  );
});

test("needs reads process.env at the call site", () => {
  const name = "OFFLINEGPT_TESTKIT_UNIT_RESOURCE";
...
test("mcp mock environment is derived from the resource name and public URLs", () => {
  assert.deepEqual(
    ...
    {
      OFFLINEGPT_EVAL_MOCK_ACME_TICKETS_URL: "https://mock.example.test",
      OFFLINEGPT_EVAL_MOCK_ACME_TICKETS_MCP_URL: "https://mock.example.test/mcp",
    },
  );
});
