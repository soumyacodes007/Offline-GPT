...
export async function readLoadedExtensions(app: Surface): Promise<string[]> {
  ...
  await waitFor(app, () => (document.body.innerText.includes("OfflineGPT Browser")), {
    timeoutMs: 10_000,
    label: "OfflineGPT Browser extension",
  });
  const value = await waitFor(app, () => (([...document.querySelectorAll("button")]
    .map((button) => (button.textContent ?? "").replace(/\s+/g, " ").trim())
    .filter((label) => label.includes("OfflineGPT Browser")))), { timeoutMs: 60_000, label: "OfflineGPT Browser extension rows" });
