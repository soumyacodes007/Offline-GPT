...
import { notImplemented } from "@offlinegpt/labs";
...
export function doInternalMarkEmailVerified(command: string, email: string): void {
  if (!command.trim()) throw new Error("OFFLINEGPT_EVAL_MARK_VERIFIED_CMD is required to verify a newly-created member.");
  try {
...
export async function provisionOrg(den: DenRef, input: ProvisionOrgInput): Promise<ProvisionedOrg> {
  ...
  const email = `offlinegpt-eval-admin-${unique}@example.test`;
  const name = `OfflineGPT Eval ${unique}`;
  const signUp = await denFetch(den, "/api/auth/sign-up/email", {
    method: "POST",
  ...
  for (const connector of connectors) {
    const configured = await denFetch(den, `/v1/oauth-providers/${connector}/client`, {
      ...
      headers: auth(admin),
      body: JSON.stringify({
        clientId: `offlinegpt-eval-google-client-${unique}`,
        clientSecret: `offlinegpt-eval-google-secret-${unique}`,
      }),
    });
...
export async function createDesktopHandoffGrant(member: DenSession, desktopScheme = "offlinegpt"): Promise<string> {
