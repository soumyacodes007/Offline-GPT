 ...
 *   6. A demo-owner session token, exported as OFFLINEGPT_EVAL_DEN_API_URL /
 *      OFFLINEGPT_EVAL_DEN_TOKEN so env-gated specs run without manual setup.
 *
...
const EVAL_ELECTRON_USERDATA = process.env.OFFLINEGPT_EVAL_ELECTRON_USERDATA?.trim()
...
const DEMO_PASSWORD = process.env.DEN_DEMO_OWNER_PASSWORD ?? "OfflineGPTDemo123!";
...
const COMPOSE_ARGS = ["compose", "-p", "offlinegpt-den-local", "-f", "packaging/docker/docker-compose.web-local.yml"];
const DEFAULT_MOCK_IDP_ISSUER = "http://127.0.0.1:19190";
const MOCK_IDP_ISSUER = (process.env.OFFLINEGPT_EVAL_MOCK_IDP_ISSUER?.trim() || DEFAULT_MOCK_IDP_ISSUER).replace(/\/+$/, "");
const DEN_WEB_ORIGIN = (process.env.OFFLINEGPT_EVAL_DEN_WEB_URL ?? "http://localhost:3005").replace(/\/+$/, "");
...
const DEN_TRUSTED_ORIGINS = [
  ...
  "http://127.0.0.1:5173",
  MOCK_IDP_ISSUER,
  ...envCsv("OFFLINEGPT_EVAL_DEN_EXTRA_TRUSTED_ORIGINS"),
].filter((origin, index, origins) => origins.indexOf(origin) === index).join(",");

// Override with OFFLINEGPT_EVAL_DATABASE_URL to isolate a run from the shared
// dev database (e.g. a dedicated schema on the same MySQL container).
const DEN_DATABASE_URL = process.env.OFFLINEGPT_EVAL_DATABASE_URL ?? "mysql://root:password@127.0.0.1:3306/offlinegpt_den";
const DEN_DATABASE_NAME = new URL(DEN_DATABASE_URL).pathname.replace(/^\//, "") || "offlinegpt_den";
...
function devBootstrapPath(): string {
  return join(devUserDataHome(), "offlinegpt-dev-data", "home", ".config", "offlinegpt", "desktop-bootstrap.json");
}
...
async function ensureMysql(log: (message: string) => void): Promise<void> {
  if (process.env.OFFLINEGPT_EVAL_DATABASE_URL) {
    if (!(await nativeMysqlHealthy())) {
      throw new Error("OFFLINEGPT_EVAL_DATABASE_URL is configured but not reachable.");
    }
...
async function ensureDenApi(log: (message: string) => void, orgMode?: DenOrgMode): Promise<void> {
  ...
  if (publicHealthOk) {
    throw new Error(
      ...
      "The desktop app cannot use a bare den-api there; rerun with OFFLINEGPT_EVAL_DEN_PORT=<free port>.",
    );
  }
  ...
  const proxyPid = spawnDetached(process.execPath, [join(HOSTS_SRC_DIR, "den-proxy.ts")], {
    ...
    env: {
      ...
      OFFLINEGPT_EVAL_DEN_PROXY_CONTROL: "1",
    },
  });
...
async function ensureApp(log: (message: string) => void, cdpCandidates: string[]): Promise<void> {
  ...
  const pid = spawnDetached("pnpm", ["dev"], {
    logName: "app",
    env: { OFFLINEGPT_ELECTRON_USERDATA: EVAL_ELECTRON_USERDATA },
  });
...
export async function ensureDenStack({ log, cdpCandidates, skipApp = false, orgMode }: EnsureDenStackOptions): Promise<void> {
  ...
  process.env.OFFLINEGPT_EVAL_DEN_WEB_URL = DEN_WEB_ORIGIN;
  process.env.OFFLINEGPT_EVAL_DEN_TOKEN = token;
  log(`Den stack ready — flows get OFFLINEGPT_EVAL_DEN_API_URL=${DEN_API_URL}, OFFLINEGPT_EVAL_DEN_WEB_URL=${DEN_WEB_ORIGIN}, and a fresh ${DEMO_EMAIL} token.`);
}
