...
const stderrFault = process.env.OFFLINEGPT_TEST_STDOUT_STREAM === "stderr";
...
if (process.env.OFFLINEGPT_TEST_STDIO_CONTROL === "1") {
...
process.stdout.write = (...args) => {
  if (process.env.OFFLINEGPT_TEST_STDIO_CONTROL === "1") return write(...args);
  if (!String(args[0]).includes("GET /health 200")) return write(...args);
  if (stderrFault) {
    if (!stderrFaultInjected) {
      stderrFaultInjected = true;
      const code = process.env.OFFLINEGPT_TEST_STDOUT_ERROR;
  ...
  if (process.env.OFFLINEGPT_TEST_STDOUT_MODE === "sync") throw error;
