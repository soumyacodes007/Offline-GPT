...
import { productDiagnosticsPrecondition } from "@offlinegpt/behaviors";
...
test("openssl argv construction includes CA, AIA, and fullchain prerequisites", () => {
  const commands = opensslCertificateCommands({
    dir: "/tmp/offlinegpt-egress-test",
    hostname: "localhost",
  ...
  assert.ok(commands.some((command) => command.args.includes("/CN=OfflineGPT Egress Lab Corporate Interception CA")));
