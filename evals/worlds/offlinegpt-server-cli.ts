...
export function engineBinary(): string | null {
  const fromEnv = process.env.OFFLINEGPT_OPENCODE_BIN?.trim();
...
/**
 * Spawns the offlinegpt-server CLI with a managed engine. Every stdout/stderr
...
export function bootServer(env: NodeJS.ProcessEnv, token: string, workspace: string, sink: (chunk: string) => void): { child: ChildProcess; listening: Promise<string> } {
  ...
  const listening = new Promise<string>((resolveBase, reject) => {
    const timer = setTimeout(() => reject(new Error(`offlinegpt-server did not report a port within 60s:\n${seen.slice(-2_000)}`)), 60_000);
    const onChunk = (chunk: Buffer) => {
      const text = chunk.toString("utf8");
      seen += text;
      sink(text);
      const match = seen.match(/OfflineGPT server listening on (http:\/\/127\.0\.0\.1:\d+)/);
      if (match?.[1]) {
        clearTimeout(timer);
        resolveBase(match[1]);
      }
    ...
    child.on("exit", (code) => {
      clearTimeout(timer);
      reject(new Error(`offlinegpt-server exited early (code ${code}):\n${seen.slice(-2_000)}`));
    });
...
export interface ManagedOfflineGptServer {
  child: ChildProcess;
  base: string;
  binary: string;
  workspaceId: string;
...
export async function bootManagedOfflineGptServer(options: {
  scratch: string;
  workspace: string;
  token: string;
  sink: (chunk: string) => void;
  binary?: string;
}): Promise<ManagedOfflineGptServer> {
  ...
  // The spec may itself run under an OfflineGPT or OpenCode session. The parent's
  // OPENCODE_* and OFFLINEGPT_* variables (config path, models URL, credentials,
  // workspaces) must not reach the isolated server and engine under test.
  const inherited = Object.fromEntries(Object.entries(process.env).filter(([key]) => !key.startsWith("OPENCODE") && !key.startsWith("OFFLINEGPT_")));
  const env = {
    ...inherited,
    HOME: home,
    ...
    OFFLINEGPT_OPENCODE_BIN: binary,
  };
  ...
  try {
    ...
    for (let attempt = 1; ; attempt += 1) {
      ...
      try {
        ...
        if (attempt >= 2 || !(error instanceof Error && error.message.startsWith("offlinegpt-server exited early"))) throw error;
      }
    }
    ...
    if (!workspaceId) throw new Error(`offlinegpt-server reported no workspace: ${JSON.stringify(workspaces)}`);
    ...
    if (!ready) throw new Error(`The managed engine never answered through offlinegpt-server:\n${output.slice(-3_000)}`);
