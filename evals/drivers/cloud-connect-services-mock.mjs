...
const MICROSOFT_MESSAGES = [
  {
    ...
    toRecipients: [{ emailAddress: { name: "OfflineGPT Tester", address: "tester@example.test" } }],
...
const MICROSOFT_Q3_FILE = {
  ...
  parentReference: { driveId: "drive-offlinegpt-test", path: "/drive/root:" },
};
...
function sendJson(response, status, body, headers = {}) {
  response.writeHead(status, {
    ...
    "access-control-allow-headers": "authorization,content-type,x-offlinegpt-host-token",
...
function handleMicrosoftGraph(request, response, url) {
  ...
  if (url.pathname === "/graph/v1.0/me" && request.method === "GET") {
    sendJson(response, 200, {
      id: "microsoft-user-offlinegpt-test",
      displayName: "OfflineGPT Tester",
      givenName: "OfflineGPT",
      surname: "Tester",
  ...
  if (url.pathname === "/graph/v1.0/me/messages" && request.method === "POST") {
    sendJson(response, 201, {
      id: "draft-offlinegpt-test",
      subject: "OfflineGPT permission parity draft",
      bodyPreview: "Drafted by the deterministic Microsoft Graph mock.",
      webLink: "https://outlook.office.test/mail/draft-offlinegpt-test",
      isDraft: true,
    });
    return true;
  }
  ...
  if (url.pathname === "/graph/v1.0/me/events" && request.method === "POST") {
    sendJson(response, 201, {
      id: "event-offlinegpt-test",
      subject: "OfflineGPT permission parity review",
      start: { dateTime: "2026-07-13T10:00:00Z", timeZone: "UTC" },
      end: { dateTime: "2026-07-13T10:30:00Z", timeZone: "UTC" },
      webLink: "https://outlook.office.test/calendar/event-offlinegpt-test",
    });
    return true;
  }
  ...
  if (url.pathname === "/graph/v1.0/me/drive/root:/OfflineGPT/permission-parity.txt:/content" && request.method === "PUT") {
    sendJson(response, 201, {
  ...
  if (url.pathname === "/graph/v1.0/me/chats" && request.method === "GET") {
    sendJson(response, 200, graphEnvelope([{
      id: "chat-offlinegpt-test",
      topic: "OfflineGPT launch",
      chatType: "group",
      webUrl: "https://teams.office.test/chats/chat-offlinegpt-test",
      lastUpdatedDateTime: "2026-07-13T09:00:00Z",
    }]));
    return true;
  }

  if (url.pathname === "/graph/v1.0/chats/chat-offlinegpt-test/messages" && request.method === "GET") {
    sendJson(response, 200, graphEnvelope([{
      ...
      from: { user: { id: "microsoft-user-offlinegpt-test", displayName: "OfflineGPT Tester" } },
      webUrl: "https://teams.office.test/messages/teams-message-existing",
    }]));
    return true;
  }

  if (url.pathname === "/graph/v1.0/chats/chat-offlinegpt-test/messages" && request.method === "POST") {
    sendJson(response, 201, {
...
function hasWorkerDualAuth(request) {
  return request.headers["x-offlinegpt-host-token"] === MOCK_WORKER_HOST_TOKEN
...
function workerUnauthorized(response) {
  sendJson(response, 401, {
    ...
    message: "Both the OfflineGPT host token and client bearer token are required.",
  });
}
...
function workerMessages(session, includeReply) {
  ...
  if (includeReply && session.prompt) {
    messages.push(workerMessage(session, "assistant", `${session.messageId}-assistant`, `OfflineGPT worker reply: ${session.prompt}`, session.messageId));
  }
  return messages;
}
