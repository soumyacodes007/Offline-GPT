...
test("Microsoft OAuth redirects with state and Graph serves deterministic mail, calendar, and files", async (context) => {
  ...
  authorizeUrl.searchParams.set("state", "state-from-offlinegpt");
  ...
  assert.equal(callback.searchParams.get("state"), "state-from-offlinegpt");
...
test("worker mock rejects partial credentials and completes a prompt through native session reads", async (context) => {
  ...
  const partial = await fetch(`${origin}/worker/workspaces`, {
    headers: { "x-offlinegpt-host-token": MOCK_WORKER_HOST_TOKEN },
  });
  assert.equal(partial.status, 401);

  const headers = {
    "x-offlinegpt-host-token": MOCK_WORKER_HOST_TOKEN,
  ...
  assert.equal(messages.at(-1).parts[0].text, "OfflineGPT worker reply: Summarize the launch notes");
});
