...
description: Daytona seeded cloud demo, demo credentials, Acme Robotics seed. Use when the user asks to spin up, keep running, seed, or prepare an OfflineGPT Cloud/Den Daytona demo instance.
---

# Daytona Seeded Cloud Demo

Use this skill to quickly create a persistent-enough OfflineGPT Cloud/Den Daytona server demo, seed it with Acme Robotics demo data, validate the login, and return copy-pasteable demo details.
...
SANDBOX="offlinegpt-cloud-demo-seeded"
...
```bash
SANDBOX="offlinegpt-cloud-demo-seeded-$(date +%Y%m%d-%H%M)"
bash .devcontainer/test-server-on-daytona.sh "$BRANCH" --name "$SANDBOX"
```
...
```bash
daytona exec "$SANDBOX" -- 'bash -lc '\''cd /workspace && pnpm --filter @offlinegpt/email build && cd /workspace/ee/apps/den-api && OFFLINEGPT_DEV_MODE=1 DATABASE_URL=mysql://root:password@127.0.0.1:3306/offlinegpt_den DEN_DB_ENCRYPTION_KEY=daytona-den-db-encryption-key-please-change-1234567890 BETTER_AUTH_SECRET=daytona-den-auth-secret-please-change-1234567890 BETTER_AUTH_URL="'"$DEN_WEB_URL"'" pnpm exec tsx scripts/seed-demo-org.ts --reset'\'''
```
...
curl -fsS -X POST "$DEN_WEB_URL/api/auth/sign-in/email" \
  ...
  --data '{"email":"alex@acme.test","password":"OfflineGPTDemo123!"}'
```
...
If sign-in returns `403` for email verification, Den API is not running with `OFFLINEGPT_DEV_MODE=1` or did not restart after env changes. Restart the Den stack and rerun the seed.
