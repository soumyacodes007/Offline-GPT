...
const paths = {
  ...
  models: path.resolve("ee/apps/inference/src/models/offlinegpt-models.json"),
  aliases: path.resolve("packages/types/src/den/inference.ts"),
}

function usage() {
  console.log(`Usage:
  offlinegpt-models.mjs search <query>
  offlinegpt-models.mjs add <query-or-id>
  offlinegpt-models.mjs remove <id> [<id2>]
  offlinegpt-models.mjs discount <usageFactor> <id> [<id2>]
  offlinegpt-models.mjs sync
  offlinegpt-models.mjs validate`)
}
...
async function readOfflineGptModels() {
...
function resolveExactOfflineGptIds(offlinegptModels, ids) {
  const resolved = []
  for (const id of ids) {
    if (offlinegptModels[id]) {
      resolved.push(id)
      continue
    }

    const matches = findMatches(offlinegptModels, id)
    console.error(`OfflineGPT model "${id}" was not exact. Please clarify with exact id:`)
    displayMatches(matches)
    process.exitCode = 2
    return null
  }
...
async function syncAliases(models, usageOverrides = new Map()) {
  ...
  const entries = Object.entries(models).map(([id, model]) => {
    ...
    const displayName = `OfflineGPT: ${model.name}`
...
async function validate() {
  const models = await readOfflineGptModels()
  ...
  console.log(`validated ${Object.keys(models).length} OfflineGPT model(s)`)
}
...
async function remove(ids) {
  const models = await readOfflineGptModels()
  const resolved = resolveExactOfflineGptIds(models, ids)
  if (!resolved) return
...
if (command === "search") {
  ...
  await syncAll(await readOfflineGptModels())
