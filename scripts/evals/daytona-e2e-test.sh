...
LOG_DIR="${OFFLINEGPT_E2E_TEST_LOG_DIR:-/workspace/evals/results/e2e-test-run}"
mkdir -p "$LOG_DIR"
...
if [ -x "$HOME/mark-verified.sh" ]; then
  export OFFLINEGPT_EVAL_MARK_VERIFIED_CMD="bash $HOME/mark-verified.sh {email}"
fi
...
if [ "${OFFLINEGPT_E2E_TEST_NEEDS_DEN:-0}" = "1" ]; then
...
DEN_API_CANDIDATE="${OFFLINEGPT_EVAL_DEN_API_URL:-http://127.0.0.1:8790}"
if curl -sf -m 3 -o /dev/null "$DEN_API_CANDIDATE/health"; then
  export OFFLINEGPT_EVAL_DEN_API_URL="$DEN_API_CANDIDATE"
  export OFFLINEGPT_EVAL_DEN_WEB_URL="${OFFLINEGPT_EVAL_DEN_WEB_URL:-http://localhost:3005}"
  echo "==> Den answers at $OFFLINEGPT_EVAL_DEN_API_URL"
else
  unset OFFLINEGPT_EVAL_DEN_API_URL OFFLINEGPT_EVAL_DEN_WEB_URL
  echo "==> No Den running; Den-gated tests will skip (set OFFLINEGPT_E2E_TEST_NEEDS_DEN=1 to start one)"
fi
