...
| ---- | ---- | ------------------------ |
| 01   | Ready to ship the migration | Tauri users see "OfflineGPT is moving" prompt on next update |
...
- Pushes to `dev` or `main` refresh the rolling prerelease bucket at
  <https://github.com/different-ai/offlinegpt/releases/tag/electron-preview-latest>.
...
- The install handoff requires a pasted Electron artifact URL plus two explicit
  ...
  `OfflineGPT.app.migrate-bak` for rollback.
...
3. Ask testers to download the matching platform artifact:
   - macOS Apple Silicon: `offlinegpt-mac-arm64-*.dmg` or `.zip`
   - macOS Intel: `offlinegpt-mac-x64-*.dmg` or `.zip`
   - Windows: `offlinegpt-win-x64-*.exe`
   - Linux: `offlinegpt-linux-x64-*.AppImage` or `.tar.gz`
...
node scripts/migration/01-cut-migration-release.mjs --version 0.12.0 \
  --mac-url 'https://github.com/different-ai/offlinegpt/releases/download/v0.12.0/OfflineGPT-darwin-arm64-0.12.0-mac.zip' \
...
3. Creates a release-config fragment at
   ...
   `VITE_OFFLINEGPT_MIGRATION_RELEASE=1` and the per-platform download
...
- Users still on Tauri: the migrate prompt is gated on
  `VITE_OFFLINEGPT_MIGRATION_RELEASE=1` at build time. Re-cut the
...
- Users mid-migration: the Rust `migrate_to_electron` command keeps
  the previous `.app` at `OfflineGPT.app.migrate-bak`. Instruct users
  to `mv OfflineGPT.app.migrate-bak OfflineGPT.app` if the Electron
  launch fails.
