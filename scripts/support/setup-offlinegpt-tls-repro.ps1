param(
    [string]$Hostname = "poc.offlinegpt.test",
...
function Stop-ReproJobs {
    try {
        $jobs = Get-Job -Name "OfflineGPTTlsRepro-*" -ErrorAction SilentlyContinue
        foreach ($job in $jobs) {
...
function Invoke-Cleanup {
    ...
    if (-not $Quiet) {
        Write-Step "Cleaning OfflineGPT TLS repro artifacts..."
    }
...
function Start-ReproListenerJob {
    ...
    $jobName = "OfflineGPTTlsRepro-{0}" -f $Port
    ...
    $job = Start-Job -Name $jobName -ArgumentList $Port, $Label -ScriptBlock {
        ...
        try {
            while ($true) {
                ...
                $context.Response.Headers.Add("X-OfflineGPT-TLS-Repro", $Label)
...
Write-Step "OfflineGPT TLS repro setup"
...
$rootSubject = "CN=OfflineGPT TLS Repro Root CA, O=$Marker"
$healthyIntermediateSubject = "CN=OfflineGPT TLS Repro Healthy Intermediate CA, O=$Marker"
$brokenIntermediateSubject = "CN=OfflineGPT TLS Repro Broken Intermediate CA, O=$Marker"
...
Write-Step ("   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\offlinegpt-doctor.ps1 -WebUrl {0} -ApiUrl {1} -ExpectedIssuerMatch `"OfflineGPT TLS Repro`"" -f $healthyUrl, $brokenUrl)
...
Write-Step "   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\support\setup-offlinegpt-tls-repro.ps1 -Cleanup"
