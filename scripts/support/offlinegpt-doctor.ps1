...
$emdash = [char]0x2014
Write-Report ("OfflineGPT Network Doctor v1 {0} {1}" -f $emdash, $timestamp)
Write-Report "This entire output is safe to copy/paste back to OfflineGPT support."
...
if ($script:VerdictHints.Count -eq 0) {
    Write-Report "- No likely root cause was detected by this script. If the app still fails, send this full report to OfflineGPT support."
}
else {
