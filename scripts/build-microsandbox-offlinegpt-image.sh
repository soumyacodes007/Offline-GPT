...
IMAGE_REF="${1:-offlinegpt-microsandbox:dev}"
...
printf '  offlinegpt-server@%s\n' "$OFFLINEGPT_SERVER_VERSION"
printf '  opencode@%s\n' "$OPENCODE_VERSION"
...
printf 'Run example:\n'
printf '  docker run --rm -p 8787:8787 -e OFFLINEGPT_CONNECT_HOST=127.0.0.1 %s\n' "$IMAGE_REF"
printf 'Verify:\n'
