...
SNAPSHOT_NAME="${1:-${DAYTONA_SNAPSHOT_NAME:-offlinegpt-runtime}}"
...
LOCAL_IMAGE_TAG="${DAYTONA_LOCAL_IMAGE_TAG:-offlinegpt-daytona-snapshot:${SNAPSHOT_NAME//[^a-zA-Z0-9_.-]/-}}"

OFFLINEGPT_SERVER_VERSION="${OFFLINEGPT_SERVER_VERSION:-$(node -e 'const fs=require("fs"); const pkg=JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(String(pkg.version));' "$ROOT_DIR/apps/server/package.json")}"
...
echo "- offlinegpt-server@$OFFLINEGPT_SERVER_VERSION" >&2
...
docker buildx build \
  ...
  -t "$LOCAL_IMAGE_TAG" \
  -f "$DOCKERFILE" \
  --build-arg "OFFLINEGPT_SERVER_VERSION=$OFFLINEGPT_SERVER_VERSION" \
