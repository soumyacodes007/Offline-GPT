...
function skillCatalogArguments(messages, skillName) {
  ...
  if (!system.includes("You are OfflineGPT.")) throw new Error("The model did not receive OfflineGPT operating instructions");
...
<html>
  ...
  <body style="font-family: system-ui, sans-serif; max-width: 560px; margin: 48px auto;">
    ...
    <p>This fake OAuth provider is for OfflineGPT MCP end-to-end tests.</p>
    ${requestedScopesHtml}
    <form method="post" action="${escapeHtml(`${approveUrl.pathname}${approveUrl.search}`)}">
      <button style="font: inherit; padding: 10px 14px;">Approve OfflineGPT</button>
    </form>
  </body>
</html>`);
}
...
async function registerClient(req, res, entry) {
  ...
  if (entry) {
    ...
    // are the public RFC 7591 fields OfflineGPT is expected to send.
    entry.registration = {
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/v1/models" && req.method === "GET") {
      json(res, 200, {
        object: "list",
        data: [{ id: "mock-agent-workload-model", object: "model", owned_by: "offlinegpt-testkit" }],
      });
      return;
    }
