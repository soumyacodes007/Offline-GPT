...
test("selection prunes only unprotected dev snapshots", () => {
  const snapshots = [
    snapshot("old", "offlinegpt-dev-old", "2026-01-01T00:00:00Z"),
    snapshot("stopped", "offlinegpt-dev-stopped", "2026-01-02T00:00:00Z"),
    snapshot("explicit", "offlinegpt-dev-explicit", "2026-01-03T00:00:00Z"),
    snapshot("pulling", "offlinegpt-dev-pulling", "2026-01-04T00:00:00Z", "pulling"),
    snapshot("active", "offlinegpt-dev-active-ref", "2026-01-05T00:00:00Z"),
    snapshot("recent-1", "offlinegpt-dev-recent-1", "2026-01-06T00:00:00Z"),
    snapshot("recent-2", "offlinegpt-dev-recent-2", "2026-01-07T00:00:00Z"),
    snapshot("release", "offlinegpt-0.18.23", "2026-01-08T00:00:00Z"),
    snapshot("base", "daytonaio/sandbox:0.8.0", "2026-01-09T00:00:00Z"),
    snapshot("windows", "windows-small", "2026-01-10T00:00:00Z"),
  ]
  const sandboxes = [
    { id: "started", state: "started", snapshot: "offlinegpt-dev-active-ref" },
    { id: "stopped", state: "stopped", snapshot: "offlinegpt-dev-stopped" },
    { id: "archived", state: "archived", snapshot: "offlinegpt-dev-stopped" },
  ]
  ...
  const result = selectDevSnapshotPrunes({
    ...
    sandboxes,
    nameBase: "offlinegpt",
    keepNames: ["offlinegpt-dev-explicit"],
    keepCount: 2,
  })

  assert.deepEqual(result.prune.map((item) => item.name), [
    "offlinegpt-dev-old",
    "offlinegpt-dev-stopped",
  ])
  assert.deepEqual(result.keep.map((item) => [item.name, item.reason]), [
    ["offlinegpt-dev-explicit", "explicit"],
    ["offlinegpt-dev-pulling", "in-flight"],
    ["offlinegpt-dev-active-ref", "active-ref"],
    ["offlinegpt-dev-recent-1", "recent"],
    ["offlinegpt-dev-recent-2", "recent"],
  ])
  assert.equal(result.prune.some((item) => !item.name.startsWith("offlinegpt-dev-")), false)
})

test("release selection prunes only unprotected release snapshots", () => {
  const snapshots = [
    snapshot("old", "offlinegpt-0.18.1", "2026-01-01T00:00:00Z"),
    snapshot("rc", "offlinegpt-0.18.2-rc.1", "2026-01-02T00:00:00Z"),
    snapshot("explicit", "offlinegpt-0.18.3", "2026-01-03T00:00:00Z"),
    snapshot("pulling", "offlinegpt-0.18.4", "2026-01-04T00:00:00Z", "pulling"),
    snapshot("active", "offlinegpt-0.18.5", "2026-01-05T00:00:00Z"),
    snapshot("recent-1", "offlinegpt-0.18.6", "2026-01-06T00:00:00Z"),
    snapshot("recent-2", "offlinegpt-0.18.7", "2026-01-07T00:00:00Z"),
    snapshot("dev", "offlinegpt-dev-abc1234", "2026-01-08T00:00:00Z"),
    snapshot("base", "daytonaio/sandbox:0.8.0", "2026-01-09T00:00:00Z"),
    snapshot("windows", "windows-small", "2026-01-10T00:00:00Z"),
  ]
  const sandboxes = [
    { id: "started", state: "started", snapshot: "offlinegpt-0.18.5" },
    { id: "stopped", state: "stopped", snapshot: "offlinegpt-0.18.1" },
  ]
  ...
  const result = selectReleaseSnapshotPrunes({
    ...
    sandboxes,
    nameBase: "offlinegpt",
    keepNames: ["offlinegpt-0.18.3"],
    keepCount: 2,
  })

  assert.deepEqual(result.prune.map((item) => item.name), [
    "offlinegpt-0.18.1",
    "offlinegpt-0.18.2-rc.1",
  ])
  assert.deepEqual(result.keep.map((item) => [item.name, item.reason]), [
    ["offlinegpt-0.18.3", "explicit"],
    ["offlinegpt-0.18.4", "in-flight"],
    ["offlinegpt-0.18.5", "active-ref"],
    ["offlinegpt-0.18.6", "recent"],
    ["offlinegpt-0.18.7", "recent"],
  ])
  assert.equal(
...
test("release channel defaults to keeping the 20 newest release snapshots", async () => {
  ...
  const result = await pruneDaytonaSnapshots({
    apiKey,
    channel: "release",
    keepNames: ["offlinegpt-0.18.20"],
    fetchImpl,
    log: () => {},
  })

  assert.deepEqual(result.prune.map((item) => item.name), ["offlinegpt-0.18.0"])
...
test("release channel never deletes dev snapshots even when unprotected", async () => {
  const snapshots = [
    snapshot("dev-old", "offlinegpt-dev-old", "2026-01-01T00:00:00Z"),
    snapshot("release-old", "offlinegpt-0.18.1", "2026-01-02T00:00:00Z"),
    snapshot("release-new", "offlinegpt-0.18.2", "2026-01-03T00:00:00Z"),
  ]
  const { calls, fetchImpl } = inventoryFetch(snapshots)
...
test("parseArgs resolves channel and rejects unknown channels", () => {
  ...
  assert.equal(parseArgs(["--keep", "offlinegpt-dev-a"]).channel, "dev")
  assert.throws(
    () => parseArgs(["--channel", "nightly", "--keep", "offlinegpt-dev-a"]),
    /Invalid channel/,
  )
})
...
test("exactly keepCount in-scope snapshots prunes nothing", () => {
  const snapshots = [
    snapshot("one", "offlinegpt-dev-one", "2026-01-01T00:00:00Z"),
    snapshot("two", "offlinegpt-dev-two", "2026-01-02T00:00:00Z"),
  ]
  const result = selectDevSnapshotPrunes({
    snapshots,
    sandboxes: [],
...
test("dry-run fetches inventory but sends no DELETE requests", async () => {
  const snapshots = [
    snapshot("old", "offlinegpt-dev-old", "2026-01-01T00:00:00Z"),
    snapshot("new", "offlinegpt-dev-new", "2026-01-02T00:00:00Z"),
  ]
  const { calls, fetchImpl } = inventoryFetch(snapshots)

  const result = await pruneDaytonaDevSnapshots({
    apiKey,
    keepNames: ["offlinegpt-dev-new"],
    keepCount: 1,
    dryRun: true,
    fetchImpl,
    log: () => {},
...
test("real run deletes every selected id and no kept ids", async () => {
  const snapshots = [
    snapshot("old-1", "offlinegpt-dev-old-1", "2026-01-01T00:00:00Z"),
    snapshot("old-2", "offlinegpt-dev-old-2", "2026-01-02T00:00:00Z"),
    snapshot("new", "offlinegpt-dev-new", "2026-01-03T00:00:00Z"),
  ]
  const { calls, fetchImpl } = inventoryFetch(snapshots)
...
test("pagination protects newest snapshots and reads all sandbox pages", async () => {
  const firstPage = Array.from({ length: 200 }, (_, index) =>
    snapshot(
      `old-${index}`,
      `offlinegpt-dev-old-${String(index).padStart(3, "0")}`,
      `2025-01-${String((index % 28) + 1).padStart(2, "0")}T00:00:00Z`,
    ),
  )
  const newest = snapshot("newest", "offlinegpt-dev-newest", "2026-01-01T00:00:00Z")
  const calls = []
  const fetchImpl = async (url, options) => {
    calls.push({ url, options })
    if (url.includes("/snapshots?")) {
