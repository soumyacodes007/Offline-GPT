#!/usr/bin/env bash
#
# offlinegpt-debug.sh — one-stop observability + lifecycle control for the
# OfflineGPT dev stack.
#
# Subcommands:
...
#   kill-orphans    remove orphan offlinegpt/opencode processes (ppid == launchd)
...
#   wait-healthy    block until offlinegpt-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Dev app: pnpm --filter @offlinegpt/desktop dev:electron launches the Electron
...
#   4. opencode sidecars and offlinegpt-server helpers
#
...
DEV_LOG_FILE="${OFFLINEGPT_DEV_LOG_FILE:-$HOME/.offlinegpt/debug/offlinegpt-dev.log}"
PNPM_DEV_LOG="${OFFLINEGPT_PNPM_DEV_LOG:-/tmp/offlinegpt-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${OFFLINEGPT_PNPM_DEV_PID:-/tmp/offlinegpt-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${OFFLINEGPT_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT:-9823}"

# ---------------------------------------------------------------------------
# Helpers

log() { printf '[offlinegpt-debug] %s\n' "$*"; }
...
discover_offlinegpt_server_port() {
  ps -Ao command \
    | grep -E "offlinegpt-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  ps -Ao pid,ppid,command | awk '/node_modules\/electron\/dist\/Electron\.app\/Contents\/MacOS\/Electron|apps\/desktop\/scripts\/electron-dev\.mjs|apps\/desktop\/resources\/sidecars\/opencode( |\/)|offlinegpt-server|vite|pnpm .*dev/ && !/awk/ && !/grep/' | sed -E 's#/Users/[^ ]*/#…/#g' | head -20

  echo
  echo "=== offlinegpt-server ==="
  local port
  port=$(discover_offlinegpt_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev offlinegpt-server running)"
  else
  ...
  ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /offlinegpt-server|opencode( |\/)/' | head
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with OFFLINEGPT_DEV_LOG_FILE set)"
  fi
}
...
kill_orphans() {
  local pids
  pids=$(ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /offlinegpt-server|opencode( |\/)/ {print $1}')
  if [[ -z "$pids" ]]; then
    log "no orphans"
    return 0
  fi
...
diagnose_hang() {
  ...
  echo "=== offlinegpt hang diagnosis ==="
  echo "time=$now"
  echo "repo=$REPO_ROOT"
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(OFFLINEGPT_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_offlinegpt_server_port)
  if [[ -n "$port" ]]; then
    echo "  offlinegpt-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no offlinegpt-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/offlinegpt-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set OFFLINEGPT_FORCE_SAMPLE=1 not currently supported."
  else
...
stop() {
  ...
  kill_by_pattern "offlinegpt-server"
...
start() {
  ...
  env OFFLINEGPT_DEV_LOG_FILE="$DEV_LOG_FILE" \
    nohup pnpm --filter @offlinegpt/desktop dev:electron >"$PNPM_DEV_LOG" 2>&1 &
  pid=$!
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "offlinegpt-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "offlinegpt-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
